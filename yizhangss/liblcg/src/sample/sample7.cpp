...
public:
	...
	//定义共轭梯度中Ax的算法
	void AxProduct(const vector_cd &x, vector_cd &prod_Ax, matrix_layout_e layout, complex_conjugate_e conjugate)
	{
	...
	void MxProduct(const vector_cd &x, vector_cd &prod_Mx, matrix_layout_e layout, complex_conjugate_e conjugate)
	{
		// No preconditioning
		//prod_Mx = x;
		...
		solve_lower_triangle(l_tri, x, p);
		solve_upper_triangle(u_tri, p, prod_Mx);
		return;
	}
...
void TESTFUNC::set_preconditioner()
{
	...
	*/
	...
	incomplete_Cholesky(kernel, l_tri);
