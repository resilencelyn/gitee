...
public class DlxCardFlow {
     ...
     */
    @RabbitHandler
    @RabbitListener(queues = "polling_card_flow_tow")
        ...
        if (ChannelArr != null && ChannelArr.size() > 0) {
            ...
            try {
                ...
                for (int i = 0; i < ChannelArr.size(); i++) {
                    ...
                    Object isExecute = redisCache.getCacheObject("polling-dex-Listener-CardFlow-" + cd_id);
                    if (isExecute == null) {
                        //redis 存储
                        ChannelObj.put("pollingtime", VeDate.getStringDateShort());
                        redisCache.setCacheObject("polling-dex-Listener-CardFlow-" + cd_id, ChannelObj);// 缓存 避免 重复创建 消费 监听
    ...
    class SynTask implements Runnable {
        ...
        public void activateConsumer(String cd_id, Map<String, Object> ChannelObj, Channel channel) {
            ...
            try {
                ...
                EXCHANGE_NAME = "polling_dlxcardCardFlow_exchange";
                QUEUE_NAME = "polling_dlxcardCardFlow_queue_" + cd_id;
                ROUTINGKEY = "polling.dlxcardCardFlow.routingKey." + cd_id;
                ...
                DefaultConsumer defaultConsumer = new DefaultConsumer(channel) {
                    ...
                    public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                        ...
                        //System.out.println("polling_dlxcardCardFlow_queue_"+cd_id+" 消费者获取消息:" + msg);
                ...
                try {
                    ...
                    log.error(">> 创建 消费者失败 > 删除 redis记录Key polling-dex-Listener-CardFlow-{} | {}<<", cd_id);
                } catch (Exception e1) {
                    log.error(">> 创建 消费者失败 > 删除 redis记录Key polling-dex-Listener-CardFlow-{} 失败 | {}<<", cd_id, e1.getMessage());
                }
            }
        }
