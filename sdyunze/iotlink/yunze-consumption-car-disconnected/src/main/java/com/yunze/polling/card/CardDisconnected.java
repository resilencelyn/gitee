...
public class CardDisconnected {
     ...
     */
    @RabbitHandler
    @RabbitListener(queues = "polling_card_Disconnected_one")
    ...
    public void activateConsumer(String cd_id,Channel channel){
        try {
            ...
            EXCHANGE_NAME = "polling_cardCardDisconnected_exchange";
            QUEUE_NAME = "polling_cardCardDisconnected_queue_"+cd_id;
            ROUTINGKEY = "polling.cardCardDisconnected.routingKey."+cd_id;
            ...
            DefaultConsumer defaultConsumer = new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    ...
                    //System.out.println("polling_cardCardDisconnected_queue_"+cd_id+" 消费者获取消息:" + msg);
            ...
            log.error(">>DB - 未订购资费停机轮询 创建 轮序消费者 同步未订购资费停机轮询失败:{} | {}<<",e.getMessage().toString());
        }
    }
            ...
            if(isExecute==null){
                ...
                Map<String,Object> Rmap =  internalApiRequest.queryFlow(Parammap,map);
                    ...
                    if(code.equals("200")){
                        ...
                        if(Rmap.get("Use")!=null &&  Rmap.get("Use")!="" &&  Rmap.get("Use").toString().trim().length()>0){
                            ...
                            if(Use>=0){
                                try {
                                    ...
                                    log.info(">>cardFlowSyn - 未订购资费停机轮询-卡用量轮序消费者 同步卡用量返回:{} | {}<<",iccid,JSON.toJSON(cardFlowSynRMap));
                                    ...
                                    log.error(">>cardFlowSyn - 未订购资费停机轮询-卡用量轮序消费者 同步卡用量失败:{} | {}<<",iccid,e.getMessage());
                                }
                            }else{
                        ...
                        log.info(">>API - 未订购资费停机轮询-卡用量轮序消费者 未获取到卡用量:{} | {}<<",iccid,Rmap);
                    }
            }
        } catch (Exception e) {
            log.error(">>错误 - 未订购资费停机轮询轮序消费者:{}<<", e.getMessage().toString());
        }
    }
