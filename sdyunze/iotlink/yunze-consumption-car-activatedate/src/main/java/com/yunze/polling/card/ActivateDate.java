...
public class ActivateDate {
     ...
     */
    @RabbitHandler
    @RabbitListener(queues = "polling_card_activateDate_one")
    ...
    public void RunEx(List<String> Ex_ChannelArr,List<Map<String, Object>> ChannelArr,boolean bool){
        ...
        if(ChannelArr!=null && ChannelArr.size()>0){
            ...
            for (int i = 0; i < ChannelArr.size(); i++) {
                ...
                Object  isExecute = redisCache.getCacheObject("polling-Listener-ActivateDate-"+cd_id );
                if(isExecute==null || bool==false){
                    //redis 存储
                    ChannelObj.put("pollingtime", VeDate.getStringDateShort());
                    redisCache.setCacheObject("polling-Listener-ActivateDate-"+cd_id, ChannelObj);// 缓存 避免 重复创建 消费 监听
                    try {
    ...
    public void activateConsumer(String cd_id,Map<String,Object> ChannelObj,Channel channel){
        try {
            ...
            EXCHANGE_NAME = "polling_cardActivateDate_exchange";
            QUEUE_NAME = "polling_cardActivateDate_queue_"+cd_id;
            ROUTINGKEY = "polling.cardActivateDate.routingKey."+cd_id;
            ...
            DefaultConsumer defaultConsumer = new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    ...
                    //System.out.println("polling_cardActivateDate_queue_"+cd_id+" 消费者获取消息:" + msg);
            ...
            try {
                redisCache.redisTemplate.delete("polling-Listener-ActivateDate-"+cd_id);
                log.error(">> 创建 消费者失败 > 删除 redis记录Key polling-Listener-ActivateDate-{} | {}<<", cd_id);
            }catch (Exception e1){
                log.error(">> 创建 消费者失败 > 删除 redis记录Key polling-Listener-ActivateDate-{} 失败 | {}<<", cd_id, e1.getMessage());
            }
        }
    }
                    ...
                    if(Rmap.get("activateDate")!=null &&  Rmap.get("activateDate")!="" &&  Rmap.get("activateDate").toString().trim().length()>0){
                        ...
                        try {
                           ...
                           rabbitTemplate.convertAndSend(card_exchangeName, card_routingKey, JSON.toJSONString(Upd_Map), message -> {
                            ...
                            log.info(">>发送 变更激活时间 指令成功 ！ iccid:{} | upd:{} | Upd_Map {}<<", iccid,upd, Upd_Map);
                        } catch (Exception e) {
