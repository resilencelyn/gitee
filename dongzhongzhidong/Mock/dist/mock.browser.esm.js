...
const realFetch = window$2.fetch;
function mockFetch() {
    // 代理 fetch 的初始化函数
    if (window$2.fetch && !window$2.fetch.hasOwnProperty('$mock')) {
        window$2.fetch = fakeFetch;
        window$2.fetch.$mock = true;
        console.warn('fetch 已经被代理');
    }
}

async function fakeFetch(url, options = {}) {
    if (window$2.fetch.$mock === true) {
...
const XMLHttpRequest = window$1 !== undefined ? window$1.XMLHttpRequest : class NULL {};
...
// 拦截 XHR
if (window.XMLHttpRequest !== MockXMLHttpRequest) window.XMLHttpRequest = MockXMLHttpRequest;
Mock.XHR = MockXMLHttpRequest;
Mock.fetch = fetch;
mockFetch();
Mock.setup = (settings) => MockXMLHttpRequest.setup(settings);Mock.version = '1.1.1-es6';
