...
static void save_packet(struct dump_ring *r, struct dump_header *pkt, u32 pos)
{
	pcaprec_hdr_t hdr;
	u32 left;
	...
	write(dump_conf.outfd, &hdr, sizeof(hdr));

	left = r->size - pos;

	if (left >= pkt->len) {
		write(dump_conf.outfd, r->pkt + pos, pkt->len);
		return;
	}

	write(dump_conf.outfd, r->pkt + pos, left);
	write(dump_conf.outfd, r->pkt, pkt->len - left);
...
static void consume(struct dump_ring *r)
{
	pcap_hdr_t hdr;
...
int main(int argc, char *argv[])
{
	...
	if (*dump_conf.outfile == '-')
		...
		dump_conf.outfd = open(dump_conf.outfile, O_WRONLY | O_CREAT);
