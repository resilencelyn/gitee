...
public class MycatConfig {
	...
	public MycatConfig() {
		...
		if (YesOrNo.YES.getCode() == this.system.getUseInternalEncryptAlgorithm()) {
			String secretKey = this.system.getSecretKey();
			if (secretKey == null || secretKey.length() != 16) {
				LOGGER.error("server.xml -> secretKey must be a 16-bit string!");
				throw new RuntimeException("server.xml -> secretKey must be a 16-bit string!");
			}
			//如果使用内置加密算法，给密钥赋值
		...
		//zrx 设置加密
		this.encryptConfigMap = confInit.getEncryptConfigMap();
