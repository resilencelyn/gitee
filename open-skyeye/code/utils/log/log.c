...
int instr_log_parse(struct skyeye_option_t *option, int num_params, const char *params[])
{
    ...
    for (i = 0; i < num_params; i++)
    {
        if (split_param(params[i], name, value) < 0)
            SKYEYE_ERR("Error: %s option has wrong parameter \"%s\".\n", log_option_name, name);
        ...
        else if (!strncmp("filename", name, strlen(name)))
        {
            ...
            if (env == NULL)
                return -1;
            character_count = strlen(env) + strlen("/") + strlen(".skyeye") + +strlen("/") + strlen(log_filename) + 1;
            if (character_count > 256)
                return -1;
            sprintf(filepath, "%s/.skyeye/%s", env, log_filename);
            sprintf(dirpath, "%s/.skyeye", env);
            int ret;
