...
int do_os_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    ...
    for (i = 0; i < MAX_SUPP_OS; i++)
    {
        if (skyeye_os[i] == NULL)
            continue;
        if (!strncasecmp(params[0], skyeye_os[i]->os_name, MAX_PARAM_NAME))
        {
    ...
    for (i = 0; i < MAX_SUPP_OS; i++)
    {
        if (!strncasecmp(default_os_name, skyeye_os[i]->os_name, MAX_PARAM_NAME))
        {
            config->os = skyeye_os[i];
