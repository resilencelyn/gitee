...
static int do_code_cov_option(skyeye_option_t * this_option, int num_params, const char *params[]);
...
int cov_conf_parse(struct skyeye_option_t *option, int num_params, const char *params[])
{
    ...
    for (i = 0; i < num_params; i++)
    {
        if (split_param(params[i], name, value) < 0)
            SKYEYE_ERR("Error: code_coverage option has wrong parameter \"%s\".\n", name);
...
void cov_state_on(char *arg)
{
    ...
    if (cov_state == COV_START)
    {                                   //run this function first time
