...
/* python initialize on main for ubuntu 12.04 or laster */
#include <Python.h>
void python_init(void)
{
    char new_path[1024];
    char *skyeye_bin = "/opt/skyeye/bin/";
...
static void base_termios_exit(void)
{
    ...
    else
    {
        do
        {
            ...
            if (skyeye_config.log.logon >= 1)
            {
                ...
                if (pc >= skyeye_config.log.start && pc <= skyeye_config.log.end)
                {
            ...
            if (skyeye_ice.tps_status == TRACE_STARTED)
            {
                for (i = 0; i < skyeye_ice.num_tps; i++)
                {
                    if (((skyeye_ice.tps[i].tp_address == addr) &&
                         (skyeye_ice.tps[i].status == TRACEPOINT_ENABLED))
                        || (skyeye_ice.tps[i].status == TRACEPOINT_STEPPING))
                    {
                        handle_tracepoint(i);
                    }
...
void usage()
{
    ...
    printf("-n               Non-interactive mode, i.e. command line is not available.\n");
    ...
    printf("-c config-file   The Skyeye configuration file name.\n");
    printf("-h               The SkyEye command options, and ARCHs and CPUs simulated.\n");
    printf("-u               User application emulation. To use with -e.\n");
    printf("-m               Direct mapping to memory (in User application emulation).\n");
    printf("                 Faster but may not be compatible with some applications.\n");
    printf("--------------------------------------------------------------------------------\n");
}
...
static BOOL init_win32_socket()
{
    ...
    if (WSAStartup(0x202, &wsdData) != 0 || LOBYTE(wsdData.wVersion) != 2 || HIBYTE(wsdData.wVersion) != 2)
        return FALSE;
    return TRUE;
}
...
int init_option(int argc, char **argv, sky_pref_t * pref)
{
    ...
    bool_t autotest_mode = False;
    bool_t autoload_mode = False;
    ...
    opterr = 0;
    while ((c = getopt(argc, argv, "Gbe:as:t:dc:p:nl:humig:q:")) != -1)
        switch (c)
        {
            case 'e':
                exec_file = optarg;
  ...
  loop_exit:
    ...
    pref->autotest = autotest_mode;
    pref->autoload = autoload_mode;
    pref->net_port = net_port;
