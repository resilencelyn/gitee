...
static int do_shutdown_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    int ret;
    char *value;
...
void module_init()
{
    ...
    if (register_option
        ("shutdown_device", do_shutdown_option,
         "Used to stop machine by writing a special address or set the max executed instruction number.") != No_exp)
