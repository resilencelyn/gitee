        ...
        while (current_record != NULL)
        {
            if (current_record->ca->type == COLLECT_MEMORY)
            {
                if ((addr >= current_record->ca->description.mc.offset)
                    && (addr + size <=
                        current_record->ca->description.mc.offset + current_record->ca->description.mc.length))
                {
                    memcpy(buffer, (current_record->collect_data) + (addr - current_record->ca->description.mc.offset),
                           size);
                    return (size);
                } else
                {
            ...
            case ACTION_COLLECT:
                {
                    switch (current_action->action_data.ca.type)
                    {
                        case COLLECT_REGISTERS:
                            {
                                current_collect_record->collect_data =
                                    (char *) malloc(current_reg_type->register_bytes);
                        ...
                        case COLLECT_MEMORY:
                            {
                                current_collect_record->collect_data =
                                    (char *) malloc(current_action->action_data.ca.description.mc.length);
                                ...
                                } else
                                {
                                    ...
                                    fetch_inferior_registers(current_action->action_data.ca.description.mc.base_reg,
                                                             registers);
                                    base_reg_val =
                                        registers[current_reg_type->
                                                  register_byte(current_action->action_data.ca.description.mc.
                                                                base_reg)];
                                    free(registers);
