...
int 
traffic_generator::traffic_send
( unsigned base_addr,
  unsigned  offset,
  void *buf,
  unsigned  len
...
int traffic_generator::traffic_receive
( unsigned base_addr,
  unsigned  offset,
  void *buf,
  unsigned  len
)
{
  ...
  sc_dt::uint64 base_address = base_addr;
	...
	++m_active_txn_count;
      ...
      check_complete();
      memcpy(buf, transaction_ptr->get_data_ptr(), transaction_ptr->get_data_length()); 
      check_all_complete();
}
...
)
{
  ...
  printf("=========================================================\n"); 
   printf("%s, %s, %s\n", filename, __FUNCTION__,  "####  Traffic Generator Complete  #### " );
