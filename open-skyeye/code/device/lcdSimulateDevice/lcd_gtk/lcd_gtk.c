...
//		case 2:
		...
		case 32:
			if(lcd->endian == Big_endian) {
				for (i = lcd->virtual_width * rect.y; i < wordnum; i++) {
					uint32 bgra = ((guint32*)lcd->fbmem)[i];
					/* big endian: argb >>> little endian: bgra
					 * and we should translate: bgra >>> abgr
					 */
					((guint32*)lcd->rgbbuf)[i] = ((bgra & 0xffffff00) >> 8) | ((bgra & 0x000000ff) << 24);
				}
			}
			gdk_draw_rgb_32_image(widget->window,
					      widget->style->fg_gc[GTK_STATE_NORMAL],
...
static int gtk_lcd_open(conf_object_t *lcd_dev, lcd_surface_t* surface)
{
	...
	lcd_touchscreen_t* lcd_ts = SKY_get_iface(lcd_dev, LCD_TS_INTF_NAME);
	SkyEyeLCD_GTK *lcd;
	guint32 *fbmem;
	char *title;
	...
	generic_arch_t *arch = get_arch_instance(NULL);
	lcd->endian = arch->endianess;
	...
	if(dev->lcd_lookup_color != NULL) {
		...
		case 32:
			if(lcd->endian == Big_endian)
				lcd->rgbbuf = (guchar*)malloc(lcd->virtual_width * lcd->height * 4);
			else
...
static conf_object_t* new_gtk_lcd(char* obj_name)
{
	...
	SKY_register_iface(refresh_signal, obj_name, SIMPLE_SIGNAL_INTF_NAME);
	dev->master = refresh_signal;
#endif
	return dev->obj;
}
static void gtk_lcd_register_interface(conf_class_t* class){
	static const lcd_control_intf lcd_ctrl = {
		.lcd_open = gtk_lcd_open,
		.lcd_close = gtk_lcd_close,
		.lcd_update = gtk_lcd_update,
		.lcd_filter_read = NULL,
		.lcd_filter_write = gtk_lcd_filter_write,
		.lcd_lookup_color = NULL,
	};
	SKY_register_iface(class, LCD_CTRL_INTF_NAME, &lcd_ctrl);
	simple_signal_intf refresh_signal = {
		.trigger = NULL,
	};
	SKY_register_iface(class, SIMPLE_SIGNAL_INTF_NAME, &refresh_signal);
...
void init_gtk_lcd(){
	...
	gtk_lcd_register_interface(class);
