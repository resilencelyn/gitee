...
char* s3c6410_lcd_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	s3c6410_fb_device *dev = conf_obj->obj;
	return dev->regs_name[id];
}

uint32_t s3c6410_lcd_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id){
	s3c6410_fb_device *dev = conf_obj->obj;
...
static exception_t s3c6410_lcd_signal_set(conf_object_t *obj, conf_object_t *obj2, const char *port, int index)
{
	s3c6410_fb_device* dev = (s3c6410_fb_device*)(obj->obj);
	dev->signal_target = obj2;
	dev->master = (general_signal_intf *)SKY_get_iface(dev->signal_target, GENERAL_SIGNAL_INTF_NAME);

	if (dev->master == NULL){
		if (dev->signal_target == NULL)
		{
...
static exception_t s3c6410_lcd_signal_get(conf_object_t *obj, conf_object_t **obj2, const char **port, int index)
{
	s3c6410_fb_device* dev = (s3c6410_fb_device*)(obj->obj);
	*obj2 = dev->signal_target;
	*port = NULL;
	return No_exp;
}

static attr_value_t get_attr_lcd_ctrl(void* arg, conf_object_t* conf_obj, attr_value_t* idx){
	s3c6410_fb_device* dev = (s3c6410_fb_device*)(conf_obj->obj);
	attr_value_t lcd_ctrl = SKY_make_attr_string(dev->lcd_ctrl);
	return lcd_ctrl;
}
static exception_t set_attr_lcd_ctrl(void* arg, conf_object_t* conf_obj, attr_value_t* value, attr_value_t* idx){
	s3c6410_fb_device* dev = (s3c6410_fb_device*)(conf_obj->obj);
	dev->lcd_ctrl = SKY_attr_string(*value);
	return No_exp;
}

static void s3c6410_lcd_register_attribute(conf_class_t* clss) {
	SKY_register_attribute(clss, "lcd_ctrl_0", get_attr_lcd_ctrl, NULL, set_attr_lcd_ctrl, NULL, SKY_Attr_Optional, "string", "lcd_ctrl attribute");
...
void init_s3c6410_lcd(){
	...
	static const memory_space_intf io_memory = {
		.read = s3c6410_fb_read,
		.write = s3c6410_fb_write,
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);

	static const simple_signal_intf refresh_signal = {
		.trigger = refresh_trigger,
	};
	SKY_register_iface(clss, SIMPLE_SIGNAL_INTF_NAME, &refresh_signal);

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = s3c6410_lcd_get_regvalue_by_id,
		.get_regname_by_id = s3c6410_lcd_get_regname_by_id,
	...
	static const struct InterfaceDescription ifaces[] = {
		(struct InterfaceDescription) {
			.name = SIMPLE_SIGNAL_INTF_NAME,
			.iface = &refresh_signal,
		},
		(struct InterfaceDescription) {
	...
	static const struct ConnectDescription connects[] = {
		(struct ConnectDescription) {
			...
			.set = s3c6410_lcd_signal_set,
			.get = s3c6410_lcd_signal_get,
		}
	};
	class_register_ifaces(clss, ifaces);
	class_register_connects(clss, connects);

	s3c6410_lcd_register_attribute(clss);
	return;
}
