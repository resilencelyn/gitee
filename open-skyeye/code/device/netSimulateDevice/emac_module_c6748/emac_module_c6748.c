...
{
	emac_module_c6748_device *dev = opaque->obj;
...
static void do_emac_tx(conf_object_t *opaque, uint32_t *txnhdp, uint32_t *txncp, int index)
{
	...
	while (guest_addr) {
		*txncp = guest_addr;
        ...
        else{ // not SOP, update pkt_flg for get correct EOP
            sop_desc->pkt_flg = ppos->pkt_flg;
        }
		if (ispktf) {
			if (dev->memory_iface)
			{
...
{
	emac_module_c6748_device *dev = ((conf_object_t *)opaque)->obj;
	get_page_t *page;
	buff_desc_t *ppos, *sop_desc;
...
static exception_t emac_module_c6748_read(conf_object_t *opaque, generic_address_t offset, void *buf, size_t count)
{
	emac_module_c6748_device *dev = opaque->obj;
	switch (offset)
	{
		...
		case RXDMAOVERRUNS:
			*(uint32_t *)buf = dev->regs.rxdmaoverruns;
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__, "Can not read the register at 0x%x in emac_module_c6748\n", offset);
			break;
	}
	return 0;
}
static exception_t emac_module_c6748_write(conf_object_t *opaque, generic_address_t offset, void *buf, size_t count)
{
	emac_module_c6748_device *dev = opaque->obj;
	...
	switch (offset)
	{
		...
		case RXDMAOVERRUNS:
			...
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__, "Cannot write the register at 0x%x in emac_module_c6748\n", offset);
			break;
	}
	return 0;
}
static char* emac_module_c6748_get_regname_by_id(conf_object_t *conf_obj, uint32_t id)
{
	return regs_name[id];
}
static exception_t emac_module_c6748_set_regvalue_by_id(conf_object_t *conf_obj, uint32_t value, uint32_t id)
{
	emac_module_c6748_device *dev = conf_obj->obj;
	struct registers *regs = &(dev->regs);
	*((uint32_t*)regs + id) = value;
	return No_exp;
}
static uint32_t emac_module_c6748_get_regnum(conf_object_t *conf_obj, uint32_t id)
{
	return  sizeof(struct registers) / 4;
}
static uint32_t emac_module_c6748_get_regvalue_by_id(conf_object_t *conf_obj, uint32_t id)
{
	emac_module_c6748_device *dev = conf_obj->obj;
	struct registers *regs = &(dev->regs);
...
static uint32_t emac_module_c6748_get_regid_by_name(conf_object_t *conf_obj, char *name)
{
...
{
	emac_module_c6748_device *dev = obj->obj;
...
static conf_object_t* new_emac_module_c6748(char *obj_name)
{
	emac_module_c6748_device* dev = skyeye_mm_zero(sizeof(emac_module_c6748_device));
...
static exception_t free_emac_module_c6748(conf_object_t* opaque)
{
	emac_module_c6748_device *dev = opaque->obj;
...
void init_emac_module_c6748(void)
{
	static skyeye_class_t class_data =
	{
		.class_name = "c6748_emac_module",
		.class_desc = "c6748_emac_module",
		.new_instance = new_emac_module_c6748,
		.free_instance = free_emac_module_c6748,
		.module_type = SKYML_ONCHIP(SC_c6748),
	};
	conf_class_t* clss = SKY_register_device_class(class_data.class_name, &class_data);
	static memory_space_intf io_memory =
	{
		.read = emac_module_c6748_read,
		.write = emac_module_c6748_write
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);
	static skyeye_reg_intf reg_intf =
	{
		.get_regvalue_by_id = emac_module_c6748_get_regvalue_by_id,
		.get_regname_by_id = emac_module_c6748_get_regname_by_id,
		.set_regvalue_by_id = emac_module_c6748_set_regvalue_by_id,
		.get_regnum = emac_module_c6748_get_regnum,
		.get_regid_by_name = emac_module_c6748_get_regid_by_name
	};
