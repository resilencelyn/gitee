...
static int is_tap_win32_dev(const char *guid)
{
	...
	for(i = 0; found < 0; i++)
	{
		...
		if(RegQueryValueEx(unit_key, component_id_string, NULL,
				   &data_type, (BYTE*)component_id, &len) == ERROR_SUCCESS && data_type == REG_SZ)
		{
			len = sizeof(net_cfg_instance_id);
			if(RegQueryValueEx(unit_key, net_cfg_instance_id_string, NULL,
					   &data_type, (BYTE*)net_cfg_instance_id, &len) == ERROR_SUCCESS && data_type == REG_SZ)
			{
...
static int initDevice(struct net_device *net_dev)
{
	...
	memset(&tapVer[0], 0, sizeof(tapVer));
...
static int recvPacket(tap_win32 *dev, void *buf, int size)
{
	...
	if(size < 0 || dev == NULL) return -1;

	if ((retVal = waitRecvPacket(dev, &tv)) != 0) return -1;
...
static int tuntap_open(struct net_device *net_dev)
{
...
static int tuntap_close(struct net_device *net_dev)
{
...
static int tuntap_read(struct net_device *net_dev, void *buf, size_t count)
{
...
static int tuntap_write(struct net_device *net_dev, void *buf, size_t count)
{
	if(net_dev->priv == NULL) return -1;
	return sendPacket((tap_win32*)net_dev->priv, buf, count);
}

static int tuntap_wait_packet(struct net_device *net_dev, struct timeval *tv)
{
	if(net_dev->priv == NULL) return -1;
...
static exception_t set_attr_ip(void* arg, conf_object_t* conf_obj, attr_value_t* value, attr_value_t* idx){
	struct net_device *dev = (struct net_device *)conf_obj->obj;
	char *ip = (char *)SKY_attr_string(*value);
    char *tmp_ip;
    int i = 0;

    dev->hostip[i] = atoi(strtok(ip, "."));
    while(tmp_ip = strtok(NULL, "."))
    {
        i++;
        dev->hostip[i] = atoi(tmp_ip);
    }

	return No_exp;
}

static void win_netsim_register_attribute(conf_class_t* clss) {
...
static conf_object_t* new_win_netsim(const char* obj_name){
	struct net_device* dev = (struct net_device *)skyeye_mm_zero(sizeof(struct net_device));
	dev->obj = new_conf_object(obj_name, dev);
	dev->priv = LocalAlloc(LMEM_ZEROINIT, sizeof(tap_win32));

	return dev->obj;
}
static exception_t free_win_netsim(conf_object_t* obj) {
	struct net_device *dev = (struct net_device *)obj->obj;
...
extern "C" void init_win_netsim(){
	static skyeye_class_t class_data;
	class_data.class_name = "netcard_simulation_level";
	class_data.class_desc = "win_netsim";
	class_data.new_instance = new_win_netsim;
	class_data.free_instance = free_win_netsim;
	class_data.reset_instance = NULL;
	class_data.get_attr = NULL;
	class_data.set_attr = NULL;

	conf_class_t* clss = SKY_register_device_class(class_data.class_name, &class_data);

	static skyeye_net_ops_intf net_ops_intf;
	net_ops_intf.net_open = tuntap_open;
	net_ops_intf.net_close = tuntap_close;
	net_ops_intf.net_read = tuntap_read;
	net_ops_intf.net_write = tuntap_write;
	net_ops_intf.net_wait_packet = tuntap_wait_packet;

	SKY_register_iface(clss, SKYEYE_NET_INTF_NAME, &net_ops_intf);
	static struct InterfaceDescription ifaces[] = {
		{SKYEYE_NET_INTF_NAME, &net_ops_intf}
	};
	...
	win_netsim_register_attribute(clss);
}
