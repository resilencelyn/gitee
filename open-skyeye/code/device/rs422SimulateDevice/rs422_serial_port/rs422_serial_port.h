...
#ifndef __rs422_serial_port_H__
#define __rs422_serial_port_H__

typedef struct reg_send_cache{
	uint32_t send_empty :1;
	uint32_t reserved :31;
}send_cache_t;
typedef struct reg_recv_cache{
	uint32_t recv_full :1;
	uint32_t reserved :31;
}recv_cache_t;
...
typedef struct reg_status{
	uint32_t recv_finish:1;
	uint32_t send_finish:1;
	uint32_t reserved :31;
}stu_t;

#define FIFO_SIZE	1024
typedef struct rs422_serial_port_reg{
	union {
		uint32_t value;
		send_cache_t flag;
	}send_cache;
...
}rs422_serial_port_reg_t;

typedef struct rs422_serial_port_device_t{
	conf_object_t* obj;
	rs422_serial_port_reg_t* regs;
	struct fifo{
