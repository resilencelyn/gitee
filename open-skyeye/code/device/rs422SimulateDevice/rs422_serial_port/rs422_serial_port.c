...
static int rs422_serial_port_set_state(conf_object_t* obj, char *buf, int Length)
{
	rs422_serial_port_device *dev = obj->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
...
static exception_t rs422_serial_port_read(conf_object_t *opaque, generic_address_t offset, void* buf, size_t count)
{
	struct rs422_serial_port_device_t *dev = opaque->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
	switch(offset) {
		case 0x0:
			break;
		...
		case 0x8:
			if (dev->bus_iface != NULL){
				*(uint16_t *)buf = dev->recv_buf[dev->recv_cnt];
				dev->recv_cnt ++;
				if (dev->recv_cnt == regs->recv_data_length){
					dev->recv_cnt = 0;
					regs->stu_reg.flag.recv_finish = 0;
					memset(dev->recv_buf, 0, sizeof(regs->recv_data_length));
				}
			}else{
		...
		case 0x3c:
			*(uint16_t *)buf = regs->count3_reg;
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__, "Can not read the register at 0x%x in rs422_serial_port\n", offset);
			return Invarg_exp;
	}
	return No_exp;
}
...
void Save_Data_to_Cache(conf_object_t *opaque, char *buf, int data_length){
	struct rs422_serial_port_device_t *dev = opaque;
	int i;
...
static exception_t rs422_serial_port_write(conf_object_t *opaque, generic_address_t offset, uint32_t* buf, size_t count)
{
	struct rs422_serial_port_device_t *dev = opaque->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
	...
	switch(offset) {
		...
		case 0x3c:
			...
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__, "Can not write the register at 0x%x in rs422_serial_port\n", offset);
			return Invarg_exp;
	}
	return No_exp;
}

static char* rs422_serial_port_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	return regs_name[id];
}

static uint32_t rs422_serial_port_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id){
	rs422_serial_port_device *dev = conf_obj->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
	uint32_t* regs_value = (uint32_t*)regs + id;
	return *regs_value;
}

static char* rs422_serial_port_set_regvalue_by_id(conf_object_t* conf_obj, uint32_t value, uint32_t id)
{
	rs422_serial_port_device *dev = conf_obj->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
...
static uint32_t rs422_serial_port_get_regnum(conf_object_t* conf_obj, uint32_t id)
{
	rs422_serial_port_device *dev = conf_obj->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
	return  16;
...
static uint32_t rs422_serial_port_get_regid_by_name(conf_object_t* conf_obj, char *name)
{
	rs422_serial_port_device *dev = conf_obj->obj;
	rs422_serial_port_reg_t* regs = dev->regs;
	uint32_t regnum;
	regnum = sizeof(rs422_serial_port_reg_t)/ 4;
	int i;
	for(i =0; i < regnum; i++){
...
static void rs422_serial_port_register_attribute(conf_class_t* clss) {
	SKY_register_attribute(clss, "dev_id", get_attr_dev_id, NULL, set_attr_dev_id, NULL, SKY_Attr_Optional, "uinteger", "device number ");
	return;
}
static conf_object_t* new_rs422_serial_port(char* obj_name){
	rs422_serial_port_device* dev = skyeye_mm_zero(sizeof(rs422_serial_port_device));
	rs422_serial_port_reg_t* regs =  skyeye_mm_zero(sizeof(rs422_serial_port_reg_t));

	dev->obj = new_conf_object(obj_name, dev);
	dev->send_buffer = (char *)skyeye_mm_zero(32 * sizeof(int));
	dev->recv_buf = (char *)skyeye_mm_zero(32 * sizeof(int));
	/* init rs422_serial_port regs */
	dev->regs = regs;
	dev->fifo.start_send = 0;
	dev->fifo.end_send = 0;
...
static void free_rs422_serial_port(conf_object_t* conf_obj){
	rs422_serial_port_device* dev = conf_obj->obj;
...
static exception_t rs422_serial_port_set(conf_object_t *obj, conf_object_t *obj2, const char *port, int index)
{
	rs422_serial_port_device* dev = (rs422_serial_port_device*)(obj->obj);
	dev->bus_obj = obj2;
	...
	if (Error == dev->bus_iface->register_device(obj2, obj)){
		printf("In %s, Register rs422_serial_port device Error.\n", __FUNCTION__);
	}
	return No_exp;

}

static exception_t rs422_serial_port_get(conf_object_t *obj, conf_object_t **obj2, const char **port, int index)
{
	rs422_serial_port_device* dev = (rs422_serial_port_device*)(obj->obj);
	*obj2 = dev->bus_obj;
	*port = NULL;
...
void init_rs422_serial_port(){
	static skyeye_class_t class_data = {
		.class_name = "rs422_serial_port",
		.class_desc = "rs422_serial_port",
		.new_instance = new_rs422_serial_port,
		.free_instance = free_rs422_serial_port,
		.get_attr = NULL,
		.set_attr = NULL,
	...
	static const memory_space_intf io_memory = {
		.read = rs422_serial_port_read,
		.write = rs422_serial_port_write,
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);

	static const Rs422_emif_intf rs422_emif_interface = {
		.set_state = rs422_serial_port_set_state,
	};
	SKY_register_iface(clss, RS422_EMIF_INTF, &rs422_emif_interface);

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = rs422_serial_port_get_regvalue_by_id,
		.get_regname_by_id = rs422_serial_port_get_regname_by_id,
		.set_regvalue_by_id = rs422_serial_port_set_regvalue_by_id,
		.get_regnum = rs422_serial_port_get_regnum,
		.get_regid_by_name = rs422_serial_port_get_regid_by_name,
	};
	...
	static const struct ConnectDescription connects[] = {
		(struct ConnectDescription ){
			.name = RS422_BUS_INTF,
			.set = rs422_serial_port_set,
			.get = rs422_serial_port_get,
		},
	};
	class_register_ifaces(clss, ifaces);
	class_register_connects(clss, connects);

	rs422_serial_port_register_attribute(clss);
}
