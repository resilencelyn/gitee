...
static int cat24c256_i2c_set_state(conf_object_t* opaque, i2c_device_state_t state, uint8_t address)
{
	cat24c256_dev *dev=  (cat24c256_dev *)opaque->obj;
	dev->op_mod = state;
	dev->addr_count = 2;
	return 0;
}

static uint8_t cat24c256_i2c_read_data(conf_object_t *opaque){
	cat24c256_dev *dev=  (cat24c256_dev *)opaque->obj;
	uint8_t ch;
	ch = image_read(opaque, dev->offset);
	dev->offset++;
	return ch;
}

static void  cat24c256_i2c_write_data(conf_object_t *opaque, uint8_t value){
	cat24c256_dev *dev=  (cat24c256_dev *)opaque->obj;
	...
	if(dev->op_mod == -1)                                     /*i2c bus idle*/
		...
		if(dev->addr_count == 0 && dev->op_mod == 0){    /*slave receive mod*/
			image_write(opaque, dev->offset++, value);
		}else{
			(dev->addr_count)--;
			if(dev->addr_count == 1){
				dev->addr_h = value;
...
static uint8_t cat24c256_i2c_get_address(conf_object_t *opaque)
{
	cat24c256_dev *dev=  (cat24c256_dev *)opaque->obj;
	return dev->address;
}
...
static exception_t eeprom_cat24c256_set_attr(conf_object_t* opaque, const char* attr_name, attr_value_t* value)
{
	cat24c256_dev *dev=  (cat24c256_dev *)opaque->obj;
	return No_exp;
}

static int8_t *get_eeprom_addr(cat24c256_dev *dev)
{
	...
	sprintf(imagename, "%s_cat24c256.image", dev_name);
...
static conf_object_t* new_eeprom_cat24c256(char* obj_name)
{
	cat24c256_dev* dev = skyeye_mm_zero(sizeof(cat24c256_dev));
	dev->obj = new_conf_object(obj_name, dev);
	dev->capacity = CAPACITY;
#if 0
	i2c_device_intf *i2c_dev_iface = skyeye_mm_zero(sizeof(i2c_device_intf));
	i2c_dev_iface->set_state = cat24c256_i2c_set_state;
	i2c_dev_iface->write_data = cat24c256_i2c_write_data;
	i2c_dev_iface->read_data = cat24c256_i2c_read_data;
	i2c_dev_iface->get_address = cat24c256_i2c_get_address;
...
static exception_t reset_eeprom_cat24c256(conf_object_t* opaque, const char* args)
{
	cat24c256_dev *dev = opaque->obj;
	return No_exp;        
}


static exception_t free_eeprom_cat24c256(conf_object_t* conf_obj)
{
	cat24c256_dev *dev = conf_obj->obj;
#ifndef __MINGW32__
...
static void eeprom_cat24c256_register_interface(conf_class_t* class){
	static const i2c_device_intf i2c_dev_iface = {
		.set_state = cat24c256_i2c_set_state,
		.write_data = cat24c256_i2c_write_data,
		.read_data = cat24c256_i2c_read_data,
		.get_address = cat24c256_i2c_get_address,
	};
...
void init_eeprom_cat24c256(){
	static skyeye_class_t class_data = {
		.class_name = "eeprom_cat24c256",
		.class_desc = "eeprom_cat24c256",
		.new_instance = new_eeprom_cat24c256,
		.reset_instance = reset_eeprom_cat24c256,
		.free_instance = free_eeprom_cat24c256,
		.get_attr = NULL,
		.set_attr = eeprom_cat24c256_set_attr,
		.module_type = SKYML_OFFCHIP,
	};

	conf_class_t* class = SKY_register_device_class(class_data.class_name, &class_data);
	eeprom_cat24c256_register_interface(class);
}
