...
static void do_signal_raise(timer_c645x_device *dev)
{
...
{
	timer_c645x_device *dev = opaque->obj;
	RW_WRLOCK(dev->lock);
	dev->regs->cnt = 0;
	RW_UNLOCK(dev->lock);
...
{
	timer_c645x_device *dev = opaque->obj;
	timer_c645x_reg_t* regs = dev->regs;
...
static exception_t timer_c645x_read(conf_object_t *opaque, generic_address_t offset, void* buf, size_t count)
{
	timer_c645x_device *dev = opaque->obj;
	timer_c645x_reg_t* regs = dev->regs;
	...
	switch(offset) {
		...
		case WDTCR:
			*data = regs->wdtcr;
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__, "Can not read the register at 0x%x in timer_c645x\n", offset);
			return Invarg_exp;
	}
	return No_exp;
}

static exception_t timer_c645x_write(conf_object_t *opaque, generic_address_t offset, uint32_t* buf, size_t count)
{
	timer_c645x_device *dev = opaque->obj;
	timer_c645x_reg_t* regs = dev->regs;
	...
	case WDTCR:
		...
		break;
	default:
		skyeye_log(Error_log, __FUNCTION__, "Can not write the register at 0x%x in timer_c645x\n", offset);
		return Invarg_exp;
	}
	return No_exp;
}

static char* timer_c645x_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	return regs_name[id];
}

static uint32_t timer_c645x_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id){
	timer_c645x_device *dev = conf_obj->obj;
	timer_c645x_reg_t* regs = dev->regs;
	uint32_t* regs_value = (uint32_t*)regs + id;
	return *regs_value;
}

static char* timer_c645x_set_regvalue_by_id(conf_object_t* conf_obj, uint32_t value, uint32_t id)
{
	timer_c645x_device *dev = conf_obj->obj;
	timer_c645x_reg_t* regs = dev->regs;
...
static uint32_t timer_c645x_get_regnum(conf_object_t* conf_obj, uint32_t id)
{
	timer_c645x_device *dev = conf_obj->obj;
	timer_c645x_reg_t* regs = dev->regs;
	return  sizeof(timer_c645x_reg_t) / 4;
}

static uint32_t timer_c645x_get_regid_by_name(conf_object_t* conf_obj, char *name)
{
	timer_c645x_device *dev = conf_obj->obj;
	timer_c645x_reg_t* regs = dev->regs;
	uint32_t regnum;
	regnum = sizeof(timer_c645x_reg_t)/ 4;
	int i;
	for(i =0; i < regnum; i++){
...
static conf_object_t* new_timer_c645x(char* obj_name){
	timer_c645x_device* dev = skyeye_mm_zero(sizeof(timer_c645x_device));
	timer_c645x_reg_t* regs =  skyeye_mm_zero(sizeof(timer_c645x_reg_t));
	dev->obj = new_conf_object(obj_name, dev);
	/* init timer_c645x regs */
	dev->regs = regs;
...
static void free_timer_c645x(conf_object_t* conf_obj){
	timer_c645x_device* dev = conf_obj->obj;
...
{
	timer_c645x_device *dev = obj->obj;
	dev->core = obj2;
...
void init_timer_c645x(){
	static skyeye_class_t class_data = {
		.class_name = "c645x_timer",
		.class_desc = "c645x_timer",
		.new_instance = new_timer_c645x,
		.free_instance = free_timer_c645x,
		.get_attr = NULL,
		.set_attr = NULL 
	};
	...
	static const memory_space_intf io_memory = {
		.read = timer_c645x_read,
		.write = timer_c645x_write,
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = timer_c645x_get_regvalue_by_id,
		.get_regname_by_id = timer_c645x_get_regname_by_id,
		.set_regvalue_by_id = timer_c645x_set_regvalue_by_id,
		.get_regnum = timer_c645x_get_regnum,
		.get_regid_by_name = timer_c645x_get_regid_by_name,
	};
