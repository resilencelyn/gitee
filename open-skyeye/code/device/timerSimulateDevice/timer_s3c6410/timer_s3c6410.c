...
#include "timer_s3c6410.h"

static void s3c6410x_timer_callback(conf_object_t* obj)
{

	timer_s3c6410_device *dev = obj->obj;
	RW_WRLOCK(dev->lock);
	struct timeval tv;
	if ((dev->regs.tcon & 0x100000) != 0) {
		/*tcntx is the orignal value we set, it equals tcntbx firstly*/
		//dev->regs.tcnt[4] = dev->regs.tcnt[4] - 100;
...
static exception_t timer_s3c6410_read(conf_object_t *opaque, generic_address_t offset, void* buf, size_t count)
{
	timer_s3c6410_device *dev = opaque->obj;
	...
	switch (offset) {
		...
		case TINT_CSTAT:
			*(uint32_t *)buf = dev->regs.tint_cstat;
			break;
		default:
			break;
		}
...
static exception_t timer_s3c6410_write(conf_object_t *opaque, generic_address_t offset, uint32_t* buf, size_t count)
{
	timer_s3c6410_device *dev = opaque->obj;
	switch (offset) {
                case TCFG0:
		...
		case TCON:
			{
				...
				if ((dev->regs.tcon & 0x100000) != 0) {
					...
					if(dev->scheduler_id == -1 ){
						/* prescaler for timer4 */
						int scaler = ((dev->regs.tcfg0 & 0xffff00) >> 8);
						/* divider selection for timer4 frequency */
						int div = ((dev->regs.tcfg1 & 0x30000) >> 16);
						...
						create_thread_scheduler((unsigned int)us,mode, s3c6410x_timer_callback, (void*)opaque, &dev->scheduler_id);
						//create_timer_scheduler((unsigned int)ms,mode, s3c6410x_timer_callback, (void*)state, &dev->scheduler_id);
					}
				} else {
					if (dev->scheduler_id != -1) {
						del_thread_scheduler(dev->scheduler_id);
						//del_timer_scheduler(dev->scheduler_id);
						dev->scheduler_id = -1;
		...
		case TCNTB4:
			{
				...
				#if 0
				/* dev->regs.tcntb[n] = data; */
				/* temp data taken from linux source */
				dev->regs.tcntb[n] = 25350 / 20;
				#endif 
			}
			break;
		case TCMPB0:
		...
		case TCNTO4:
			...
			break;
		case TINT_CSTAT:
		        dev->regs.tint_cstat = *(uint32_t*)buf;
			break;
		default:
			break;
	}
...
char* timer_s3c6410_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	timer_s3c6410_device *dev = conf_obj->obj;
	return regs_name[id];
}

int32_t timer_s3c6410_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id){
	timer_s3c6410_device *dev = conf_obj->obj;
	uint32_t* regs_value = (uint32_t*)&(dev->regs) + id;
	return *regs_value;
}

static char* timer_s3c6410_set_regvalue_by_id(conf_object_t* conf_obj, uint32_t value, uint32_t id)
{
	timer_s3c6410_device *dev = conf_obj->obj;
...
static uint32_t timer_s3c6410_get_regnum(conf_object_t* conf_obj, uint32_t id)
{
	timer_s3c6410_device *dev = conf_obj->obj;
	struct registers *regs = &(dev->regs);
...
static uint32_t timer_s3c6410_get_regid_by_name(conf_object_t* conf_obj, char *name)
{
	timer_s3c6410_device *dev = conf_obj->obj;
	struct registers *regs = &(dev->regs);
	uint32_t regnum;
...
static exception_t timer_s3c6410_set_attr(conf_object_t* opaque, const char* attr_name, attr_value_t* value)
{
	timer_s3c6410_device *dev = opaque->obj;
}

static conf_object_t* new_timer_s3c6410(char* obj_name){
	timer_s3c6410_device* dev = skyeye_mm_zero(sizeof(timer_s3c6410_device));
	dev->obj = new_conf_object(obj_name, dev);
	/* init timer_s3c6410 regs */
	dev->scheduler_id = -1;
	dev->regs.tcfg0 = 0x101;
	return dev->obj;
}
static void free_timer_s3c6410(conf_object_t* conf_obj){
	timer_s3c6410_device *dev = conf_obj->obj;
    skyeye_free(dev);
	return;	
}
...
void init_timer_s3c6410(){
	static skyeye_class_t class_data = {
		.class_name = "s3c6410_timer",
		.class_desc = "s3c6410_timer",
		.new_instance = new_timer_s3c6410,
		.free_instance = free_timer_s3c6410,
		.get_attr = NULL,
		.set_attr = timer_s3c6410_set_attr,
		.module_type = SKYML_ONCHIP(SC_s3c6410),
	};

	conf_class_t* clss = SKY_register_device_class(class_data.class_name, &class_data);
	static const memory_space_intf io_memory = {
		.read = timer_s3c6410_read,
		.write = timer_s3c6410_write,
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = timer_s3c6410_get_regvalue_by_id,
		.get_regname_by_id = timer_s3c6410_get_regname_by_id,
		.set_regvalue_by_id = timer_s3c6410_set_regvalue_by_id,
		.get_regnum = timer_s3c6410_get_regnum,
		.get_regid_by_name = timer_s3c6410_get_regid_by_name,
	};
