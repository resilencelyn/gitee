/* Copyright (C) * 2011 - Michael.Kang blackfin.kang@gmail.com * This program is free software; you can redistribute it and/or
...
char* s3c6410_camif_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	s3c6410_camif_device *dev = conf_obj->obj;
	return dev->regs_name[id];
}

uint32_t s3c6410_camif_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id){
	s3c6410_camif_device *dev = conf_obj->obj;
	uint32_t* regs_value = (uint32_t*)(dev->regs) + id;
	return *regs_value;
}

static char* s3c6410_camif_set_regvalue_by_id(conf_object_t* conf_obj, uint32_t value, uint32_t id)
{
	s3c6410_camif_device *dev = conf_obj->obj;
	camif_reg_t *regs = (dev->regs);
...
static uint32_t s3c6410_camif_get_regnum(conf_object_t* conf_obj, uint32_t id)
{
	s3c6410_camif_device *dev = conf_obj->obj;
	camif_reg_t *regs = (dev->regs);
	return  sizeof(camif_reg_t) / 4;


}

static uint32_t s3c6410_camif_get_regid_by_name(conf_object_t* conf_obj, char *name)
{
	s3c6410_camif_device *dev = conf_obj->obj;
	camif_reg_t *regs = (dev->regs);
	uint32_t regnum;
	regnum = sizeof(camif_reg_t)/ 4;
	int i;
	for(i =0; i < regnum; i++){
...
void free_s3c6410_camif(conf_object_t* conf_obj){
	s3c6410_camif_device *dev = conf_obj->obj;
...
void init_s3c6410_camif(){
	...
	static const memory_space_intf io_memory = {
		.read = s3c6410_camif_read,
		.write = s3c6410_camif_write,
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);	

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = s3c6410_camif_get_regvalue_by_id,
		.get_regname_by_id = s3c6410_camif_get_regname_by_id,
		.set_regvalue_by_id = s3c6410_camif_set_regvalue_by_id,
		.get_regnum = s3c6410_camif_get_regnum,
		.get_regid_by_name = s3c6410_camif_get_regid_by_name,
	};
