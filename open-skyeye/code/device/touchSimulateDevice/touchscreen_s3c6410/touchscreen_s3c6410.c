...
char* s3c6410_touchscreen_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	s3c6410_touchscreen_device *dev = conf_obj->obj;
	return dev->regs_name[id];
}

uint32_t s3c6410_touchscreen_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id){
	s3c6410_touchscreen_device *dev = conf_obj->obj;
	uint32_t* regs_value = (uint32_t*)(dev->regs) + id;
	return *regs_value;
}

static char* s3c6410_touchscreen_set_regvalue_by_id(conf_object_t* conf_obj, uint32_t value, uint32_t id)
{
	s3c6410_touchscreen_device *dev = conf_obj->obj;
	touchscreen_reg_t *regs = (dev->regs);
...
static uint32_t s3c6410_touchscreen_get_regnum(conf_object_t* conf_obj, uint32_t id)
{
	s3c6410_touchscreen_device *dev = conf_obj->obj;
	touchscreen_reg_t *regs = (dev->regs);
	return  sizeof(touchscreen_reg_t) / 4;


}

static uint32_t s3c6410_touchscreen_get_regid_by_name(conf_object_t* conf_obj, char *name)
{
	s3c6410_touchscreen_device *dev = conf_obj->obj;
	touchscreen_reg_t *regs = (dev->regs);
	uint32_t regnum;
	regnum = sizeof(touchscreen_reg_t)/ 4;
	int i;
	for(i =0; i < regnum; i++){
...
static conf_object_t* new_s3c6410_touchscreen(char* obj_name){
	...
	regs->adcupdn = 0x0000;
	dev_ts->regs = regs;
	dev_ts->regs_name = regs_name;
	dev_ts->status = status;

	return dev_ts->obj;
}
void free_s3c6410_touchscreen(conf_object_t* obj){
	s3c6410_touchscreen_device *dev = obj->obj;
    skyeye_free(dev->status);
    skyeye_free(dev->regs);
    skyeye_free(dev);
}

void s3c6410_touchscreen_register_attribute(conf_class_t* clss){
}
...
void init_s3c6410_touchscreen(){
	...
	static const lcd_touchscreen_t lcd_ts = {
		.touchscreen_update_status = touchscreen_update_status,
	};
	SKY_register_iface(clss, LCD_TS_INTF_NAME, &lcd_ts);

	static const memory_space_intf io_memory = {
		.read = s3c6410_touchscreen_read,
		.write = s3c6410_touchscreen_write,
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);	

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = s3c6410_touchscreen_get_regvalue_by_id,
		.get_regname_by_id = s3c6410_touchscreen_get_regname_by_id,
		.set_regvalue_by_id = s3c6410_touchscreen_set_regvalue_by_id,
		.get_regnum = s3c6410_touchscreen_get_regnum,
		.get_regid_by_name = s3c6410_touchscreen_get_regid_by_name,
	};
	...
	class_register_connects(clss, connects);
 	...
 	s3c6410_touchscreen_register_attribute(clss);
}
