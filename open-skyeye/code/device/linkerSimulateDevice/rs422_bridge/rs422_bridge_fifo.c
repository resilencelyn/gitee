...
FIFO* CreateFIFO(unsigned int  FIFOLength)
{
    ...
    if(fifo == NULL)
	    return NULL;
    rs422_msg_t *pfifo;
    pfifo = skyeye_mm_zero(FIFOLength*sizeof(rs422_msg_t));
    memset(pfifo, 0, FIFOLength*sizeof(rs422_msg_t));
    if(pfifo == NULL)
	    return NULL;
    fifo->pFirst = pfifo;
...
unsigned int  WriteFIFO(FIFO *fifo, rs422_msg_t *pSource,unsigned int  WriteLength)
{
	...
	for (i = 0; i < WriteLength; i++){
		...
		memcpy(fifo->pIn, pSource, sizeof(rs422_msg_t));
		pSource++;
...
unsigned int  ReadFIFO(FIFO *fifo, rs422_msg_t *pAim,unsigned int  ReadLength)
{
	...
	for (i = 0; i < ReadLength; i++){
		...
		memcpy(pAim, fifo->pOut, sizeof(rs422_msg_t));
		pAim++;
