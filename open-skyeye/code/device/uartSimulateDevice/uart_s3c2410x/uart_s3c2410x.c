...
#include "uart_s3c2410x.h"
#include "s3c2410x_fifo.h"

static char* s3c2410x_uart_attr[] = {"term", "signal"};

void uart_do_cycle(conf_object_t* opaque)
{
	struct uart_s3c2410x_device *dev = opaque->obj;
	uart_s3c2410x_reg_t* regs = dev->regs;
	exception_t ret = 0;
	int sig_no;
	struct timeval tv;
	unsigned char buf;
...
static exception_t s3c2410x_uart_set_attr(conf_object_t* opaque, const char* attr_name, attr_value_t* value)
{
	struct uart_s3c2410x_device *dev = opaque->obj;
	uart_s3c2410x_reg_t* regs = dev->regs;
	int index;
	for(index = 0; index < sizeof(s3c2410x_uart_attr) / sizeof(s3c2410x_uart_attr[0]); index++) {
		if(!strncmp(attr_name, s3c2410x_uart_attr[index], strlen(s3c2410x_uart_attr[index])))
			break;
	}
...
static exception_t uart_s3c2410x_read(conf_object_t *opaque, generic_address_t offset, void* buf, size_t count)
{
	struct uart_s3c2410x_device *dev = opaque->obj;
	uart_s3c2410x_reg_t* regs = dev->regs;
	...
	switch(offset) {
		...
		case UBRDIV:
			*(uint32_t*)buf = regs->ubrdiv;
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__,"Can not read the register at 0x%x in uart_s3c2410x\n", offset);
			return Invarg_exp;
	}
	return No_exp;
}

static exception_t uart_s3c2410x_write(conf_object_t *opaque, generic_address_t offset, uint32_t* buf, size_t count)
{
	struct uart_s3c2410x_device *dev = opaque->obj;
	uart_s3c2410x_reg_t* regs = dev->regs;
	uint32_t data = *buf;
	...
	switch(offset) {
		...
		case UBRDIV:
			...
			break;
		default:
			skyeye_log(Error_log, __FUNCTION__,"Can not write the register at 0x%x in uart_s3c2410x\n", offset);
			return Invarg_exp;
	}
	return No_exp;
}
static char *uart_s3c2410x_get_regname_by_id(conf_object_t* conf_obj, uint32_t id)
{
	uart_s3c2410x_device *dev= conf_obj->obj;
	return dev->regs_name[id];
}

static uint32_t uart_s3c2410x_get_regvalue_by_id(conf_object_t* conf_obj, uint32_t id)
{
	uart_s3c2410x_device *dev= conf_obj->obj;
...
static exception_t uart_s3c2410x_set_regvalue_by_id(conf_object_t* conf_obj, uint32_t value, uint32_t id)
{
	uart_s3c2410x_device *dev = conf_obj->obj;
	uart_s3c2410x_reg_t *regs = (dev->regs);
...
static uint32_t uart_s3c2410x_get_regnum(conf_object_t* conf_obj, uint32_t id)
{
	uart_s3c2410x_device *dev = conf_obj->obj;
	uart_s3c2410x_reg_t *regs = (dev->regs);
	return  sizeof(uart_s3c2410x_reg_t) / 4;
}

static uint32_t uart_s3c2410x_get_regid_by_name(conf_object_t* conf_obj, char *name)
{
	uart_s3c2410x_device *dev = conf_obj->obj;
	uart_s3c2410x_reg_t *regs = (dev->regs);
	uint32_t regnum;
	regnum = sizeof(uart_s3c2410x_reg_t)/ 4;
	int i;
	for(i =0; i < regnum; i++) {
...
static conf_object_t* new_uart_s3c2410x(char* obj_name) {
	uart_s3c2410x_device* dev = skyeye_mm_zero(sizeof(uart_s3c2410x_device));
	uart_s3c2410x_reg_t* regs =  skyeye_mm_zero(sizeof(uart_s3c2410x_reg_t));
	dev->obj = new_conf_object(obj_name, dev);
	/* init uart_s3c2410x regs */
	dev->regs = regs;
	dev->regs->ulcon = 0x00;
	dev->regs->ucon = 0x00;
	dev->regs->ufcon = 0x0;
	dev->regs->umcon = 0x0;
	dev->regs->utrstat = 0x6;
	dev->regs->uerstat = 0x0;
	dev->regs->ufstat = 0x00;
	dev->regs->umstat = 0x0;
...
#define UART_ULCON_INIT	0x3 /*8-bits, 1-stop, no-parity*/

static exception_t reset_uart_s3c2410x(conf_object_t* opaque, const char* parameters) {
	struct uart_s3c2410x_device *dev = opaque->obj;
	uart_s3c2410x_reg_t* regs = dev->regs;

	regs->utrstat = UART_UTRSTAT_INIT;
	regs->ulcon = UART_ULCON_INIT;
	dev->sig_no = 28;
	return No_exp;
}

static exception_t free_uart_s3c2410x(conf_object_t* opaque) {
	struct uart_s3c2410x_device *dev = opaque->obj;
	if(dev->read_fifo)
...
static attr_value_t get_attr_signal(void* arg, conf_object_t* conf_obj, attr_value_t* idx) {
	struct uart_s3c2410x_device *dev = conf_obj->obj;
...
{
	struct uart_s3c2410x_device *dev = obj->obj;
	dev->term = obj2;
...
void uart_s3c2410x_register_attribute(conf_class_t* clss){
...
void init_uart_s3c2410x(){
	static skyeye_class_t class_data = {
		.class_name = "s3c2410x_uart",
		.class_desc = "s3c2410x_uart",
		.new_instance = new_uart_s3c2410x,
		.free_instance = free_uart_s3c2410x,
		.reset_instance = reset_uart_s3c2410x,
		.get_attr = NULL,
		.set_attr = s3c2410x_uart_set_attr,
	...
	static const memory_space_intf io_memory = {
		.read = uart_s3c2410x_read,
		.write = uart_s3c2410x_write
	};
	SKY_register_iface(clss, MEMORY_SPACE_INTF_NAME, &io_memory);

	static const skyeye_reg_intf reg_intf = {
		.get_regvalue_by_id = uart_s3c2410x_get_regvalue_by_id,
		.get_regname_by_id = uart_s3c2410x_get_regname_by_id,
		.set_regvalue_by_id = uart_s3c2410x_set_regvalue_by_id,
		.get_regnum = uart_s3c2410x_get_regnum,
		.get_regid_by_name = uart_s3c2410x_get_regid_by_name,
	};
	...
	uart_s3c2410x_register_attribute(clss);
}
