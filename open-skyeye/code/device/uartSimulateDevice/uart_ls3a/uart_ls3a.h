...
#ifndef __UARTLS3A_H__
#define __UARTLS3A_H__
...
#define UART_ls3a_BANK_LEN 0x11

#define REGS_V_DEF(reg, off) uint8 reg;
#define REGS_A_DEF(reg, off, n, intv) uint8 reg[n];
typedef struct reg_ls3a {
	UART_ls3a_REGS_LIST(REGS_V_DEF, REGS_A_DEF)
	/* rtd */
	uint8 rbr;
	uint8 thr;
	uint8 dlb;
...
enum {
	UART_ls3a_REGS_LIST(REGS_V_MACRO, REGS_A_MACRO)
};
...
const char *uart_ls3a_reg_name[REGS_NUM(UART_ls3a_REGS_LIST)];

typedef struct uart_ls3a {
	conf_object_t *obj;
	reg_ls3a_t    reg;
	/* internal implement */
