...
static exception_t c6678_n25q128_read(conf_object_t *opaque, generic_address_t offset, void *buf, size_t count)
{
	c6678_n25q128_device *dev = (c6678_n25q128_device *)opaque->obj;
...
static exception_t c6678_n25q128_write(conf_object_t *opaque, generic_address_t offset, void *buf, size_t count)
{
	c6678_n25q128_device *dev = opaque->obj;
	uint8_t data = *(uint8_t *)buf;
	uint32_t i = 0;

	debug_printf("write_flag : 0x%x\n",dev->write_flag);
	if (dev->write_flag == 1)
	{
		if (dev->image_space)
		{
	...
	switch (offset)
	{
		case REG_N25Q128_READ_OPERATION:
		{
			dev->regs.reg_n25q128_read_opearation = data;
			break;
		}
		case REG_N25Q128_WRITE_OPERATION:
		{
			dev->regs.reg_n25q128_write_opearation = data;
			break;
		}
		case REG_N25Q128_STATUS_OPERATION:
		{
			dev->regs.reg_n25q128_status = data;
			break;
		}
		case REG_N25Q128_FLAGSTATUS_OPERATION:
		{
			dev->regs.reg_n25q128_flagstatus = data;
			break;
		}
		case REG_N25Q128_PROGRAM_OPERATION:
		{
			dev->regs.reg_n25q128_program_operation = data;
			
			if (dev->regs.reg_n25q128_program_operation == N25Q128_FLASH_INS_PP)
			{
		...
		case REG_N25Q128_ERASE_OPERATION:
		{
			dev->regs.reg_n25q128_erase_operation = data;
			if (dev->regs.reg_n25q128_write_opearation == N25Q128_FLASH_INS_WREN)
			{
				if (dev->regs.reg_n25q128_erase_operation ==  N25Q128_FLASH_INS_DE)
				{
					for (i = 0; i < N25Q128_FLASHSIZE; i++)
					{
						data = 0xff;
						if (dev->image_space)
						{
					...
					dev->erase_ready = 1;
					dev->write_flag = 0;
				} else if (dev->regs.reg_n25q128_erase_operation ==  N25Q128_FLASH_INS_SE)
				{
					for(i=0; i<N25Q128_FLASHSECTORSIZE; i++)
					{
						data = 0xff;
						if (dev->image_space)
						{
				...
				{
					skyeye_log(Error_log, __FUNCTION__, "Cannot write the register at 0x%x in c6678_n25q128\n", offset);
				}
			}
			break;
		}
...
static conf_object_t* new_c6678_n25q128(const char *obj_name)
{
	c6678_n25q128_device* dev = skyeye_mm_zero(sizeof(c6678_n25q128_device));
...
static exception_t config_c6678_n25q128(conf_object_t* conf_obj)
{
	c6678_n25q128_device *dev = (c6678_n25q128_device *)conf_obj->obj;
...
static exception_t free_c6678_n25q128(conf_object_t* conf_obj)
{
	c6678_n25q128_device *dev = (c6678_n25q128_device *)conf_obj->obj;
	fclose(dev->rd_fp);
	fclose(dev->wr_fp);
...
{
	c6678_n25q128_device *dev = conf_obj->obj;
...
{
	c6678_n25q128_device *dev = (c6678_n25q128_device *)obj->obj;
	dev->image = obj2;
...
void init_c6678_n25q128(){
	static skyeye_class_t class_data =
	{
		.class_name = "c6678_n25q128",
		.class_desc = "c6678_n25q128 device",
		.new_instance = new_c6678_n25q128,
		.free_instance = free_c6678_n25q128,
		.config_instance = config_c6678_n25q128,
	...
	{
		.read = c6678_n25q128_read,
		.write = c6678_n25q128_write,
	};
