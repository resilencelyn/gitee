/*
	skyeye_mach_mpc8560.c - define machine AM3359 for skyeye
...
#include "../../../arch/ppc/common/ppc_irq.h"
#include "../../../arch/ppc/common/ppc_exc.h"
#include "../../../arch/ppc/common/ppc_cpu.h"
#include <bank_defs.h>
...
#define MAX_IDLE_COUNT 10
		...
		exception_t ret = term->read(term->conf_obj, &buf, 1);
		if (ret == No_exp) {
			//printf("In %s, get a char %c\n", __FUNCTION__, buf);
			/* build a bd for rx */
			int recv_bd_base = (*(uint16 *) & ram[rx_base]);
			short bd_flag;
			...
			if (!(cpm->int_ctrl.simr_l & 0x10)) {
				...
				*((sint16 *) &ram[recv_bd_base + curr_rx_bd + 2]) = 0x1;
	...
	if (cpm->scc[0].gsmrl & 0x00000010) {
		...
		int trans_bd_base =(*(uint16 *) & ram[tx_base]);
		short bd_flag = (*(sint16 *) &
		...
		uint32 buf_addr =(*
					  ((uint32 *) &
					   ram[trans_bd_base + curr_tx_bd +
					       4]));
		//fprintf("trans_bd_base=0x%x,bd_flag=0x%x,curr_tx_bd=0x%x,bd_len=0x%x,buf_addr=0x%x\n",trans_bd_base, bd_flag, curr_tx_bd, bd_len, buf_addr);
		/* If data ready */
		if (bd_flag & 0x8000) {
			int i = 0;
			for (; i < bd_len; i++) {
				...
				term->write(term->conf_obj, &c, 1);
			}

			/* set Empty bit */
			*((sint16 *) & ram[trans_bd_base + curr_tx_bd]) &=(~0x8000);
			if (bd_flag & 0x2000)	/* check wrap bit */
...
mpc8560_io_read_byte (void *state, uint32_t offset)
{
	...
	case 0x300C:
		...
		/*
		printf ("KSDBG:read i2csr result=0x%x\n",
			 *result);
		*/
	case 0x8004:
	case 0x8005:
...
mpc8560_io_write_byte (void *state, uint32_t offset, uint32_t data)
{
	...
	if (offset >= 0x80000 && offset < 0x8C000) {
		//printf("DBG_CPM:in %s,offset=0x%x,data=0x%x,\n",__FUNCTION__, offset, data);
...
mpc8560_io_write_halfword (void *state, uint32_t offset, uint32_t data)
{
	...
	if (offset >= 0x80000 && offset < 0x8C000) {
		//printf("DBG_CPM:in %s,offset=0x%x,data=0x%x,\n",__FUNCTION__, offset, data);
		*((sint16 *) & io->cpm_reg.dpram[offset - 0x80000]) = data;
		return;
	}
...
mpc8560_io_write_word (void *state, uint32_t offset, uint32_t data)
{
	...
	if (offset >= 0x80000 && offset < 0x8C000) {
		...
		*((sint32 *) & io->cpm_reg.dpram[offset - 0x80000]) =data;
		return;
	}
...
mpc8560_mach_init (void *arch_instance, machine_config_t * this_mach)
{
	...
	term = (skyeye_uart_intf*)SKY_get_interface(uart_term0, SKYEYE_UART_INTF);
}
#endif
typedef struct mpc8560_mach{
	conf_object_t* obj;
} mpc8560_mach_t;
...
static int mpc8560_update_boot_info(conf_object_t *obj)
{
	mpc8560_mach_t *mach = obj->obj;
	...
	for(i = 0; i < soc->cpu_cnt; i++){
		if ((cpu = soc->cpus + i) != NULL) {
            ...
            mpc8560_boot_linux(cpu->cpu);
		}
	}
	return 0;
}

static conf_object_t *new_mpc8560_mach(char* obj_name){
	mpc8560_mach_t* mach = skyeye_mm_zero(sizeof(mpc8560_mach_t));
	mach->obj = new_conf_object(obj_name, mach);

	return  mach->obj;
}

static void mpc8560_register_attribute(conf_class_t* clss){
}

static void mpc8560_register_interface(conf_class_t* clss){
	static const skyeye_machine_intf inface = {
		.update_boot_info = mpc8560_update_boot_info
	};
...
static void free_mpc8560_mach(conf_object_t *mach)
{
	mpc8560_mach_t *mpc8560_mach = mach->obj;
	skyeye_free(mpc8560_mach->obj);
	skyeye_free(mpc8560_mach);
	return;
}
void init_mpc8560_mach(void){
	static skyeye_class_t class_data = {
		.class_name = "mpc8560",
		.class_desc = "mpc8560 machine",
		.new_instance = new_mpc8560_mach,
		.free_instance = free_mpc8560_mach,
		.get_attr = NULL,
		.set_attr = NULL
	};
	conf_class_t* class = SKY_register_soc_class(class_data.class_name, &class_data);

	mpc8560_register_attribute(class);
	mpc8560_register_interface(class);
}
