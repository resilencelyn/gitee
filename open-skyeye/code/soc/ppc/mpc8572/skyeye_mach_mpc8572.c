...
static conf_object_t* mpc8572_io_conf_obj;
...
typedef struct skyeye_mach_mpc8572_device{
        conf_object_t* obj;
        common_machine_attr_t attr;
}skyeye_mach_mpc8572_device;

static conf_object_t *new_mpc8572_mach(char* obj_name){
    skyeye_mach_mpc8572_device* mach = skyeye_mm_zero(sizeof(skyeye_mach_mpc8572_device));
    mach->obj = new_conf_object(obj_name, mach);

        
    mpc8572_io_conf_obj = mach->obj;
    skyeye_config_t *config = get_current_config();
    machine_config_t* mach_config = skyeye_mm_zero(sizeof(machine_config_t));
    config->mach = mach_config;

    mpc8572_mach_init(NULL , config->mach);

    return  mach->obj;
}


int mpc8572_update_boot_info(conf_object_t *obj)
{
        
    int i;
    skyeye_mach_mpc8572_device *mach = obj->obj;
...
static void mpc8572_register_attribute(conf_class_t* clss){
}

static void mpc8572_register_interface(conf_class_t* clss){
        static const skyeye_machine_intf inface = {
                    .update_boot_info = mpc8572_update_boot_info
                            };
...
static void free_mpc8572_mach(conf_object_t *mach)
{
       
    skyeye_mach_mpc8572_device *mpc8572_mach = mach->obj;
    skyeye_free(mpc8572_mach->obj);
    skyeye_free(mpc8572_mach);
    return;
}

void init_skyeye_mach_mpc8572(void){
    static skyeye_class_t class_data = {
        .class_name = "mpc8572_machine",
        .class_desc = "mpc8572 machine",
        .new_instance = new_mpc8572_mach,
        .free_instance = free_mpc8572_mach,
        .get_attr = NULL,
        .set_attr = NULL
    };
    
    conf_class_t* class = SKY_register_soc_class(class_data.class_name,&class_data);
    mpc8572_register_attribute(class);
    mpc8572_register_interface(class);
}
