...
{
	//conf_object_t* arch_instance = get_arch_instance("");
	s3c2410x_update_int (NULL);
}

static int s3c2410x_scheduler_id = -1;
static void s3c2410x_timer_callback(conf_object_t* conf_obj)
{
	RW_WRLOCK(lock);
#if 0
...
static void
s3c2410x_io_reset (conf_object_t* conf_obj)
{
	int i;
	...
	reg_intf->set_regvalue_by_id(reg_intf->conf_obj, 193, 1);
...
static void
s3c2410x_io_do_cycle (conf_object_t* conf_obj)
{
...
static void
s3c2410x_uart_write (conf_object_t* conf_obj, uint32 offset, uint32 data, int index)
{
...
static void
s3c2410x_timer_write (conf_object_t *state, uint32 offset, uint32 data)
{
	switch (offset) {
	case TCFG0:
		io.timer.tcfg0 = data;
...
static void
s3c2410x_io_write_word (conf_object_t *state, uint32 addr, uint32 data)
{
	if ((addr >= UART_CTL_BASE0)
	    && (addr < UART_CTL_BASE0 + UART_CTL_SIZE)) {
...
static void
s3c2410x_io_write_byte (conf_object_t * state, uint32 addr, uint32 data)
{
...
static void
s3c2410x_io_write_halfword (conf_object_t * state, uint32 addr, uint32 data)
{
