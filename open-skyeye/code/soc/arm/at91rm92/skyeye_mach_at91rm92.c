...
at91rm92_io_do_cycle (generic_arch_t *state)
{
	...
	if (1) {
		...
		int i = 0;
		for(;i < UART_NUM; i++){
			if(io.uart[i].term == NULL)
				continue;
			ret = io.uart[1].term->read(io.uart[1].term->conf_obj, &buf, 1);
			if(ret != No_exp)
				continue;
	...
	case US_THR:
		{
			char c = data;
			/* 2007-01-18 modified by Anthony Lee : for new uart device frame */
			io.uart[i].term->write(io.uart[i].term->conf_obj, &c, 1);
...
at91rm92_mach_init (generic_arch_t *arch_instance, machine_config_t *this_mach)
{
	...
	io.uart[1].term = (skyeye_uart_intf*)SKY_get_interface(uart_term0, SKYEYE_UART_INTF);
