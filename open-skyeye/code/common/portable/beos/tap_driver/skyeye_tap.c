    ...
    static status_t domulti(skyeye_tap * dev, uint8 * addr, bool add)
{
    ...
    if ((retVal = acquire_sem(dev->ioLocker)) != B_NO_ERROR)
        return retVal;
    ...
    if ((nmulti = dev->nmulti) < MAX_MULTI || !add)
    {
        for (i = 0; i < nmulti; i++)
        {
            if (memcmp(&(dev->multi[i]), addr, 6) == 0)
                break;
        }
        ...
        {
            if (i != nmulti - 1)
                memcpy(&dev->multi[i], &dev->multi[i + 1], (nmulti - i - 1) * 6);
            dev->nmulti--;
        }
    } else
    {
...
static bool check_packet(skyeye_tap * dev, uint8 * addr)
{
    ...
    if (dev->promisc)
        return true;
    if (memcmp(addr, SKYEYE_TAP_MAC_ADDRESS, 6) == 0)
        return true;
    if (memcmp(addr, "\xff\xff\xff\xff\xff\xff", 6) == 0)
        return true;
    for (i = 0; i < dev->nmulti; i++)
        if (memcmp(addr, &dev->multi[i], 6) == 0)
            return true;
    return false;
}
...
static int recv_packet(skyeye_tap * dev, void *buf, size_t buflen)
{
    ...
    if (buf == NULL || acquire_sem(dev->ioLocker) != B_NO_ERROR)
        return -1;
...
{
    if (buf == NULL || buflen < 0 || buflen > SKYEYE_TAP_FRAME_SIZE)
        return -1;
    if (write_port_etc(dev->txPort, SKYEYE_TAP_PORT_MSG_CODE, buf, buflen, B_TIMEOUT, 1000) == B_OK)
        return (int) buflen;
    return 0;
}
...
static status_t open_hook(const char *name, uint32 flags, void **cookie)
{
    ...
    *cookie = NULL;
    ...
    if (strcmp(skyeye_tap_devs[0], name) != 0)
        return EINVAL;

    /* open once */
    if (atomic_or(&skyeye_tap_status, 1) & 1)
        return B_BUSY;
    ...
    if ((dev->rxPort = create_port(PACKET_QUEUE_LENGTH, SKYEYE_TAP_RX_PORT_NAME)) < 0 ||
        ...
        set_port_owner(dev->txPort, B_SYSTEM_TEAM) != B_OK || set_sem_owner(dev->ioLocker, B_SYSTEM_TEAM) != B_OK)
    {
        if (dev->rxPort >= 0)
            delete_port(dev->rxPort);
        if (dev->txPort >= 0)
            delete_port(dev->txPort);
        if (dev->ioLocker >= 0)
