...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset)
{
    ...
    if ((flags & (MAP_FIXED | MAP_SHARED)) || ((prot & PROT_WRITE) && fd != -1) || length == 0)
        return MAP_FAILED;

    length = (length + (B_PAGE_SIZE - 1)) & ~(B_PAGE_SIZE - 1);

    if ((area = create_area("none", &retVal, B_ANY_ADDRESS, length, B_NO_LOCK, B_READ_AREA | B_WRITE_AREA)) < 0)
        return MAP_FAILED;

    if (fd != -1)
    {
        if (fstat(fd, &stat) != 0 || offset >= stat.st_size || lseek(fd, offset, SEEK_SET) == (off_t) - 1)
            goto err;
