...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset)
{
    ...
    if ((flags & MAP_SHARED) ||
        ...
        !(fd == -1 || (file = (HANDLE) _get_osfhandle(fd)) != INVALID_HANDLE_VALUE) || length == 0)
    {
        return MAP_FAILED;
    }

    if ((handle = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, (DWORD) length, NULL)) == NULL)
        return MAP_FAILED;
    ...
    if ((retVal = (LPTSTR) MapViewOfFileEx(handle, FILE_MAP_ALL_ACCESS, 0, 0, length, addr)) == NULL)
    {
        dw = GetLastError();
    ...
    if (fd != -1)
    {
        if (SetFilePointer(file, (DWORD) offset, NULL, FILE_BEGIN) == INVALID_SET_FILE_POINTER)
            goto err;
        if (ReadFile(file, retVal, (DWORD) length, &nRead, NULL) == 0)
            goto err;
    }
