...
* @return 
*/
static exception_t alloc_thread(work_thread_t ** thread, void *(*thread_func) (void *), void *argvp)
{
    int i;
...
skyeye_cell_t *get_cell_by_thread_id(pthread_t id)
{
    ...
    skyeye_cell_t *cell = (skyeye_cell_t *) (thread->priv_data->obj);
...
{
    work_thread_t *pthread = (work_thread_t *) arg;

    pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
    pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
    pthread->thread_func(pthread->argvp);
    return;
}
...
void create_thread(void *(*start_funcp) (void *), void *argp, pthread_t * idp)
{
    ...
    if (alloc_thread(&pthread, start_funcp, argp) != No_exp)
    {
    ...
    do
    {
        ...
        res = pthread_create(idp, &attr, pthread_func, (void *) pthread);
        pthread_attr_destroy(&attr);
    } while (res == -1 && errno == EAGAIN);

    if (res == -1)
    {
        skyeye_log(Error_log, __FUNCTION__, "failed to create thread\n");
    }
...
void destroy_threads(void)
{
    ...
    for (i = 0; i < MAX_THREAD_NUMBER; i++)
    {
        ...
        if (pthread_pool[i].state != Blank_state)
        {
            if (pthread_equal(pthread_pool[i].id, pthread_self()))
            {
                continue;
            }
            int ret;

            ret = pthread_cancel(pthread_pool[i].id);
#ifndef __MINGW32__
