...
static void thread_scheduler(void)
{
    ...
    start_usec = (unsigned long long) ((litmp.QuadPart / freq) * 1000000);
    current_utime = start_usec;
#else
    struct timeval tv;
    ...
    while (1)
    {
...
#ifdef __MINGW32__
        current_utime = (unsigned long long) ((litmp.QuadPart / freq) * 1000000);
#else
        ...
        {
            /* Decrease dleta */
            tmp->delta -= passed_utime * time_num;
            if (tmp->delta > (int) tmp->expiration)
...
void list_thread_scheduler(void)
{
    ...
    LIST_FOREACH(tmp, &thread_head, list_entry)
    {
        printf("%d\t%d\t%d\t%s\n", tmp->id, tmp->expiration, tmp->delta,
               (tmp->mode == 0) ? "Oneshot_sched" : "Periodic_sched");
    }
}
