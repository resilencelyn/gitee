...
* @param size
*/
static inline void write_phys(memory_space_intf * memory_space, uint32 addr, uint8_t * buffer, int size)
{
    int i, fault;
    mem_bank_t *bank;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (memory_space->write(memory_space->conf_obj, addr + i, &buffer[i], 1) != No_exp)
        {
...
* @param size
*/
static inline void write_virt(memory_space_intf * memory_space, uint32 addr, uint8_t * buffer, int size)
{
    int i, fault;
    ...
    for (i = 0; i < size; i++)
    {
         ...
         */
        /* byte write */
        fault = memory_space->write(memory_space->conf_obj, addr + i, &buffer[i], 1);
...
static inline void tea_set(uint32 addr, uint8_t value, int size)
{
    int i, fault;
...
static int elf32_checkFile(struct Elf32_Header *file)
{
    if (file->e_ident[EI_MAG0] != ELFMAG0
        || file->e_ident[EI_MAG1] != ELFMAG1 || file->e_ident[EI_MAG2] != ELFMAG2 || file->e_ident[EI_MAG3] != ELFMAG3)
...
* @return 
*/
static exception_t load_exec(memory_space_intf * memory_space, const char *file, addr_type_t addr_type)
{
...
exception_t get_elf_start_addr(const char *file, generic_address_t * addr, generic_address_t * code_start,
                               generic_address_t * code_end)
{
    ...
    *addr = bfd_get_start_address(tmp_bfd);

    asection *ss = bfd_get_section_by_name(tmp_bfd, ".text");

    if (ss)
    {
        //fprintf(stderr,"size and vma lma is %d,%x,%x\n",bfd_section_size(tmp_bfd,ss),bfd_section_vma(tmp_bfd,ss),bfd_section_lma(tmp_bfd,ss));
        *code_start = bfd_section_vma(tmp_bfd, ss);
        *code_end = bfd_section_vma(tmp_bfd, ss) + bfd_section_size(tmp_bfd, ss);
    } else
    {
        fprintf(stderr, "do not get .text section\n");
    }
    ret = No_exp;
  out:
    if (tmp_bfd)
...
* @return 
*/
exception_t SKY_load_elf(memory_space_intf * memory_space, const char *filename, addr_type_t addr_type)
{
    if (is_elf(filename) == True)
        return load_exec(memory_space, filename, addr_type);
    else if (is_coff(filename))
        return load_coff(memory_space, filename, addr_type);
    else
        /* something wrong */
