...
static uint32_t io_read_byte(uint32_t addr)
{
    struct device_desc *dev;
    uint32_t data;
    int i;
    ...
    for (i = 0; i < config->mach->dev_count; i++)
    {
        ...
        if (dev->size == 0)
        {
            ...
            if (dev->read_byte(dev, addr, (uint8 *) & data) != ADDR_NOHIT)
                return data & 0xff;
...
static uint32_t io_read_halfword(uint32_t addr)
{
    struct device_desc *dev;
    uint32_t data;
    int i;
    ...
    for (i = 0; i < config->mach->dev_count; i++)
    {
        ...
        if (dev->size == 0)
        {
            if (dev->read_halfword(dev, addr, (uint16 *) & data) != ADDR_NOHIT)
                return data & 0xffff;
        } else if ((addr >= dev->base) && (addr < (dev->base + dev->size)))
        {
            if (dev->read_halfword(dev, addr, (uint16 *) & data) != ADDR_NOHIT)
...
static uint32_t io_read_word(uint32_t addr)
{
    struct device_desc *dev;
    uint32_t data;
    int i;
...
static void io_write_byte(uint32_t addr, uint32_t data)
{
    struct device_desc *dev;
    int i;
    skyeye_config_t *config = get_current_config();
    ...
    for (i = 0; i < config->mach->dev_count; i++)
    {
        ...
        if (dev->size == 0)
        {
            if (dev->write_byte(dev, addr, (uint8) data) != ADDR_NOHIT)
                return;
        } else if ((addr >= dev->base) && (addr < (dev->base + dev->size)))
        {
            if (dev->write_byte(dev, addr, (uint8) data) != ADDR_NOHIT)
...
static void io_write_halfword(uint32_t addr, uint32_t data)
{
    struct device_desc *dev;
    int i;
    skyeye_config_t *config = get_current_config();
    ...
    for (i = 0; i < config->mach->dev_count; i++)
    {
        ...
        if (dev->size == 0)
        {
            if (dev->write_halfword(dev, addr, (uint16) data) != ADDR_NOHIT)
                return;
        } else if ((addr >= dev->base) && (addr < (dev->base + dev->size)))
        {
            if (dev->write_halfword(dev, addr, (uint16) data) != ADDR_NOHIT)
...
static void io_write_word(uint32_t addr, uint32_t data)
{
    struct device_desc *dev;
    int i;
    skyeye_config_t *config = get_current_config();
