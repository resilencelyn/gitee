...
* @return 
*/
static int parse_mem(int num_params, const char *params[]);
...
* @return 
*/
static int do_mem_bank_option(skyeye_option_t * this_option, int num_params, const char *params[]);
static int do_bus_bank_option(skyeye_option_t * this_option, int num_params, const char *params[]);

static void insert_bank(mem_bank_t * bank);

char *ram_params[] = { "name", "type", "map", "addr", "size" };
static int do_ram_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
...
static int do_addr_space_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    ...
    for (i = 0; i < num_params; i++)
    {
        ...
        if (!strncmp("name", name, strlen(name)))
        {
            skyeye_config_t *config = get_current_config();

            config->mach->phys_mem = new_addr_space(value);
        }
    }
    return 0;
}
...
void init_bus()
{
    ...
    register_option("addr_space", do_addr_space_option, "instance addr_space class for machine in skyeye.conf");
