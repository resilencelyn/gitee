...
* @return 
*/
static uint32_t mem_read_byte(uint32_t addr)
{
    ...
    /* judge the alignment */
    if (SIM_get_current_align() == Align)
    {
        data = memory->rom[global_mbp - memmap->mem_banks][(addr - global_mbp->addr) >> 2];
    } else if (SIM_get_current_align() == UnAlign)
    {
    ...
    /* judge the endianess */
    if (SIM_get_current_endianess() == Little_endian)
    {
        offset = (((uint32_t) SIM_get_current_endianess() * 3) ^ (addr & 3)) << 3;      /* bit offset into the word */
        return (data >> offset & 0xffL);
    } else if (SIM_get_current_endianess() == Big_endian)
    {
        return data;
    }
...
* @return 
*/
static uint32_t mem_read_halfword(uint32_t addr)
{
    uint32_t data, offset;
    mem_bank_t *global_mbp = bank_ptr(addr);
    ...
    if (SIM_get_current_align() == Align)
    {
        data = global_memory.rom[global_mbp - memmap->mem_banks][(addr - global_mbp->addr) >> 2];
    } else if (SIM_get_current_align() == UnAlign)
    {
        uint8_t *base = &global_memory.rom[global_mbp - memmap->mem_banks][0];
    ...
    if (SIM_get_current_endianess() == Little_endian)
    {
        offset = (((uint32_t) SIM_get_current_endianess() * 2) ^ (addr & 2)) << 3;      /* bit offset into the word */
...
static uint32_t mem_read_word(uint32_t addr)
{
    ...
    /* judge the endianess */
    endian_t endian = SIM_get_current_endianess();
...
*/
static void mem_write_byte(uint32_t addr, uint32_t data)
{
    ...
    if (SIM_get_current_align() == Align)
    {
        temp = &global_memory.rom[global_mbp - memmap->mem_banks][(addr - global_mbp->addr) >> 2];
    } else if (SIM_get_current_align() == UnAlign)
    {
        uint8_t *base = &global_memory.rom[global_mbp - memmap->mem_banks][0];
    ...
    /* judge the endianess */
    endian_t endianess = SIM_get_current_endianess();

    if (endianess == Little_endian)
    {
        offset = (((uint32_t) endianess * 3) ^ (addr & 3)) << 3;        /* bit offset into the word */
...
* @param data the written data
*/
static void mem_write_halfword(uint32_t addr, uint32_t data)
{
    uint32_t *temp, offset;
    mem_bank_t *global_mbp = bank_ptr(addr);
    ...
    if (endianess == Little_endian)
    {
        offset = (((uint32_t) endianess * 2) ^ (addr & 2)) << 3;        /* bit offset into the word */
...
* @param data the written data
*/
static void mem_write_word(uint32_t addr, uint32_t data)
{
    uint32_t *temp;
    mem_bank_t *global_mbp = bank_ptr(addr);
...
exception_t mem_reset()
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        } else
        {
            ...
            } else
            {
                global_memory.rom[bank] =
                    mmap(mb[bank].addr, mb[bank].len, PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED, -1, 0);
        ...
        {
            if (fread(global_memory.rom[bank], 1, mb[bank].len, f) <= 0)
            {
                perror("fread");
            ...
            while (s < global_memory.rom_size[bank])
            {
                ...
                if (pref->endian == Big_endian)
                    swap = ((uint32_t) p[3]) |
                        (((uint32_t) p[2]) << 8) | (((uint32_t) p[1]) << 16) | (((uint32_t) p[0]) << 24);
                else
                    swap = ((uint32_t) p[0]) |
