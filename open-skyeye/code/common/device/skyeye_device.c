...
void set_device_default(struct device_desc *dev, struct device_default_value *def)
{
    ...
    {
        if ((strcasecmp(dev->name, def_p->name) == 0) || (strcasecmp(mc->machine_name, def_p->name) == 0))
        {
            if (dev->base == 0)
                dev->base = def_p->base;
            if (dev->size == 0)
            ...
            if (dev->intr.interrupts[0] == 0)
                memcpy(dev->intr.interrupts, def_p->interrupts, sizeof (def_p->interrupts));
            return;
        }
        def_p++;
    }
 ...
 * */
static struct device_module *find_module(const char *name, struct device_module_set *mod_set)
{
    int i;
...
* @return 
*/
static struct device_module_set *find_module_set(const char *name)
{
    int i;
...
int setup_device(char *dev_name, struct common_config *conf, void *option, machine_config_t * mc)
{
    ...
    mc->devices = realloc(mc->devices, sizeof (struct device_desc *) * (mc->dev_count + 1));
    if (mc->devices == NULL)
    {
    ...
    memcpy(dev->intr.interrupts, conf->interrupts, sizeof (conf->interrupts));
...
int register_device_module_set(struct device_module_set *mod_set)
{
    ...
    {
        mod_set_count_max += 4;
        global_mod_set = realloc(global_mod_set, sizeof (struct device_module_set *) * mod_set_count_max);
 ...
 * */
int register_device_module(char *name, struct device_module_set *mod_set, int (*setup) (struct device_desc * dev))
{
    ...
    {
        mod_set->count_max += 4;
        mod_set->mod = realloc(mod_set->mod, sizeof (struct device_module *) * mod_set->count_max);
        if (mod_set->mod == NULL)
            return 1;
 ...
 * 
 */
int setup_device_option(char *option_name, void *dev_option, int num_params, const char *params[])
{
...
exception_t set_general_device(general_device_t * dev_opt, int num_params, const char *params[])
{
    char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
    int i;
    ...
    {
        if (split_param(params[i], name, value) < 0)
            SKYEYE_ERR("Error: Wrong parameter \"%s\".\n", name);
        if (!strncmp("priority", name, strlen(name)))
        {
            dev_opt->priority = strtoul(value, NULL, 0);
        } else if (!strncmp("name", name, strlen(name)))
        {
            dev_opt->name = strdup(value);
        } else if (!strncmp("addr", name, strlen(name)))
        {
            dev_opt->addr = strtoul(value, NULL, 0);
        } else if (!strncmp("size", name, strlen(name)))
        {
            dev_opt->size = strtoul(value, NULL, 0);
        } else if (!strncmp("interrupt", name, strlen(name)))
        {
            dev_opt->interrupt = strdup(value);
        }
    }
...
int instance_general_device(skyeye_option_t * this_option, int num_params, const char *params[])
{
    char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
    int i;
    general_device_t dev_opt;
    ...
    conf_object_t *obj = pre_conf_obj(dev_opt.name, this_option->option_name);
    memory_space_intf *io_memory = (memory_space_intf *) SKY_get_interface(obj, MEMORY_SPACE_INTF_NAME);

    if (config->mach->phys_mem == NULL)
    {
        skyeye_log(Error_log, __FUNCTION__, "Configure error: Please instance addr_space\n");
        exit(0);
    }

    exception_t ret = add_map(config->mach->phys_mem, dev_opt.addr, dev_opt.size, 0x0, io_memory, dev_opt.priority, 1);

    if (ret != No_exp)
    {
        skyeye_log(Error_log, __FUNCTION__, "Can not register io memory for %s\n", dev_opt.name);
    }
    return obj;
}
