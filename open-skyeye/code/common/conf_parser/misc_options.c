...
int do_dbct_test_speed_sec(struct skyeye_option_t *this_opion, int num_params, const char *params[])
{
    if (num_params != 1)
    {
        goto error_out;
...
int do_regfile_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
...
int do_load_addr_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    ...
    for (i = 0; i < num_params; i++)
    {
        if (split_param(params[i], name, value) < 0)
            SKYEYE_ERR("Error: sound has wrong parameter \"%s\".\n", name);
...
* @return 
*/
int do_deprecated_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
...
* @return 
*/
int do_load_file_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    int i;
    char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
    unsigned long load_mask;
...
* @return 
*/
int do_load_elf_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    char result[64];
    exception_t ret;
...
* @return 
*/
int do_cpu_rate_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    uint32_t cpu_rate;
    skyeye_config_t *config = get_current_config();
    ...
    config->cpu_rate = 0;
    if (num_params >= 1)
        sscanf(params[0], "%d", &cpu_rate);
    config->cpu_rate = cpu_rate;
    return 0;
}
