...
conf_object_t *get_conf_obj_by_cast(void *obj, const char *type_string)
{
    ...
    conf_object_t *conf_obj = skyeye_mm_zero(sizeof (struct conf_object_s));
...
static void init_conf_object(conf_object_t * obj)
{
    //init_log_object(obj);
    /* for more */
}
...
conf_object_t *new_map_object(const char *objname, void *obj, object_type_t typ)
{
    ...
    if (conf_obj != NULL)
    {
        ...
        {
            fprintf(stderr, "Can not put the %s to the hash table\n", objname);
        ...
        {
            conf_obj->type = typ;
            /* create and put object successfully */
            return conf_obj;
        }
    }
...
{
    conf_object_t *conf_obj = new_map_object(objname, obj, ConfObject);

    if (conf_obj)
    {
        init_conf_object(conf_obj);
    }
    return conf_obj;
}
