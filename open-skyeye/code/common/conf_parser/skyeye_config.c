...
* @return 
*/
static int parse_line_unformatted(char *line)
{
#define MAX_PARAMS_LEN 40
    ...
    while ((ptr) && (!comment))
    {
        ...
        if (num_params < MAX_PARAMS_LEN)
        {
            ...
            params[num_params] = skyeye_mm_zero((string_i + 16) & 0xfffffff0);
    ...
    if (num_params > 0)
    {
        if (!strncmp(params[0], "arch", strlen("arch")) && !strncmp(params[1], "arm_dyncom", strlen("arm_dyncom")))
            Dyncom_Flag = 1;

        if (!strncmp(params[0], "cpu", strlen("cpu")) && !strncmp(params[1], "cortex_a9", strlen("cortex_a9"))
            && Dyncom_Flag == 1)
        {
            free(params[1]);
            params[1] = skyeye_mm_zero(strlen("arm11") + 1);
            memmove(params[1], "arm11", strlen("arm11") + 1);
        }

        if (!strncmp(params[0], "mach", strlen("mach")) && !strncmp(params[1], "am335x", strlen("am335x"))
            && Dyncom_Flag == 1)
        {
            free(params[1]);
            params[1] = skyeye_mm_zero(strlen("s3c6410x") + 1);
            memmove(params[1], "s3c6410x", strlen("s3c6410x") + 1);
        }
    }
#endif
...
* @return 
*/
exception_t skyeye_read_config(char *skyeye_conf_filename)
{
    FILE *config_fd;
    char *ret;
    char line[MAX_STR];
    ...
    if (config_fd = fopen(skyeye_conf_filename, "r"))
    {
        ...
        if (skyeye_config.uart.count == 0)
        {
            ...
            snprintf(tmp, sizeof (tmp), "uart: mod=pipe, desc=%s", inferior_io_terminal, inferior_io_terminal);
#endif
            if (parse_line_unformatted(tmp) < 0)
                retval = -1;
        }
#endif
        if (retval < 0)
