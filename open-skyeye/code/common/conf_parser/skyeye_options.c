/* skyeye_options.c - skyeye config file options' functions Copyright (C) 2003 Skyeye Develop Group
 ...
 */
int do_load_addr_option(skyeye_option_t * this_option, int num_params, const char *params[]);
do_load_file_option(skyeye_option_t * this_option, int num_params, const char *params[]);

do_load_elf_option(skyeye_option_t * this_option, int num_params, const char *params[]);
...
int skyeye_option_init(skyeye_config_t * config)
{
    ...
    register_option("load_file", do_load_file_option,
                    "Set load base address and size value for initrd file loading.\n");
    ...
    register_option("serial", do_serial_option, "Set serial parameters for uart pc communcaiton.\n");
    register_option("cpu_rate", do_cpu_rate_option, "Set CPU Rate.\n");
...
* @return 
*/
int do_dummy_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    return 0;
};
...
int do_cpu_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    int ret;
    ...
    if (skyeye_config.arch == NULL)
    {
        ...
        for (i = 0; i < MAX_SUPP_ARCH; i++)
        {
            if (skyeye_archs[i] == NULL)
                continue;
            if (!strncmp(default_arch, skyeye_archs[i]->arch_name, MAX_PARAM_NAME))
            {
        ...
        if (skyeye_config.arch == NULL)
        {
            SKYEYE_ERR("ERROR: No arch option found! Maybe you use low version of skyeye?\n");
            skyeye_exit(-1);
        }
    }
 ...
 */
#if 0
int do_step_disassemble_option(skyeye_option_t * this_option, int num_params, const char *params[])
{
    int i;
    ...
    for (i = 0; i < num_params; i++)
    {
        ...
        if (!strncmp(params[0], "on", 2) || !strncmp(params[0], "ON", 2))
        {
            ...
            return 0;
        } else if (!strncmp(params[0], "off", 3) || !strncmp(params[0], "OFF", 3))
        {
...
int parse_line_formatted(int num_params, const char *params[])
{
    ...
    while (sop != NULL)
    {
        ...
        {
            if (retval = sop->do_option(sop, num_params - 1, &params[1]) < 0)
            {
