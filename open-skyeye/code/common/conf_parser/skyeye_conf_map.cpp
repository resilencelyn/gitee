...
/* Create skyeye_map */
typedef map < string, void *>skyeye_map_t;
skyeye_map_t skyeye_map;

/* travel callback */
typedef exception_t(*travel_map_cb_t) (void *data, void *user_data);
...
exception_t skyeye_map_operate(se_entry * entry, map_method_t method)
{
    ...
    switch (method)
    {
        case SE_FIND:
            {
                ...
                {
                    skyeye_log(Error_log, "skyeye_map_operate", "Find skyeye_map error: %s is not found\n", key);
                    break;
                }
            }
        case SE_INSERT:
            {
                ...
                Insert_Pair = skyeye_map.insert(map < string, void *>::value_type(string(key), data));

                if (Insert_Pair.second)
                    return No_exp;
                else
                {
                    skyeye_log(Error_log, "skyeye_map_operate", "Insert skyeye_map error: %s already existed\n", key);
        ...
        case SE_DELETE:
            ...
            {
                skyeye_log(Error_log, "skyeye_map_operate", "Delete skyeye_map error!\n");
                break;
            }
        default:
            return Invarg_exp;
...
exception_t delete_conf_obj(char *objname)
{
    ...
    ret = skyeye_map_operate(&entry, SE_DELETE);
    if (ret != No_exp)
        return Not_found_exp;
    return No_exp;
}
...
exception_t skyeye_map_travel(travel_map_cb_t cb, void *user_data)
{
    skyeye_map_t::iterator it;
    exception_t retval;

    for (it = skyeye_map.begin(); it != skyeye_map.end(); it++)
    {
        retval = cb(it->second, user_data);
        if (retval != No_exp)
            return retval;
    }
    return No_exp;
...
static exception_t travel_map(void *data, void *user_data)
{
    struct travel_para *para = (struct travel_para *) user_data;

    return para->cb((conf_object_t *) data, para->user_data);
}

exception_t travel_conf_obj(travel_cb_t cb, void *user_data)
{
    struct travel_para para = { cb, user_data };
    return skyeye_map_travel(travel_map, &para);
}
