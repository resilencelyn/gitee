...
typedef enum
{ mmi_false = 0, mmi_true = 1 } mmi_bool_t;
...
mmi_instance_t mmi_register_instance(const char *modname, const char *instancename, void *instance_data,
 ...
 */
typedef int (*mmi_access) (uint32_t cpuid, void *obj, uint64_t paddr, mmi_bool_t wr, uint32_t size, uint64_t * buf,
                           uint8_t bytemask);
...
typedef enum
{ MMI_CONFIG_NEW_MODULE, MMI_CONFIG_DELETE_MODULE, MMI_CONFIG_INIT_DONE } mmi_config_t;
...
void mmi_register_asi_action(mmi_instance_t instance, uint32_t asi, mmi_ld_asi_action ld_handler,
