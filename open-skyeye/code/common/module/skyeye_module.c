...
exception_t SKY_load_module(const char *module_filename)
{
    ...
    if ((err_str = dlerrordup(err_str)) != NULL)
    {
        ...
        skyeye_log(Warning_log, __FUNCTION__, "%s,invalid module in file %s\n", err_str, module_filename);
        lt_dlclose(handler);
        return Invmod_exp;
    }
...
void SKY_load_all_modules(char *lib_dir, char *suffix)
{
    ...
    if (module_dir == NULL)
    {
        skyeye_log(Error_log, __FUNCTION__, "open library dir:%s Failed!\n", lib_dir);
        return;
    }
    if (suffix == NULL)
...
char *SKY_module_get_value(lt_dlhandle * handler, char *key)
{
    exception_t ret;
    const char *err_str = NULL;
...
{
    char *value;
    skyeye_module_t *module = get_module_by_name(modulename);

    if (module != NULL)
    {
        value = SKY_module_get_value(module->handler, key);
        return value;
    }
    return NULL;
}
