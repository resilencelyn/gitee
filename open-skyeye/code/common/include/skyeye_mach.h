...
#define MAX_ELF_FILENAME_LEN 256
typedef struct soc{
	conf_object_t *mach;
...
exception_t free_all_soc();

/***************************NEW ARCHITECTURE COMMON MACHINE ATTRIBUTE*************************/
#define MAX_CORE_NUM  8                  /* max support core numbers for machine*/
typedef struct machine_attr
{
	addr_space_t *space;
	conf_object_t *core[MAX_CORE_NUM];
	int core_num;
}common_machine_attr_t;
...
/*machine interface*/
typedef struct machine_interface
{
	conf_object_t *conf_obj;
	int (*set_space)(conf_object_t *mach, addr_space_t *space);
	int (*set_core)(conf_object_t *mach, conf_object_t *core);
	int (*set_current_core)(conf_object_t* mach, conf_object_t* core);
	conf_object_t* (*get_current_core)(conf_object_t* mach);
	conf_object_t *(*get_core_by_id)(conf_object_t *mach, int id);
	addr_space_t *(*get_space)(conf_object_t *mach);
	int (*update_boot_info)(conf_object_t *mach);
}skyeye_machine_intf;
#define MACHINE_INTF_NAME "skyeye_machine"
#ifdef __cplusplus
}
#endif
