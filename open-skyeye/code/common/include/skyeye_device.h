 ...
 * */
typedef struct device_default_value
{
	char *name;
...
typedef struct flash_common_interface{
	void (*fini) (void* obj);        /*finish routine */
	void (*reset) (void* obj);       /*reset device. */
	void (*update) (void* obj);      /*called by io_do_cycle */          


	int (*read_byte) (void* obj, uint32 addr, uint8 * result);
	int (*read_halfword) (void* obj, uint32 addr,
			uint16 * result);
	int (*read_word) (void* obj, uint32 addr, uint32 * result);

	int (*write_byte) (void* obj, uint32 addr, uint8 data);
	int (*write_halfword) (void* obj, uint32 addr, uint16 data);
	int (*write_word) (void* obj, uint32 addr, uint32 data);
}flash_common_intf_t;
