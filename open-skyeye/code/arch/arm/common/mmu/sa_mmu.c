...
static fault_t sa_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype)
{
    ...
    if (cache)
    {
        update_cache(state, va, data, datatype, cache, MAIN_D_CACHE(), real_va);
    } else
    {
        /*search mini cache */
        cache = mmu_cache_search(state, MINI_D_CACHE(), va);
        if (cache)
        {
            update_cache(state, va, data, datatype, cache, MINI_D_CACHE(), real_va);
        }
    }
...
static void sa_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value)
{
    mmu_regnum_t creg = BITS(16, 19) & 15;
