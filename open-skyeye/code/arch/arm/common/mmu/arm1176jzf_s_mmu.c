...
void cpu_system_control_register_process(ARMul_State *state, ARMword va, ARMword *data, ARMword datatype, ARMword RWFlag);
...
#if 0
fault_t mmu_translate(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr)
#endif
/*  ap: AP bits value.
 *  sop: section or page description  0:section 1:page
 */
fault_t mmu_translate(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr, int *ap, int *sop)
{
    {
        ...
        {
            state->memory_space_iface->read(state->memory_space, l1addr, &l1desc, 4);
        } else
        ...
        switch (l1desc & 3)
        {
            ...
            case 1:
                /* coarse page table */
                {
                    ...
                    if (state->memory_space != NULL)
                        state->memory_space_iface->read(state->memory_space, l2addr, &l2desc, 4);
                    else
...
static fault_t arm1176jzf_s_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype);
static fault_t arm1176jzf_s_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype);

int arm1176jzf_s_mmu_init(ARMul_State * state)
{
...
void arm1176jzf_s_mmu_exit(ARMul_State * state)
{
}

static fault_t arm1176jzf_s_mmu_load_instr(ARMul_State * state, ARMword va, ARMword * instr)
{
    fault_t fault;
    int c;                              /* cache bit */
    ...
    state->memory_space_iface->read(state->memory_space, pa, instr, 4);

    return 0;
}

static fault_t arm1176jzf_s_mmu_read_byte(ARMul_State * state, ARMword virt_addr, ARMword * data)
{
    /* ARMword temp,offset; */
    fault_t fault;
...
static fault_t arm1176jzf_s_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data)
{
    /* ARMword temp,offset; */
    fault_t fault;
...
static fault_t arm1176jzf_s_mmu_read_word(ARMul_State * state, ARMword virt_addr, ARMword * data)
{
...
static fault_t arm1176jzf_s_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype)
{
    ...
    if (va >= CPU_SYSTEM_CONTROL_SPACE_BASEADDR && va < (CPU_SYSTEM_CONTROL_SPACE_BASEADDR + 0x1000))
    {
        /*最后一个参数代表是读还是写,1表示写,0表示读*/
        cpu_system_control_register_process(state, va, data, datatype, 0);
        return NO_FAULT;
    }
    ...
    if (MMU_Disabled)
    {
        ...
        {    /* *data = mem_read_byte (state, va); */
            ret = state->memory_space_iface->read(state->memory_space, va, data, 1);
        } else if (datatype == ARM_HALFWORD_TYPE)
        {
            /* *data = mem_read_halfword (state, va); */
            ret = state->memory_space_iface->read(state->memory_space, va, data, 2);
        } else if (datatype == ARM_WORD_TYPE)
        {
            /* *data = mem_read_word (state, va); */
            ret = state->memory_space_iface->read(state->memory_space, va, data, 4);
        } else
        {
  ...
  skip_translation:
    ...
    if (datatype == ARM_BYTE_TYPE)
    {
        ...
        state->memory_space_iface->read(state->memory_space, pa | (real_va & 3), data, 1);
    } else if (datatype == ARM_HALFWORD_TYPE)
    {
        /* *data = mem_read_halfword (state, pa | (real_va & 2)); */
        state->memory_space_iface->read(state->memory_space, pa | (real_va & 3), data, 2);
        /* mem_read_raw(32, pa | (real_va & 2), data); */
    } else if (datatype == ARM_WORD_TYPE)
    {
        /* *data = mem_read_word (state, pa); */
        state->memory_space_iface->read(state->memory_space, pa, data, 4);
    } else
    {
...
static fault_t arm1176jzf_s_mmu_write_byte(ARMul_State * state, ARMword virt_addr, ARMword data)
{
    return arm1176jzf_s_mmu_write(state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t arm1176jzf_s_mmu_write_halfword(ARMul_State * state, ARMword virt_addr, ARMword data)
{
    return arm1176jzf_s_mmu_write(state, virt_addr, data, ARM_HALFWORD_TYPE);
}

static fault_t arm1176jzf_s_mmu_write_word(ARMul_State * state, ARMword virt_addr, ARMword data)
{
...
#else
            memcpy((void *) ((uint32_t) state->RAM + (addr - BANK0_START)), buf, count);
#endif
#else
...
void cpu_system_control_register_process(ARMul_State *state, ARMword va, ARMword *data, ARMword datatype, ARMword RWFlag)
{
    int reg_offset = va - CPU_SYSTEM_CONTROL_SPACE_BASEADDR;
    ...
    if (RWFlag == 1)
    {
        ...
        {
            case _ICT:
                state->scb_reg.ICT = *data;
                break;
            case _SYSTICK_CS:
                state->scb_reg.SYSTICK_CS = *data;
                break;
            case _SYSTICK_RL:
                state->scb_reg.SYSTICK_RL = *data;
                state->scb_reg.SYSTICK_CUR = *data;
                break;
            case _SYSTICK_CUR:
                //This register is write-clear
                state->scb_reg.SYSTICK_CUR = 0;
                state->scb_reg.SYSTICK_CS &= ~(1 << 16);
                break;
            case _SYSTICK_CAL:
                state->scb_reg.SYSTICK_CAL = *data;
                break;
            case _NVIC_ISER0:
                state->scb_reg.NVIC_ISER0 = *data;
                break;
            case _NVIC_ICER0:
                state->scb_reg.NVIC_ICER0 = *data;
                break;
            case _NVIC_ISPR0:
                state->scb_reg.NVIC_ISPR0 = *data;
                break;
            case _NVIC_ICPR0:
                state->scb_reg.NVIC_ICPR0 = *data;
                break;
            case _NVIC_IABR0:
                state->scb_reg.NVIC_IABR0 = *data;
                break;
            case _NVIC_IPR0:
                state->scb_reg.NVIC_IPR0 = *data;
                break;
            case _NVIC_IPR1:
                state->scb_reg.NVIC_IPR1 = *data;
                break;
            case _NVIC_IPR2:
                state->scb_reg.NVIC_IPR2 = *data;
                break;
            case _NVIC_IPR3:
                state->scb_reg.NVIC_IPR3 = *data;
                break;
            case _CPUID:
                state->scb_reg.CPUID = *data;
                break;
            case _ICSR:
                state->scb_reg.ICSR = *data;
                break;
            case _VTOR:
                state->scb_reg.VTOR = *data;
                break;
            case _AIRCR:
                state->scb_reg.AIRCR = *data;
                break;
            case _SCR:
                state->scb_reg.SCR = *data;
                break;
            case _CCR:
                state->scb_reg.CCR = *data;
                break;
            case _SHPR1:
                state->scb_reg.SHPR1 = *data;
                break;
            case _SHPR2:
                state->scb_reg.SHPR2 = *data;
                break;
            case _SHPR3:
                state->scb_reg.SHPR3 = *data;
                break;
            case _SHCSR:
                state->scb_reg.SHCSR = *data;
                break;
            case _CFSR:
                state->scb_reg.CFSR = *data;
                break;
            case _HFSR:
                state->scb_reg.HFSR = *data;
                break;
            case _DFSR:
                state->scb_reg.DFSR = *data;
                break;
            case _MMFAR:
                state->scb_reg.MMFAR = *data;
                break;
            case _BFAR:
                state->scb_reg.BFAR = *data;
                break;
            case _AFSR:
                state->scb_reg.AFSR = *data;
                break;
            default:
                skyeye_log(Error_log, __FUNCTION__, "Can not write the register at 0x%x\n", reg_offset);
                break;
        }
    } else
    {
        ...
        {
            case _ICT:
                *(uint32_t *)data = state->scb_reg.ICT;
                break;
            case _SYSTICK_CS:
                *(uint32_t *)data = state->scb_reg.SYSTICK_CS;
                break;
            case _SYSTICK_RL:
                *(uint32_t *)data = state->scb_reg.SYSTICK_RL;
                break;
            case _SYSTICK_CUR:
                *(uint32_t *)data = state->scb_reg.SYSTICK_CUR;
                break;
            case _SYSTICK_CAL:
                *(uint32_t *)data = state->scb_reg.SYSTICK_CAL;
                break;
            case _NVIC_ISER0:
                *(uint32_t *)data = state->scb_reg.NVIC_ISER0;
                break;
            case _NVIC_ICER0:
                *(uint32_t *)data = state->scb_reg.NVIC_ICER0;
                break;
            case _NVIC_ISPR0:
                *(uint32_t *)data = state->scb_reg.NVIC_ISPR0;
                break;
            case _NVIC_ICPR0:
                *(uint32_t *)data = state->scb_reg.NVIC_ICPR0;
                break;
            case _NVIC_IABR0:
                *(uint32_t *)data = state->scb_reg.NVIC_IABR0;
                break;
            case _NVIC_IPR0:
                *(uint32_t *)data = state->scb_reg.NVIC_IPR0;
                break;
            case _NVIC_IPR1:
                *(uint32_t *)data = state->scb_reg.NVIC_IPR1;
                break;
            case _NVIC_IPR2:
                *(uint32_t *)data = state->scb_reg.NVIC_IPR2;
                break;
            case _NVIC_IPR3:
                *(uint32_t *)data = state->scb_reg.NVIC_IPR3;
                break;
            case _CPUID:
                *(uint32_t *)data = state->scb_reg.CPUID;
                break;
            case _ICSR:
                *(uint32_t *)data = state->scb_reg.ICSR;
                break;
            case _VTOR:
                *(uint32_t *)data = state->scb_reg.VTOR;
                break;
            case _AIRCR:
                *(uint32_t *)data = state->scb_reg.AIRCR;
                break;
            case _SCR:
                *(uint32_t *)data = state->scb_reg.SCR;
                break;
            case _CCR:
                *(uint32_t *)data = state->scb_reg.CCR;
                break;
            case _SHPR1:
                *(uint32_t *)data = state->scb_reg.SHPR1;
                break;
            case _SHPR2:
                *(uint32_t *)data = state->scb_reg.SHPR2;
                break;
            case _SHPR3:
                *(uint32_t *)data = state->scb_reg.SHPR3;
                break;
            case _SHCSR:
                *(uint32_t *)data = state->scb_reg.SHCSR;
                break;
            case _CFSR:
                *(uint32_t *)data = state->scb_reg.CFSR;
                break;
            case _HFSR:
                *(uint32_t *)data = state->scb_reg.HFSR;
                break;
            case _DFSR:
                *(uint32_t *)data = state->scb_reg.DFSR;
                break;
            case _MMFAR:
                *(uint32_t *)data = state->scb_reg.MMFAR;
                break;
            case _BFAR:
                *(uint32_t *)data = state->scb_reg.BFAR;
                break;
            case _AFSR:
                *(uint32_t *)data = state->scb_reg.AFSR;
                break;
            default:
                skyeye_log(Error_log, __FUNCTION__, "Can not read the register at 0x%x\n", reg_offset);
                break;
        }
    }
}
...
static fault_t arm1176jzf_s_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype)
{
    ...
    {
        /*最后一个参数代表是读还是写,1表示写,0表示读*/
        cpu_system_control_register_process(state, va, &data, datatype, 1);
        return NO_FAULT;
    }
    ...
    if (MMU_Disabled)
    {
        ...
        {
            skyeye_log(Error_log, __FUNCTION__, "SKYEYE:1 arm1176jzf_s_mmu_write error: unknown data type %d\n",
                       datatype);
        }
        goto finished_write;
        //return 0;
  ...
  skip_translation:
    ...
    if (datatype == ARM_BYTE_TYPE)
    {
         ...
         * data);
         */
        memory_space_write(state, state->memory_space, pa | (real_va & 3), &data, 1);
    } else if (datatype == ARM_HALFWORD_TYPE)
         ...
         * data);
         */
        memory_space_write(state, state->memory_space, pa | (real_va & 3), &data, 2);
    else if (datatype == ARM_WORD_TYPE)
        /* mem_write_word (state, pa, data); */
        memory_space_write(state, state->memory_space, pa, &data, 4);
  finished_write:
#if DIFF_WRITE
...
ARMword arm1176jzf_s_mmu_mrc(ARMul_State * state, ARMword instr, ARMword * value)
{
...
static ARMword arm1176jzf_s_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value)
{
    mmu_regnum_t creg = BITS(16, 19) & 0xf;
...
static int arm1176jzf_s_mmu_v2p_dbct(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr)
{
    fault_t fault;
    int ap, sop;
