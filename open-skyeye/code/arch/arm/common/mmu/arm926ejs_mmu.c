...
int arm926ejs_mmu_init(ARMul_State * state)
{
    ...
    state->mmu.control = 0x50078 | 1 << 13;
...
static fault_t arm926ejs_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data)
{
    ...
    fault = arm926ejs_mmu_read(state, virt_addr, data, ARM_HALFWORD_TYPE);
    return fault;
}
...
static fault_t arm926ejs_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype)
{
    ...
    } else
    {
        ...
        if (datatype == ARM_BYTE_TYPE)
            ...
            state->memory_space_iface->read(state->memory_space, pa | (real_va & 2), data, 2);
        else if (datatype == ARM_WORD_TYPE)
            //*data = mem_read_word (state, pa);
            //bus_read(32, pa, data);
...
{
    return arm926ejs_mmu_write(state, virt_addr, data, ARM_HALFWORD_TYPE);
}
                                      ...
                                      void *buf, size_t count)
{
    if (state->mode == 1 || state->mode == 3)
    {
        ...
        memcpy(addr, buf, count);
#else
        if (state->memory_space_iface)
            state->memory_space_iface->write(state->memory_space, addr, buf, count);
#endif
    } else
    {
        if (state->memory_space_iface)
...
static fault_t arm926ejs_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype)
{
    ...
    if (cache)
    {
        update_cache(state, va, data, datatype, cache, ARM926EJS_I_CACHE(), real_va);
    }
    ...
    if (cache)
    {
        update_cache(state, va, data, datatype, cache, ARM926EJS_D_CACHE(), real_va);
    }
        ...
        if (b)
        {
            if (MMU_WBEnabled)
            {
                if (datatype == ARM_WORD_TYPE)
                    mmu_wb_write_bytes(state, ARM926EJS_WB(), pa, &data, 4);
                else if (datatype == ARM_HALFWORD_TYPE)
                    mmu_wb_write_bytes(state, ARM926EJS_WB(), (pa | (real_va & 2)), &data, 2);
                else if (datatype == ARM_BYTE_TYPE)
                    mmu_wb_write_bytes(state, ARM926EJS_WB(), (pa | (real_va & 3)), &data, 1);
            ...
            } else
            {
                if (datatype == ARM_WORD_TYPE)
                    ...
                    memory_space_write(state, state->memory_space, pa | (real_va & 2), &data, 2);
                else if (datatype == ARM_BYTE_TYPE)
                    /*
...
static void arm926ejs_mmu_cache_lockdown(ARMul_State * state, ARMword instr, ARMword value)
{
    int OPC_2 = BITS(5, 7) & 0x7;
}
static void arm926ejs_mmu_tlb_lockdown(ARMul_State * state, ARMword instr, ARMword value)
{
}
