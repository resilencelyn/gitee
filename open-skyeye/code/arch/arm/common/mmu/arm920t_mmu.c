...
static fault_t arm920t_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype)
{
    ...
    if (cache)
    {
        update_cache(state, va, data, datatype, cache, ARM920T_I_CACHE(), real_va);
    }
    ...
    if (MMU_Disabled)
    {
        ...
        if (datatype == ARM_BYTE_TYPE)
            ...
            //bus_write(8, va, data);       
            state->memory_space_iface->write(state->memory_space, va, &data, 1);
        else if (datatype == ARM_HALFWORD_TYPE)
            //mem_write_halfword (state, va, data);
            //bus_write(16,va, data);
            state->memory_space_iface->write(state->memory_space, va, &data, 2);
        else if (datatype == ARM_WORD_TYPE)
            //mem_write_word (state, va, data);
            //bus_write(32, va, data);
            state->memory_space_iface->write(state->memory_space, va, &data, 4);
        else
        {
    ...
    if (cache)
    {
        update_cache(state, va, data, datatype, cache, ARM920T_D_CACHE(), real_va);
    }
        ...
        if (b)
        {
            if (MMU_WBEnabled)
            {
                if (datatype == ARM_WORD_TYPE)
                    ...
                    mmu_wb_write_bytes(state, ARM920T_WB(), (pa | (real_va & 2)), &data, 2);
                else if (datatype == ARM_BYTE_TYPE)
                    mmu_wb_write_bytes(state, ARM920T_WB(), (pa | (real_va & 3)), &data, 1);
            ...
            } else
            {
                if (datatype == ARM_WORD_TYPE)
                    ...
                    state->memory_space_iface->write(state->memory_space, pa, &data, 4);
                else if (datatype == ARM_HALFWORD_TYPE)
                    ...
                    state->memory_space_iface->write(state->memory_space, pa | (real_va & 2), &data, 2);
                else if (datatype == ARM_BYTE_TYPE)
                    ...
                    state->memory_space_iface->write(state->memory_space, pa | (real_va & 3), &data, 4);
            }
        } else
        {
            mmu_wb_drain_all(state, ARM920T_WB());
            ...
            if (datatype == ARM_WORD_TYPE)
                ...
                state->memory_space_iface->write(state->memory_space, pa | (real_va & 3), &data, 1);
        }
    }
    return 0;
}
...
static void arm920t_mmu_cache_lockdown(ARMul_State * state, ARMword instr, ARMword value)
{
    int OPC_2 = BITS(5, 7) & 0x7;
}
static void arm920t_mmu_tlb_lockdown(ARMul_State * state, ARMword instr, ARMword value)
{
}
