...
static void a71_mmu_exit(ARMul_State * state)
{
    ...
    mmu_tlb_exit(TLB());
} static fault_t a71_mmu_read_byte(ARMul_State * state, ARMword virt_addr, ARMword * data)
{
...
static fault_t a71_mmu_read(ARMul_State * state, ARMword virt_addr, ARMword * data, ARMword datatype)
{
    ...
    if ((virt_addr & 3) && (datatype == ARM_WORD_TYPE)
        ...
        && (datatype == ARM_HALFWORD_TYPE) && (state->mmu.control & CONTROL_ALIGN_FAULT))
    {
...
static void a71_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value)
{
    mmu_regnum_t creg = BITS(16, 19) & 15;
