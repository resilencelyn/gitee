 ...
 *       0: sucess
 */
int mmu_cache_init(cache_s * cache_t, int width, int way, int set, int w_mode)
{
    int i, j;
    cache_set_t *sets;
    cache_line_t *lines;
 ...
 *              cache   :cache matched
 */
cache_line_t *mmu_cache_search(ARMul_State * state, cache_s * cache_t, ARMword va)
{
    int i;
    ...
    {
        if ((cache->tag & TAG_VALID_FLAG) && (tag == va_cache_align(cache->tag, cache_t)))
            return cache;
    }
    return NULL;
}
 ...
 *              cache   :cache matched
 */
cache_line_t *mmu_cache_search_by_index(ARMul_State * state, cache_s * cache_t, ARMword index)
{
    int way = cache_t->way;
 ...
 */
cache_line_t *mmu_cache_alloc(ARMul_State * state, cache_s * cache_t, ARMword va, ARMword pa)
{
    cache_line_t *cache;
    cache_set_t *set;
    int i;
    ...
    if (cache_t->w_mode == CACHE_WRITE_BACK)
    {
        ...
        for (i = 0; i < (cache_t->width >> WORD_SHT); i++, t += WORD_SIZE)
        {
            ...
            state->memory_space_iface->read(state->memory_space, t, &cache->data[i], 4);
        }
    }
    /*store tag and pa */
 ...
 * @cache : cache line
 */
void mmu_cache_write_back(ARMul_State * state, cache_s * cache_t, cache_line_t * cache)
{
    ARMword pa = cache->pa;
 ...
 */
void mmu_cache_clean_by_index(ARMul_State * state, cache_s * cache_t, ARMword index)
{
    cache_line_t *cache;
 ...
 */
void mmu_cache_invalidate(ARMul_State * state, cache_s * cache_t, ARMword va)
{
    cache_line_t *cache;
 ...
 */
void mmu_cache_invalidate_by_index(ARMul_State * state, cache_s * cache_t, ARMword index)
{
    cache_line_t *cache;
...
void mmu_cache_soft_flush(ARMul_State * state, cache_s * cache_t, ARMword pa)
{
    ARMword set, way;
    cache_line_t *cache;
