...
void ARMul_MCRR(ARMul_State * state, ARMword instr, ARMword source1, ARMword source2)
{
    unsigned cpab;
    ...
    while (cpab == ARMul_BUSY)
    {
        ...
        if (IntPending(state))
        {
            cpab = (state->MCRR[CPNum]) (state, ARMul_INTERRUPT, instr, 0, 0);
            return;
        } else
            cpab = (state->MCRR[CPNum]) (state, ARMul_BUSY, instr, source1, source2);
    }
    if (cpab == ARMul_CANT)
    {
...
void ARMul_MRRC(ARMul_State * state, ARMword instr, ARMword * dest1, ARMword * dest2)
{
    unsigned cpab;
    ARMword result1 = 0;
    ARMword result2 = 0;
    ...
    while (cpab == ARMul_BUSY)
    {
        ...
        if (IntPending(state))
        {
            cpab = (state->MRRC[CPNum]) (state, ARMul_INTERRUPT, instr, 0, 0);
            return;
        } else
            cpab = (state->MRRC[CPNum]) (state, ARMul_BUSY, instr, &result1, &result2);
    }
    if (cpab == ARMul_CANT)
    {
