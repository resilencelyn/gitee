...
extern bool_t getCondResult(ARMul_State *, ARMword);
...
static bool_t do_check_exception(ARMul_State * state)
{
    bool_t ret = False;
    ...
    } else if (!state->NirqSig && !IFLAG)
    {
        ...
        state->NirqSig = Prev_level;
        ret = True;
    } else if (state->Emulate < ONCE)
    {
        state->NextInstr = RESUME;
        ret = True;
    } else if (state->M0_exception != 0)
    {
        if (state->PRIMASK == 1)
        {
            if (state->M0_exception >= 11)
            {
            }
            state->M0_exception = 0;
        } else
        {
            /*entry stack */
            state->Reg[13] -= 0x4;
            state->space.write(state->space.conf_obj, state->Reg[13], &(state->Cpsr), 4);
            state->Reg[13] -= 0x4;
            state->space.write(state->space.conf_obj, state->Reg[13], &(state->realPC), 4);
            state->Reg[13] -= 0x4;
            state->space.write(state->space.conf_obj, state->Reg[13], &(state->Reg[14]), 4);
            state->Reg[13] -= 0x4;
            state->space.write(state->space.conf_obj, state->Reg[13], &(state->Reg[12]), 4);
            int i = 0;

            for (i = 3; i >= 0; i--)
            {
                state->Reg[13] -= 0x4;
                state->space.write(state->space.conf_obj, state->Reg[13], &(state->Reg[i]), 4);
            }
            state->MSP = state->Reg[13];

            uint32 data = 0;

            state->space.read(state->space.conf_obj, 0x8000000 + 0x4 * state->M0_exception, &data, 4);

            state->Reg[15] = data & 0xfffffffc;     //In the cortex-M0 load pc when the instructions will be & operation, the word alignment
            state->NextInstr = RESUME;
            state->Reg[14] = 0xfffffffd;    //entry exception thr process writes an EXC_RETURN value to the LR
            state->M0_exception = 0;
            SETT;
            ret = True;
        }
    } else if (state->M3_exception != 0)
    {
        /*entry stack */
        state->Reg[13] -= 0x4;
        state->memory_space_iface->write(state->memory_space, state->Reg[13], &(state->Cpsr), 4);
        state->Reg[13] -= 0x4;
        state->memory_space_iface->write(state->memory_space, state->Reg[13], &(state->realPC), 4);
        state->Reg[13] -= 0x4;
        state->memory_space_iface->write(state->memory_space, state->Reg[13], &(state->Reg[14]), 4);
        state->Reg[13] -= 0x4;
        state->memory_space_iface->write(state->memory_space, state->Reg[13], &(state->Reg[12]), 4);
        int i = 0;

        for (i = 3; i >= 0; i--)
        {
             state->Reg[13] -= 0x4;
             state->memory_space_iface->write(state->memory_space, state->Reg[13], &(state->Reg[i]), 4);
        }
        state->MSP = state->Reg[13];

        //Read VTOR register to get Vector Table Offset
        uint32_t vtor_addr = state->scb_reg.VTOR + (state->M3_exception * 4);
        uint32_t pc_addr = 0;
        if (state->TFlag)
        {
            state->Reg[14] = state->realPC | 0x1;
        } else
        {
            state->Reg[14] = state->realPC;
        }

        if (state->memory_space != NULL)
        {
            state->memory_space_iface->read(state->memory_space, vtor_addr, &pc_addr, 4);
        }

        state->Reg[15] = pc_addr & (~1);
        FLUSHPIPE;
        state->Reg[14] = 0xfffffffd;    //entry exception thr process writes an EXC_RETURN value to the LR
        state->M3_exception = 0;
        ret = True;
    } else
    {
        //do nothing
...
static ARMword get_current_pc(ARMul_State* state)
{
    ARMword pc = 0;
...
static bool_t do_xscale_decode_process(ARMul_State* state, ARMword instr)
{
    if (BIT(20) == 0 && BITS(25, 27) == 0)
    {
        if (BITS(4, 7) == 0xD)
        {
            ...
            else
            {
                ...
                state->Reg[BITS(12, 15) + 1] = ARMul_LoadWordN(state, addr + 4);
                if (wb)
                    LSBase = temp2;
            }
            return True;
        } else if (BITS(4, 7) == 0xF)
        {
            ...
            else
            {
                ...
                ARMul_StoreWordN(state, addr + 4, state->Reg[BITS(12, 15) + 1]);
...
static bool_t do_bfx_decode_process(ARMul_State* state, ARMword instr)
{
    ...
    if ((((int) BITS(21, 27)) == 0x3f) && (((int) BITS(4, 6)) == 0x5))
    {
        ...
        if ((Rd == 15) || (Rn == 15))
        {
            printf("In %s, Line:%d ARM instruction instr = 0x%x decode error at pc = 0x%x!\n", __func__, __LINE__, instr, state->realPC);
            /* stop skyeye simulation to check error */
            SIM_stop(NULL);
        } else if ((m + width) < 32)
        {
            data = state->Reg[Rn];
    ...
    } else if ((((int) BITS(21, 27)) == 0x3e) && ((int) BITS(4, 6) == 0x1))
    {
        ...
        {
            // bfc instrcution
            state->Reg[Rd] = clear_bits(state->Reg[Rd], msb, lsb);
            return True;
        }
        else if (((msb >= lsb) && (msb < 32)))
        {
            // bfi instrcution
            state->Reg[Rd] = copy_bits(state->Reg[Rd], state->Reg[Rn], msb, lsb);
            return True;
        }
    } else
    {
...
static void do_and_mul_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_eor_mla_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_sub_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_rsb_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
    ...
    case 6:
        ...
        if (BITS(4, 7) == 0x9)
        {
            ...
            int Ra = BITS(12, 15);
            int Rm = BITS(8, 11);
            int32_t op1 = (int32_t)state->Reg[Rn];
            int32_t op2 = (int32_t)state->Reg[Rm];
            int32_t addend = (int32_t)state->Reg[Ra];
            int32_t result = addend - op1*op2;
            state->Reg[Rd] = result;
        } else
        {
            rhs = DPRegRHS;
...
static void do_add_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_adc_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_sbc_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_rsc_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_tst_mrs_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_tstp_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_teq_msr_process(ARMul_State* state, ARMword instr, ARMword mainopc, ARMword pc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_teqp_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_cmp_mrs_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_cmpp_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_cmn_msr_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_cmnp_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_orr_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_mov_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_bic_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_mvn_process(ARMul_State* state, ARMword instr, ARMword mainopc)
{
    ARMword lhs, rhs, temp, dest;
...
static void do_xscale_mia_process(ARMul_State* state, ARMword instr)
{
    switch (BITS(18, 19))
    {
        case 0x0:
...
static bool_t do_cps_blx_decode_process(ARMul_State* state, ARMword instr, ARMword pc)
{
    if (state->is_v7)
    {
...
static tdstate thumb_insn_decode(ARMul_State* state, ARMword pc, ARMword instr, ARMword *new_instr)
{
    {
        /* Check if in Thumb mode.  */
        if (thumb_insn_is_16bit(instr))
        {
            return ARMul_ThumbDecode(state, pc, instr, new_instr);
        } else
        {
            ARMword insn2 = ARMul_LoadInstrN(state, pc + 2, 2) & 0xFFFF;

            instr = insn2 | (instr << 16);
            return ARMul_Thumb2Decode(state, pc, instr, new_instr);
        }
    }
}
...
ARMword isize;
#ifdef MODE32
bool_t thumb_insn_is_16bit(uint32_t instr)
{
    /* Return true if this is a 16 bit instruction. We must be precise
     * about this (matching the decode)
     */
    if ((instr >> 11 & 0x1f) < 0x1d)
    {
...
#endif
{
    ARMword instr = 0;                      /* The current instruction.  */
    ARMword dest = 0;                   /* Almost the DestBus.  */
    ARMword temp = 0;                       /* Ubiquitous third hand.  */
    ARMword pc = 0;                     /* The address of the current instruction.  */
    ARMword lhs = 0;                        /* Almost the ABus and BBus.  */
    ARMword rhs = 0;
    ARMword mainopc = 0;
    bool_t isException = False;

    /* Execute the next instruction.  */
    pc = get_current_pc(state);
    ...
    do
    {
        ...
        for (; idx < 17; idx++)
        {
            printf("[R%-2d]:%08x  ", idx, state->Reg[idx]);
            if (((idx + 1) % 5 == 0))
                printf("\n    ");
        }
        printf("\n");
        ...
        /* this function is used for code instruction coverage testing */
        SKY_pc_hap_occurred(NULL, state->cpu_obj, pc);

        /* Any exceptions check */
        isException = do_check_exception(state);
        if (isException)
        {
            goto donext;
        }
        ...
        if (TFLAG)
        {
            /* thumb instructions decoded */

            ARMword new_instr = 0;
            /* check IT-block next four instruction is excute */
            if (state->inITblock)
            {
                int i;
                int insnNum = state->it_block.insnNum;
                int runInsn = state->it_block.runInsn;
                bool_t isRun;

                state->it_block.runInsn++;
                state->it_block.insnNum--;

                if (!state->it_block.isExec[runInsn])
                    isRun = False;
                else
                    isRun = True;
                ...
                if (insnNum == 1)
                {
                    state->inITblock = False;
                    state->it_block.isExec[0] = False;
                    state->it_block.isExec[1] = False;
                    state->it_block.isExec[2] = False;
                    state->it_block.isExec[3] = False;
                    state->it_block.runInsn = 0;
                }
                if (!isRun)
                {
            ...
            tdstate tdRet = thumb_insn_decode(state, pc, instr, &new_instr);
            switch (tdRet)
            {
                case t_undefined:
                    printf("In %s, Line:%d thumb instruction instr = 0x%x not defined at pc = 0x%x!\n", __func__, __LINE__, instr, pc);
                    /* stop skyeye simulation to check error */
                    SIM_stop(NULL);
                case t_branch:
                    /* Already processed.  */
                ...
                case t_decoded:
                    ...
                    break;
                case t_t2_decoded:
                    /* thumb2 instruction decoded */
                    pc += 2;
                    state->Reg[15] += 2;
                    goto donext;
                default:
                    printf("In %s, Line:%d thumb instruction instr = 0x%x decode error at pc = 0x%x!\n", __func__, __LINE__, instr, pc);
                    /* stop skyeye simulation to check error */
                    SIM_stop(NULL);
                    break;
            }
        ...
        {
            /*ARM instructions decoded*/

            int cond = (instr >> 28) & 0xf;
            /* check condition instruction is excute */
            if (cond < 0xE)
            {
                bool_t doit = getCondResult(state, cond);

                if (!doit)
                    goto donext;
            } else if (cond == 0xF)
            {
                if (do_cps_blx_decode_process(state, instr, pc))
                {
                    goto donext;
                }
            }
        ...
        if (state->is_XScale)
        {
            if (do_xscale_decode_process(state, instr))
            {
                goto donext;
            }
        }
        ...
        if (state->is_v6)
        {
            ...
            /* eret instrution */
            if ((instr & 0xfffffff) == 0x160006e)
            {
                ARMul_SetR15(state, state->Reg[14]);
                int Spsr = ARMul_GetSPSR(state, state->Mode);
                ARMul_SetCPSR(state, Spsr);
                goto donext;
            }

            /* sbfx and ubfx instrutions */
            if (do_bfx_decode_process(state, instr))
            {
                goto donext;
            }
            ...
            switch (mainopc)
            {
                ...
                case 0x01:             /* ANDS reg and MULS */
                    do_and_mul_process(state, instr, mainopc);
                    break;
                case 0x02:             /* EOR reg and MLA */
                case 0x03:             /* EORS reg and MLAS */
                    do_eor_mla_process(state, instr, mainopc);
                    break;
                case 0x04:             /* SUB reg */
                case 0x05:             /* SUBS reg */
                    do_sub_process(state, instr, mainopc);
                    break;
                case 0x06:             /* RSB reg */
                case 0x07:             /* RSBS reg */
                    do_rsb_process(state, instr, mainopc);
                    break;
                case 0x08:             /* ADD reg */
                case 0x09:             /* ADDS reg */
                    do_add_process(state, instr, mainopc);
                    break;
                case 0x0a:             /* ADC reg */
                case 0x0b:             /* ADCS reg */
                    do_adc_process(state, instr, mainopc);
                    break;
                case 0x0c:             /* SBC reg */
                case 0x0d:             /* SBCS reg */
                    do_sbc_process(state, instr, mainopc);
                    break;
                case 0x0e:
                case 0x0f:             /* RSCS reg */
                    do_rsc_process(state, instr, mainopc);
                    break;
                case 0x10:             /* TST reg and MRS CPSR and SWP word.  */
                    do_tst_mrs_process(state, instr, mainopc);
                    break;
                case 0x11:             /* TSTP reg */
                    do_tstp_process(state, instr, mainopc);
                    break;
                case 0x12:             /* TEQ reg and MSR reg to CPSR (ARM6).  */
                    do_teq_msr_process(state, instr, mainopc, pc);
                    break;
                case 0x13:             /* TEQP reg */
                    do_teqp_process(state, instr, mainopc);
                    break;
                case 0x14:             /* CMP reg and MRS SPSR and SWP byte.  */
                    do_cmp_mrs_process(state, instr, mainopc);
                    break;
                case 0x15:             /* CMPP reg.  */
                    do_cmpp_process(state, instr, mainopc);
                    break;
                case 0x16:             /* CMN reg and MSR reg to SPSR */
                    do_cmn_msr_process(state, instr, mainopc);
                    break;
                case 0x17:             /* CMNP reg */
                    do_cmnp_process(state, instr, mainopc);
                    break;
                case 0x18:             /* ORR reg */
                case 0x19:             /* ORRS reg */
                    do_orr_process(state, instr, mainopc);
                    break;
                case 0x1a:             /* MOV reg */
                case 0x1b:             /* MOVS reg */
                    do_mov_process(state, instr, mainopc);
                    break;
                case 0x1c:             /* BIC reg */
                case 0x1d:             /* BICS reg */
                    do_bic_process(state, instr, mainopc);
                    break;
                case 0x1e:             /* MVN reg */
                case 0x1f:             /* MVNS reg */
                    do_mvn_process(state, instr, mainopc);
                    break;
                        ...
                        if (DESTReg == 15)
                        {
                            ...
                            } else if (BITS(4,11) == 0)
                            {
                                if (BITS(0,3) == 4)  /* SEV */
                                {
                                    ...
                                    {
                                        state->event_iface->set(state->event,1,0);
                                    } else
                                    {
                                        printf("sev erro! pc at 0x%x \n", state->realPC);
                                    }
                ...
                case 0x71:             /* Load Word, No WriteBack, Pre Dec, Reg.  */
                    if (BITS(4,7) == 1)
                    {
                        ...
                        src1 = state->Reg[rn];
                        src2 = state->Reg[rm];
                        if (src2 & 0x7FFFFFFF == 0)
                        {
                            state->Reg[rd] = src1 | 0x7FFFFFFF;
                            skyeye_log(Error_log, __FUNCTION__, "sdiv instruction reg[rm] = 0,can not sidv\n");
                        }
                        else
                        {
                            src1 = src1 / src2;
                ...
                case 0x73:             /* Load Word, WriteBack, Pre Dec, Reg.  */
                    if (BITS(4,7) == 1)
                    {
                        ...
                        if (!src2)
                        {
                            ...
                            skyeye_log(Error_log, __FUNCTION__, "udiv instruction reg[rm] = 0,can not udiv\n");
                        }
                        else
                        {
                            src1 = src1 / src2;
                ...
                case 0xe2:
                    ...
                    if (state->is_XScale)
                    {
                        do_xscale_mia_process(state, instr);
                        break;
                    }
                case 0xe0:
                case 0xe4:
        ...
        case 0x6b:
            {
                ...
                switch (BITS(4, 11))
                {
                    ...
                    case 0xf3:
                        DEST =
                            ((RHS & 0xFF) << 24) | ((RHS & 0xFF00)) << 8 | ((RHS & 0xFF0000) >> 8) |
                            ((RHS & 0xFF000000) >> 24);
                        return 1;
                    case 0xfb:
                        DEST =
                            ((RHS & 0xFF) << 8) | ((RHS & 0xFF00)) >> 8 | ((RHS & 0xFF0000) << 8) |
                            ((RHS & 0xFF000000) >> 8);
                        return 1;
                    default:
                        break;
