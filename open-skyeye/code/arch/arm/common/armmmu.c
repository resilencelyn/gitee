...
extern mmu_ops_t ftc662_mmu_ops;
...
int mmu_init(ARMul_State * state)
{
    ...
    switch (state->cpu->cpu_val & state->cpu->cpu_mask)
    {
        ...
        case 0xc090:
            ...
            break;
        case 0xc080:
            SKYEYE_INFO("SKYEYE: use cortex_a8 mmu ops\n");
            state->mmu.ops = cortex_a8_mmu_ops;
            register_bus_operation(arm_mmu_read, arm_mmu_write);
            break;
        case 0xc140:
            SKYEYE_INFO("SKYEYE: use cortex_r4 mmu ops\n");
            state->mmu.ops = cortex_r4_mpu_ops;
            break;
        case 0xc660:
            SKYEYE_INFO("SKYEYE: use ftc662 mmu ops\n");
            state->mmu.ops = ftc662_mmu_ops;
            register_bus_operation(arm_mmu_read, arm_mmu_write);
            break;
        default:
            fprintf(stderr,
