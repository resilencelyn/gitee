...
#endif
#ifdef VFP_INTERPRETER_TABLE
    INTERPRETER_TRANSLATE(vfpinstr), INTERPRETER_TRANSLATE(vfpinstr),
#endif
#ifdef VFP_INTERPRETER_LABEL
    &&VFPLABEL_INST, &&VFPLABEL_INST,
#endif
#ifdef VFP_INTERPRETER_STRUCT
...
ARM_INST_PTR INTERPRETER_TRANSLATE(vfpinstr) (unsigned int inst, int index)
{
    ...
    inst_cream->d =
        (inst_cream->single ? BITS(inst, 12, 15) << 1 | BIT(inst, 22) : BITS(inst, 12, 15) | BIT(inst, 22) << 4);
    unsigned int imm8 = BITS(inst, 16, 19) << 4 | BITS(inst, 0, 3);

    if (inst_cream->single)
        inst_cream->imm =
            BIT(imm8, 7) << 31 | (BIT(imm8, 6) == 0) << 30 | (BIT(imm8, 6) ? 0x1f : 0) << 25 | BITS(imm8, 0, 5) << 19;
    else
        inst_cream->imm =
            BIT(imm8, 7) << 31 | (BIT(imm8, 6) == 0) << 30 | (BIT(imm8, 6) ? 0xff : 0) << 22 | BITS(imm8, 0, 5) << 16;
    return inst_base;
}
#endif
#ifdef VFP_INTERPRETER_IMPL
...
void VMOVR(ARMul_State * state, ARMword single, ARMword d, ARMword m)
{
    ...
    {
        /* Check endian please */
        DBG("\ts[%d-%d] <= s[%d-%d][%x-%x]\n", d * 2 + 1, d * 2, m * 2 + 1, m * 2, state->ExtReg[m * 2 + 1],
            state->ExtReg[m * 2]);
...
void VMOVBRRD(ARMul_State * state, ARMword to_arm, ARMword t, ARMword t2, ARMword n, ARMword * value1, ARMword * value2)
{
    ...
    if (to_arm)
    {
        DBG("\tr[%d-%d] <= s[%d-%d]=[%x-%x]\n", t2, t, n * 2 + 1, n * 2, state->ExtReg[n * 2 + 1],
            state->ExtReg[n * 2]);
...
VFPLABEL_INST:
{
    ...
    if ((inst_base->cond == 0xe) || CondPassed(cpu, inst_base->cond))
    {
        ...
        unsigned int base =
            (inst_cream->n == 15 ? (cpu->Reg[inst_cream->n] & 0xFFFFFFFC) + 8 : cpu->Reg[inst_cream->n]);
        ...
        } else
        {
            ...
            if (fault)
                goto MMU_EXCEPTION;
            DBG("\taddr[%x-%x] <= s[%d-%d]=[%x-%x]\n", addr + 4, addr, inst_cream->d * 2 + 1, inst_cream->d * 2,
                cpu->ExtReg[inst_cream->d * 2 + 1], cpu->ExtReg[inst_cream->d * 2]);
        }
    }
    cpu->Reg[15] += GET_INST_SIZE(cpu);
        ...
        for (i = 0; i < inst_cream->regs; i++)
        {
            ...
            } else
            {
                ...
                if (fault)
                    goto MMU_EXCEPTION;
                fault =
                    interpreter_write_memory(core, addr + 4, phys_addr, cpu->ExtReg[(inst_cream->d + i) * 2 + 1], 32);
                if (fault)
                    goto MMU_EXCEPTION;
                DBG("\taddr[%x-%x] <= s[%d-%d]=[%x-%x]\n", addr + 4, addr, (inst_cream->d + i) * 2 + 1,
                    (inst_cream->d + i) * 2, cpu->ExtReg[(inst_cream->d + i) * 2 + 1],
                    cpu->ExtReg[(inst_cream->d + i) * 2]);
                addr += 8;
            }
        }
    ...
    if ((inst_base->cond == 0xe) || CondPassed(cpu, inst_base->cond))
    {
        ...
        for (i = 0; i < inst_cream->regs; i++)
        {
            ...
            } else
            {
                ...
                if (fault)
                    goto MMU_EXCEPTION;
                DBG("\ts[%d-%d] <= [%x-%x] addr[%x-%x]\n", (inst_cream->d + i) * 2 + 1, (inst_cream->d + i) * 2, value2,
                    value1, addr + 4, addr);
...
VFPLABEL_INST:
{
    ...
    if ((inst_base->cond == 0xe) || CondPassed(cpu, inst_base->cond))
    {
        ...
        } else
        {
            ...
            DBG("\ts[%d-%d] <= [%x-%x] addr[%x-%x]\n", inst_cream->d * 2 + 1, inst_cream->d * 2, word2, word1, addr + 4,
                addr);
        }
    }
    cpu->Reg[15] += GET_INST_SIZE(cpu);
        ...
        for (i = 0; i < inst_cream->regs; i++)
        {
            ...
            } else
            {
                ...
                if (fault)
                    goto MMU_EXCEPTION;
                fault =
                    interpreter_read_memory(core, addr + 4, phys_addr, cpu->ExtReg[(inst_cream->d + i) * 2 + 1], 32);
                if (fault)
                    goto MMU_EXCEPTION;
                DBG("\ts[%d-%d] <= [%x-%x] addr[%x-%x]\n", (inst_cream->d + i) * 2 + 1, (inst_cream->d + i) * 2,
                    cpu->ExtReg[(inst_cream->d + i) * 2 + 1], cpu->ExtReg[(inst_cream->d + i) * 2], addr + 4, addr);
                addr += 8;
            }
        }
        if (inst_cream->wback)
