...
u32 vfp_double_normaliseround(ARMul_State * state, int dd, struct vfp_double *vd, u32 fpscr, u32 exceptions,
                              const char *func)
{
 ...
 */
static u32 vfp_propagate_nan(struct vfp_double *vdd, struct vfp_double *vdn, struct vfp_double *vdm, u32 fpscr)
{
...
static u32 vfp_double_fadd_nonnumber(struct vfp_double *vdd, struct vfp_double *vdn, struct vfp_double *vdm, u32 fpscr)
{
...
static u32 vfp_double_add(struct vfp_double *vdd, struct vfp_double *vdn, struct vfp_double *vdm, u32 fpscr)
{
    u32 exp_diff;
    u64 m_sig;
...
static u32 vfp_double_multiply(struct vfp_double *vdd, struct vfp_double *vdn, struct vfp_double *vdm, u32 fpscr)
{
    vfp_double_dump("VDN", vdn);
    vfp_double_dump("VDM", vdm);
 ...
 * sd = sn * sm
 */
u32 vfp_double_fmul(ARMul_State * state, int dd, int dn, int dm, u32 fpscr)
{
 ...
 * sd = sn + sm
 */
u32 vfp_double_fadd(ARMul_State * state, int dd, int dn, int dm, u32 fpscr)
{
    struct vfp_double vdd, vdn, vdm;
    u32 exceptions;
 ...
 * sd = sn - sm
 */
u32 vfp_double_fsub(ARMul_State * state, int dd, int dn, int dm, u32 fpscr)
{
    struct vfp_double vdd, vdn, vdm;
    u32 exceptions;
 ...
 * sd = sn / sm
 */
u32 vfp_double_fdiv(ARMul_State * state, int dd, int dn, int dm, u32 fpscr)
{
    struct vfp_double vdd, vdn, vdm;
    u32 exceptions = 0;
    int tm, tn;
...
u32 vfp_double_cpdo(ARMul_State * state, u32 inst, u32 fpscr)
{
    ...
    for (vecitr = 0; vecitr <= veclen; vecitr += 1 << FPSCR_LENGTH_BIT)
    {
        ...
        if (op == FOP_EXT)
            pr_debug("VFP: itr%d (%c%u) = op[%u] (d%u)\n", vecitr >> FPSCR_LENGTH_BIT, type, dest, dn, dm);
        else
