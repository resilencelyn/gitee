...
#undef VFP_STC_TRANS
    ...
    skyeye_log(Debug_log, __FUNCTION__,
               "Can't identify %x, CoProc %x, CRd %x, Rn %x, imm8 %x, P %x, U %x, D %x, W %x\n", instr, CoProc, CRd, Rn,
               imm8, P, U, D, W);
...
{
    DBG("VFP put double: s[%d-%d] <= [%08x-%08x]\n", reg * 2 + 1, reg * 2, (uint32_t) (val >> 32),
        (uint32_t) (val & 0xffffffff));
    if (state->is_v8)
    {
        if(state->isAArch64)
        {
            state->V[reg].dword.d[0] = val;
        }
        else /* isAArch32 */
        {
