...
extern unsigned xscale_cp15_cp_access_allowed(ARMul_State * state, unsigned reg, unsigned cpnum);
...
NoCoPro4R(ARMul_State * state ATTRIBUTE_UNUSED,
          unsigned a ATTRIBUTE_UNUSED, ARMword b ATTRIBUTE_UNUSED, ARMword c ATTRIBUTE_UNUSED)
{
    return ARMul_CANT;
}
...
static unsigned
do_mcrr(ARMul_State * state ATTRIBUTE_UNUSED,
          unsigned a ATTRIBUTE_UNUSED,
          ARMword b ATTRIBUTE_UNUSED, ARMword c ATTRIBUTE_UNUSED, ARMword d ATTRIBUTE_UNUSED)
{
    int Rt, Rt2, coproc, opc1, CRm;
    ...
    if (coproc == 15 && CRm == 2)
    {
        ...
        {
            case 0:
                state->system_reg.TTBR0_EL1 = (uint64_t)(d) << 32 | (c);
                state->mmu.TTBR0.all = (uint64_t)(d) << 32 | (c);
                state->mmu.translation_table_base0 = state->mmu.TTBR0.all; /* Compatible with older versions of the code */
                //printf("In %s,mmu.TTBR0.all 0x%llx at PC = 0x%llx \n", __func__, state->mmu.TTBR0.all, state->realPC);
                break;
            case 1:
                state->system_reg.TTBR1_EL1 = (uint64_t)(d) << 32 | (c);
                break;
            default:
                printf("In %s, Not Implement opc1 = %d at PC = 0x%x\n", __func__, opc1, state->realPC);
                break;
        }
    }
    else if ( coproc == 15 || CRm == 14)
    {
        switch (opc1)
        {
            case 0:
                state->system_reg.CNTPCT_EL0 = (uint64_t)(d) << 32;
                state->system_reg.CNTPCT_EL0 += c;
                break;
            case 1:
                state->system_reg.CNTVCT_EL0 = (uint64_t)(d) << 32;
                state->system_reg.CNTVCT_EL0 += c;
                break;
            case 2:
                state->system_reg.CNTP_CVAL_EL0 = (uint64_t)(d) << 32;
                state->system_reg.CNTP_CVAL_EL0 += c;
                break;
            case 3:
                state->system_reg.CNTV_CVAL_EL0 = (uint64_t)(d) << 32;
                state->system_reg.CNTV_CVAL_EL0 += c;
                break;
            default :
                printf("In %s, Not Implement opc1 = %d at PC = %x\n", __func__, opc1, state->realPC);
                break;
        }
    }
...
static unsigned
do_mrrc(ARMul_State * state ATTRIBUTE_UNUSED,
          unsigned a ATTRIBUTE_UNUSED,
          ARMword b ATTRIBUTE_UNUSED, ARMword * c ATTRIBUTE_UNUSED, ARMword * d ATTRIBUTE_UNUSED)
{
    int Rt, Rt2, coproc, opc1, CRm;
    ...
    if ( coproc == 15 && CRm == 14)
    {
        ...
        {
            case 0:
                *c = (((state->system_reg.CNTPCT_EL0) << 32) >> 32);
                *d = ((state->system_reg.CNTPCT_EL0) >> 32);
                break;
            case 1:
                *c = (((state->system_reg.CNTVCT_EL0) << 32) >> 32);
                *d = ((state->system_reg.CNTVCT_EL0) >> 32);
                break;
            case 2:
                *c = (((state->system_reg.CNTP_CVAL_EL0) << 32) >> 32);
                *d = ((state->system_reg.CNTP_CVAL_EL0) >> 32);
                break;
            case 3:
                *c = (((state->system_reg.CNTV_CVAL_EL0) << 32) >> 32);
                *d = ((state->system_reg.CNTV_CVAL_EL0) >> 32);
                break;
            default :
               printf("In %s, Not Implement opc1 = %d at PC = %x\n", __func__, opc1, state->realPC);
                break;
...
static unsigned XScale_cp15_init(ARMul_State * state ATTRIBUTE_UNUSED)
{
    int i;
...
static unsigned check_cp15_access(ARMul_State * state, unsigned reg, unsigned CRm, unsigned opcode_1, unsigned opcode_2)
{
...
static unsigned XScale_cp13_init(ARMul_State * state ATTRIBUTE_UNUSED)
{
    int i;
...
static unsigned check_cp13_access(ARMul_State * state, unsigned reg, unsigned CRm, unsigned opcode_1, unsigned opcode_2)
{
...
static unsigned XScale_cp14_init(ARMul_State * state ATTRIBUTE_UNUSED)
{
    int i;
...
static unsigned check_cp14_access(ARMul_State * state, unsigned reg, unsigned CRm, unsigned opcode1, unsigned opcode2)
{
...
unsigned ARMul_CoProInit(ARMul_State * state)
{
    ...
    if (state->is_ep9312)
    {
        ARMul_CoProAttach(state, 4, NULL, NULL, DSPLDC4, DSPSTC4, DSPMRC4, DSPMCR4, NULL, NULL, DSPCDP4, NULL, NULL);
        ARMul_CoProAttach(state, 5, NULL, NULL, DSPLDC5, DSPSTC5, DSPMRC5, DSPMCR5, NULL, NULL, DSPCDP5, NULL, NULL);
        ARMul_CoProAttach(state, 6, NULL, NULL, NULL, NULL, DSPMRC6, DSPMCR6, NULL, NULL, DSPCDP6, NULL, NULL);
    } else
    {
        ARMul_CoProAttach(state, 4, NULL, NULL, ValLDC, ValSTC, ValMRC, ValMCR, NULL, NULL, ValCDP, NULL, NULL);

        ARMul_CoProAttach(state, 5, NULL, NULL, NULL, NULL, ValMRC, ValMCR, NULL, NULL, IntCDP, NULL, NULL);
    }
    ...
    if (state->is_XScale)
    {
        ...
        if (state->is_pxa27x)
        {
            ARMul_CoProAttach(state, 6, NULL, NULL, NULL, NULL, xscale_cp6_mrc, NULL, NULL, NULL, NULL, NULL, NULL);
        }
        //chy 2005-09-19 end------------- 
        ARMul_CoProAttach(state, 13, xscale_cp13_init,
                          xscale_cp13_exit, xscale_cp13_ldc,
                          xscale_cp13_stc, xscale_cp13_mrc,
                          xscale_cp13_mcr, NULL, NULL, xscale_cp13_cdp, xscale_cp13_read_reg, xscale_cp13_write_reg);

        ARMul_CoProAttach(state, 14, xscale_cp14_init,
                          xscale_cp14_exit, xscale_cp14_ldc,
                          xscale_cp14_stc, xscale_cp14_mrc,
                          xscale_cp14_mcr, NULL, NULL, xscale_cp14_cdp, xscale_cp14_read_reg, xscale_cp14_write_reg);
        //chy: 2003-08-24.
        ARMul_CoProAttach(state, 15, xscale_cp15_init,
                          xscale_cp15_exit, xscale_cp15_ldc,
                          xscale_cp15_stc, xscale_cp15_mrc,
                          xscale_cp15_mcr, NULL, NULL, xscale_cp15_cdp, xscale_cp15_read_reg, xscale_cp15_write_reg);
    } else if (state->is_v6)
    {
        ...
        ARMul_CoProAttach(state, 15, MMUInit, NULL, NULL, NULL, MMUMRC, MMUMCR, NULL, NULL, NULL, NULL, NULL);
    } else if (state->is_v7)
    {
        ARMul_CoProAttach(state, 10, VFPInit, NULL, VFPLDC, VFPSTC,
                          VFPMRC, VFPMCR, VFPMRRC, VFPMCRR, VFPCDP, NULL, NULL);
    ...
    if (state->is_iWMMXt)
    {
        ARMul_CoProAttach(state, 0, NULL, NULL, IwmmxtLDC, IwmmxtSTC, NULL, NULL, IwmmxtCDP, NULL, NULL);

        ARMul_CoProAttach(state, 1, NULL, NULL, NULL, NULL, IwmmxtMRC, IwmmxtMCR, IwmmxtCDP, NULL, NULL);
    }
#endif
    //-----------------------------------------------------------------------------
    //chy 2004-05-25, found the user/system code visit CP 1,2, so I add below code.
    ARMul_CoProAttach(state, 1, NULL, NULL, NULL, NULL, ValMRC, ValMCR, NULL, NULL, NULL, NULL, NULL);
    ARMul_CoProAttach(state, 2, NULL, NULL, ValLDC, ValSTC, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    //------------------------------------------------------------------------------
...
ARMul_CoProAttach(ARMul_State * state,
                  ...
                  ARMul_MCRs * mcr,
                  ARMul_MRRCs * mrrc,
                  ARMul_MCRRs * mcrr, ARMul_CDPs * cdp, ARMul_CPReads * read, ARMul_CPWrites * write)
{
    if (init != NULL)
...
void ARMul_CoProDetach(ARMul_State * state, unsigned number)
{
    ARMul_CoProAttach(state, number, NULL, NULL,
                      NoCoPro4R, NoCoPro4W, NoCoPro4W, NoCoPro4R, do_mrrc, do_mcrr, NoCoPro3R, NULL, NULL);
