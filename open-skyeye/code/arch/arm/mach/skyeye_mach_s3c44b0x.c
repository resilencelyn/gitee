...
static int s3c44b0x_pending_intr(unsigned int irq)
{
    ...
    if (!((io.intcon & 0x2) == 0x0 || (io.intcon & 0x5) == 0x4))
        return 1;
    if (io.intmsk & (0x1 << INT_GLOBAL))
        return 1;
...
static void s3c44b0x_interrupt_write(ARMul_State * state, ARMword addr, ARMword data)
{
    ...
    switch (addr)
    {
        ...
        case I_ISPC:
            if ((io.intcon & 0x2) != 0x0 || ((data & 0x3ffffff) & ~io.intmod) == 0)
                break;
            io.intpnd &= (~data & 0x3ffffff);
            break;

        case F_ISPC:
            if ((io.intcon & 0x5) != 0x4 || ((data & 0x3ffffff) & io.intmod) == 0)
                break;
            io.intpnd &= (~data & 0x3ffffff);
            break;
...
static void s3c44b0x_uart_do_cycle(ARMul_State * state)
{
    ...
    do
    {
        ...
        if ((*pUcon & 0x3) == bdma_mask || (*pUcon & 0xc) == (bdma_mask << 2))
        {                               /* BDMA request */
            ...
            do
            {
                if (s3c44b0x_dma_is_valid(index) != 1)
                    break;
                if (((DMA_CCNT(io.dma[index]) >> 30) & 0x3) != 0x2)
                    break;

                if ((DMA_CDES(io.dma[index]) & 0xfffffff) == utxhb && tx_ready)
                    mask |= (bdma_mask << 2);
                if ((DMA_CSRC(io.dma[index]) & 0xfffffff) == urxhb && !rx_empty)
                    mask |= bdma_mask;
                if ((*pUcon & mask) != 0)
                    s3c44b0x_dma_proccess(state, index);
            } while (0);
        }
        ...
        {                               /* FIFO mode */
            ...
            if ((*pUfstat & 0x100) == 0 && (*pUcon & 0x3) != 0x0)
            {                           /* FIFO not full */
            ...
            while ((count = ((*pUfstat & 0xf0) >> 4)) > 0 && (*pUcon & 0xc) != 0x0)
            {                           /* handling TX FIFO */
                if (pUfifo->txcnt > 0)
                {
                    pUfifo->txcnt -= 1;
        ...
        case UFCON1:
            {
                ...
                *pUfcon = (data & ~0x6);
                ...
                if (!((data & 0x1) == 0 || (data & 0x6) == 0))
                {
                    ...
                    if (data & 0x2)
                        *pUfstat &= ~0x10f;     /* Rx FIFO reset */
                    if (data & 0x4)
                        *pUfstat &= ~0x2f0;     /* Tx FIFO reset */
                }
            }
            break;
...
static void s3c44b0x_timer_do_cycle(ARMul_State * state)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (s3c44b0x_timer_check_state(i, TIMER_OP_START) == 0)
            continue;
        ...
        if ((io.tcnt[i] -= min(io.tcnt[i], cnt_down)) > 0)
            continue;

        if (((io.tcfg1 >> 24) & 0xf) == i + 1)
        {                               /* BDMA request */
            do
            {
                if (s3c44b0x_dma_is_valid(3) != 1)
                    break;
                if (((DMA_CCNT(io.dma[3]) >> 30) & 0x3) != 0x1)
                    break;
        ...
        if (s3c44b0x_timer_check_state(i, TIMER_OP_RELOAD) == 0)
            continue;
    ...
    if (io.wtcon & 0x20)
    {
        ...
        if ((io.wtcnt -= min(io.wtcnt, cnt_down)) > 0)
            return;
...
static void s3c44b0x_rtc_do_cycle(ARMul_State * state)
{
    if ((io.rtcalm & 0x40) != 0 && (io.rtcalm & 0x3f) != 0)
    {
        ...
        if (gettimeofday(&curr_time, NULL) != 0 ||
            ((curr_timer = (time_t) ((long int) curr_time.tv_sec + io.rtc_offset)),
             (curr_tm = gmtime(&curr_timer))) == NULL)
            return;

        if (curr_tm->tm_sec >= io.rtc_alarm.tm_sec)
            almen |= 0x1;
        if (curr_tm->tm_min == io.rtc_alarm.tm_min)
            almen |= 0x2;
        if (curr_tm->tm_hour == io.rtc_alarm.tm_hour)
            almen |= 0x4;
        if (curr_tm->tm_mday == io.rtc_alarm.tm_mday)
            almen |= 0x8;
        if (curr_tm->tm_mon == io.rtc_alarm.tm_mon)
            almen |= 0x10;
        if (curr_tm->tm_year == io.rtc_alarm.tm_year)
            almen |= 0x20;
    ...
    if ((io.ticint & 0x80) != 0)
    {                                   /* WARNING: RTC tick is unexacting */
        if (io.tick_count > 0)
            io.tick_count -= 1;
        if (io.tick_count == 0)
            s3c44b0x_set_interrupt(INT_TICK);
    }
}
...
static void s3c44b0x_dma_proccess(ARMul_State * state, int index)
{
    ...
    if ((dal = ((src_addr >> 28) & 0x3)) == 0x0)
        return;
    if ((das = ((dst_addr >> 28) & 0x3)) == 0x0)
        return;
    if ((dst = ((src_addr >> 30) & 0x3)) == 0x3)
        return;
    if (i < 2 && dst != 0x2 && ((ccnt >> 26) & 0x3) == 0x2)
        return;
    if ((opt_tdm = ((dst_addr >> 30) & 0x3)) == 0x0 && i >= 2)
        return;
    ...
    if ((ccnt & 0xfffff) >= (1 << dst))
    {
        ...
        if (((ccnt >> 26) & 0x3) != 0x3 || ((ccnt >> 24) & 0x3) != 0x2)
        {                               /* read */
            ...
            if (dal == 1)
                src_addr += min(0xfffffff - (src_addr & 0xfffffff), 1 << dst);
            else if (dal == 2)
                src_addr -= min((src_addr & 0xfffffff), 1 << dst);
        ...
        {                               /* write */
            if (((ccnt >> 26) & 0x3) == 0x3)
                data = DMA_FLY(io.dma[i]);

            (*write_func) (state, (dst_addr & 0xfffffff), data);

            if (das == 1)
                dst_addr += min(0xfffffff - (dst_addr & 0xfffffff), 1 << dst);
            else if (das == 2)
                dst_addr -= min((dst_addr & 0xfffffff), 1 << dst);
    ...
    {                                   /* terminated count */
        ccnt &= ~0xfffff;
        if ((ccnt & 0x200000) == 0x0)
            ccnt &= ~0x100000;          /* clear EN bit */
        DMA_CON(io.dma[i]) &= ~0x30;
        DMA_CON(io.dma[i]) |= 0x20;
        if (((ccnt >> 22) & 0x3) > 1)
            s3c44b0x_set_interrupt(INT_ZDMA0 - i);
    } else
    {
        ccnt -= (1 << dst);
        if (((ccnt >> 22) & 0x3) == 2)
...
static int s3c44b0x_dma_is_valid(int index)
{
    ...
    if ((DMA_CON(io.dma[index]) & 0xc) != 0x0)
        return -1;
    if (((ccnt = DMA_CCNT(io.dma[index])) & 0x100000) == 0x0)
        return -1;

    if (index < 2)
    {                                   /* ZDMA */
        if (((ccnt >> 30) & 0x3) < 0x2)
            return 1;                   /* DMA request, till now it's impossible in simulation */
    } else
    {                                   /* BDMA */
        if (((ccnt >> 26) & 0x3) == 0x3)
            return -1;                  /* don't support on-the-fly mode */
        if (((ccnt >> 30) & 0x3) != 0x0)
            return 1;                   /* DMA request */
    }
...
static void s3c44b0x_dma_write(ARMul_State * state, ARMword addr, ARMword data)
{
    ...
    if ((addr & 0xff) % 0x20 == 0x00)
    {                                   /* CON */
        *val = ((*val & 0x30) | (data & ~0x33));
        if ((data & 0x3) == 0x3)
            DMA_CSRC((*dma)) &= ~0x100000;      /* clear EN bit */
        return;
    } else if ((addr & 0xff) % 0x20 < 0x10)
    {                                   /* ISRC, IDES, ICNT */
        *val = data;
        *(val + 3) = data;
        return;
...
static int s3c44b0x_iis_write_to_device(ARMul_State * state, ARMhword * buf, int count)
{
    ...
    if (buf != s3c44b0x_iisfifo_tx)
        memcpy(s3c44b0x_iisfifo_tx, buf, count * 2);
...
static void s3c44b0x_iis_do_cycle(ARMul_State * state)
{
    ...
    while (((io.iismod >> 7) & 0x1) && (io.iisfifcon & 0x2f0) > 0x200)
    {                                   /* handling TX FIFO */
        int nWritten = -1;
    ...
    while (((io.iismod >> 6) & 0x1) && ((io.iisfifcon >> 8) & 0x1))
    {                                   /* handling RX FIFO */
        ARMhword tmp;

        if ((count = io.iisfifcon & 0xf) >= 8)
            break;
        if (s3c44b0x_iis_read_from_device(state, &tmp) != 0)
            break;
    ...
    while ((io.iiscon & 0x30) != 0)
    {                                   /* BDMA request */
        int rx_empty = ((io.iisfifcon & 0x100) ? ((io.iisfifcon & 0xf) == 0) : 0);
        int tx_ready = ((io.iisfifcon & 0x100) ? (((io.iisfifcon >> 4) & 0xf) < 8) : 1);
        ARMword mask = 0;

        if (s3c44b0x_dma_is_valid(2) != 1)
            break;
        if (((DMA_CCNT(io.dma[2]) >> 30) & 0x3) != 0x1)
            break;

        if ((DMA_CDES(io.dma[2]) & 0xfffffff) == IISFIF + state->bigendSig * 2 && tx_ready)
            mask |= (0x1 << 5);
        if ((DMA_CSRC(io.dma[2]) & 0xfffffff) == IISFIF + state->bigendSig * 2 && !rx_empty)
            mask |= (0x1 << 4);
        if ((io.iiscon & mask) != 0)
            s3c44b0x_dma_proccess(state, 2);
...
static void s3c44b0x_iis_read(ARMul_State * state, ARMword addr, ARMword * data)
{
    switch (addr)
    {
        ...
        case IISFIF:
            if (!((io.iiscon & 0x1) && ((io.iismod >> 6) & 0x1)))
                break;
            if ((io.iisfifcon & 0x100) != 0)
            {                           /* FIFO */
                ...
                if (count > 0)
                {
                    *data = io.iisfifo_rx[0];
                    io.iisfifcon = ((io.iisfifcon & ~0xf) | (--count));
                    if (count > 0)
                        memmove(&io.iisfifo_rx[0], &io.iisfifo_rx[1], count * 2);
                    io.iiscon |= 0x40;
                }
            } else
            {                           /* non FIFO */
...
static void s3c44b0x_iis_write(ARMul_State * state, ARMword addr, ARMword data)
{
    ...
    switch (addr)
    {
        ...
        case IISFIF:
            if (!((io.iiscon & 0x1) && ((io.iismod >> 7) & 0x1)))
                break;
            tmp = data & 0xffff;
            if ((io.iisfifcon & 0x200) != 0)
            {                           /* FIFO */
