...
static void lpc_update_int(ARMul_State * state)
{
    u32 irq = 0;
...
static void lpc_io_reset(ARMul_State * state)
{
...
ARMword lpc_fix_int(ARMword val)
{
    /*      ARMword ret = 0;
     * if (val & (1 << 2))
     * ret |= URXINT;
...
ARMword lpc_unfix_int(ARMword val)
{
    /*      ARMword ret = 0;
     * if (val & URXINT)
     * ret |= (1 << 2);
...
ARMword lpc_uart_read(ARMul_State * state, ARMword addr, int i)
{
    ARMword data;
...
void lpc_uart_write(ARMul_State * state, ARMword addr, ARMword data, int i)
{
...
ARMword lpc_io_read_byte(ARMul_State * state, ARMword addr)
{
...
ARMword lpc_io_read_halfword(ARMul_State * state, ARMword addr)
{
    return lpc_io_read_word(state, addr);
    //SKYEYE_OUTREGS(stderr);
    //exit(-1);
        ...
        case 0xe01fc100:
            ...
            switch (io.apbdiv & 3)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    skyeye_config.mach->io_cycle_divisor = 2;
                    break;
#endif
                default:
                    break;              /* no effect */
...
void lpc_io_write_byte(ARMul_State * state, ARMword addr, ARMword data)
{
...
void lpc_io_write_halfword(ARMul_State * state, ARMword addr, ARMword data)
{
    lpc_io_write_word(state, addr, data);
    //SKYEYE_OUTREGS(stderr);
    //exit(-1);
...
void lpc_mach_init(ARMul_State * state, machine_config_t * this_mach)
{
