/////////////////////////////////////////////////////////////////////////
// $Id: ne2k2.h 11148 2012-04-23 17:06:19Z vruppert $
/////////////////////////////////////////////////////////////////////////
//
...
#ifndef BX_IODEV_NE2K2
#define BX_IODEV_NE2K2

#if BX_USE_NE2K2_SMF
#  define BX_NE2K2_SMF  static
#  define BX_NE2K2_THIS theNE2k2Device->
#  define BX_NE2K2_THIS_PTR theNE2k2Device
#else
#  define BX_NE2K2_SMF
#  define BX_NE2K2_THIS this->
#  define BX_NE2K2_THIS_PTR this
#endif

#define  BX_NE2K2_MEMSIZ    (32*1024)
#define  BX_NE2K2_MEMSTART  (16*1024)
#define  BX_NE2K2_MEMEND    (BX_NE2K2_MEMSTART + BX_NE2K2_MEMSIZ)

class eth_pktmover_c;

typedef struct {
    //
    // ne2k2 register state
    ...
    Bit8u  mem[BX_NE2K2_MEMSIZ];  // on-chip packet memory

    // ne2k2 internal state
    Bit32u  base_address;
    int     base_irq;
    int     tx_timer_index;
...
class bx_ne2k2_c : public bx_devmodel_c
#if BX_SUPPORT_PCI
...
public:
  ...
  Bit32u (*page0_fj_read_handler)(void* this_ptr, Bit32u address, unsigned io_len);
private:
  bx_ne2k2_t s;

  eth_pktmover_c *ethdev;

  BX_NE2K2_SMF Bit32u read_cr(void);
  BX_NE2K2_SMF void   write_cr(Bit32u value);
  BX_NE2K2_SMF void   set_irq_level(bx_bool level);

  BX_NE2K2_SMF Bit32u chipmem_read(Bit32u address, unsigned io_len) BX_CPP_AttrRegparmN(2);
  BX_NE2K2_SMF Bit32u asic_read(Bit32u offset, unsigned io_len) BX_CPP_AttrRegparmN(2);
  BX_NE2K2_SMF Bit32u page0_read(Bit32u offset, unsigned io_len);
  BX_NE2K2_SMF Bit32u page1_read(Bit32u offset, unsigned io_len);
  BX_NE2K2_SMF Bit32u page2_read(Bit32u offset, unsigned io_len);
  BX_NE2K2_SMF Bit32u page3_read(Bit32u offset, unsigned io_len);

  BX_NE2K2_SMF void chipmem_write(Bit32u address, Bit32u value, unsigned io_len) BX_CPP_AttrRegparmN(3);
  BX_NE2K2_SMF void asic_write(Bit32u address, Bit32u value, unsigned io_len);
  BX_NE2K2_SMF void page0_write(Bit32u address, Bit32u value, unsigned io_len);
  BX_NE2K2_SMF void page1_write(Bit32u address, Bit32u value, unsigned io_len);
  BX_NE2K2_SMF void page2_write(Bit32u address, Bit32u value, unsigned io_len);
  BX_NE2K2_SMF void page3_write(Bit32u address, Bit32u value, unsigned io_len);

  static void tx_timer_handler(void *);
  BX_NE2K2_SMF void tx_timer(void);

  static Bit32u rx_status_handler(void *arg);
  BX_NE2K2_SMF Bit32u rx_status(void);
  static void rx_handler(void *arg, const void *buf, unsigned len);
  BX_NE2K2_SMF unsigned mcast_index(const void *dst);
  BX_NE2K2_SMF void rx_frame(const void *buf, unsigned io_len);

#if BX_SUPPORT_PCI
  BX_NE2K2_SMF bx_bool mem_read_handler(bx_phy_address addr, unsigned len, void *data, void *param);
  BX_NE2K2_SMF bx_bool mem_write_handler(bx_phy_address addr, unsigned len, void *data, void *param);
#endif
...
#endif
  static Bit32u page0_fj_read(void *this_ptr, Bit32u address, unsigned io_len); /* for netcard register fault injection */
};
