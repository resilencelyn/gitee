...
void bx_devices_c::init(BX_MEM_C *newmem)
{
  ...
  if (pci.enabled) {
    chipset = SIM->get_param_enum(BXPN_PCI_CHIPSET)->get();
        ...
        PLUG_load_plugin(bu_61580, PLUGTYPE_OPTIONAL);
#endif

#if BX_SUPPORT_SJA1000
        PLUG_load_plugin(sja1000, PLUGTYPE_OPTIONAL);
#endif


#if BX_SUPPORT_GPIO
        PLUG_load_plugin(gpio, PLUGTYPE_OPTIONAL);
#endif

#if BX_SUPPORT_AD
        PLUG_load_plugin(ad, PLUGTYPE_OPTIONAL);
#endif

#if BX_SUPPORT_RS422
        PLUG_load_plugin(rs422, PLUGTYPE_OPTIONAL);
#endif
#if BX_SUPPORT_PCI
