...

{
  ...
  if (setjmp (context) == 0) {
    ...
    const char* ci_name = "textconfig";
    ci_param->set_by_name(ci_name);
...
uint32 x86_get_regval_by_id(int id){
	...
	switch(id){
		...
		case 8:
                        ...
                        break;
		case 9:
            ret = BX_CPU_THIS_PTR read_eflags();
            break;
        case 10:
...
exception_t x86_set_register_by_id(int reg, uint32 value){
        //state->Reg[id] = value;
        switch(reg)
        {
                      case 0:
...
char * x86_get_register_name(int id)
{
    return (char*)x86_regstr[id];
}

int x86_get_register_id(char * name)
{
    int i;    
    for(i =0; i < 16; i++){
...
void
x86_step_once (conf_object_t* conf_obj){
    if (BX_SMP_PROCESSORS == 1) {
      // only one processor, run as fast as possible by not messing with
      // quantums and loops.
     // while (1) {
        BX_CPU(0)->skyeye_cpu_loop(conf_obj);
        if (bx_pc_system.kill_bochs_request)
          //break;
	  return;
      //}
...
void bx_init_hardware()
{
  ...
  BX_INFO(("  NE2002 support: %s", BX_SUPPORT_NE2K2?"yes":"no"));
