...
extern void set_x86_timer_handler_low(bool isLow);
/* 
 *
 * ===  FUNCTION  ======================================================================
 *         Name:  x86_set_pc
...
endian_t x86_core_get_endian(conf_object_t* conf_obj){
        return Little_endian;
}

void core_reset(conf_object_t* conf_obj)
{
	x86_core_t *x86_state = (x86_core_t*)conf_obj->obj;
...
exception_t x86_memory_write(conf_object_t* obj, generic_address_t addr, void* buf, size_t count){
	BX_MEM(0)->dmaWritePhysicalPage(addr, count, (uint8_t*)buf);
	return No_exp;
}
...
uint32_t x86_get_regnum(conf_object_t* obj){
        return 16;
}
...
char* x86_get_regname_by_id(conf_object_t* conf_obj, uint32_t id){
        assert((id >= 0) && (id <= 16));
                return (char*)x86_get_register_name(id);
...
uint32 x86_get_regid_by_name(conf_object_t* conf_obj, char* name)
{
	...
	return x86_get_register_id(name);
}
...
static int x86_insert_bp(conf_object_t* conf_obj, generic_address_t addr){
    ...
    insert_breakpoint(addr); //insert bp in bochs
	if(ret != 0)
		return ret;
	return 0;
}
...
static int x86_delete_bp(conf_object_t* conf_obj, generic_address_t addr){
    ...
    remove_breakpoint(addr,1);  //remove bp in bochs
	if(ret != 0)
		return ret;
	return 0;
}
...
extern unsigned long long get_total_us();
uint64_t x86_get_usec(conf_object_t *obj)
{
//	uint64_t us = get_total_us();
	uint64_t us = bx_pc_system.time_usec();
	return us;
}

void set_x86_timer_low(conf_object_t *obj, bool flag)
{
	bool timer_isLow = flag;

	set_x86_timer_handler_low(timer_isLow);
	return;
}

void x86_get_run_status(conf_object_t *obj,uint32_t* cpu_freq,uint64_t* cpu_insn_num){
	uint32_t default_freq_hz = 100000000;
	*cpu_freq = default_freq_hz;
	cpu_insn_num = x86_get_usec(obj);
	return ;
}
uint32_t x86_get_address_width(conf_object_t *obj){
    return 4;
}
...
void x86_core_register_interface(conf_class_t* clss) {
	static const core_info_intf core_info = {
		...
		get_endian : x86_core_get_endian,
		get_alignment : NULL,
		get_architecture : x86_get_architecture,
		reset : x86_core_reset,
		disassemble : x86_core_disassemble,
		get_usec: x86_get_usec,
		set_x86_timer_low: set_x86_timer_low,
		get_cpu_run_status: x86_get_run_status,
		get_address_width: x86_get_address_width,
	};
	SKY_register_iface(clss, CORE_INFO_INTF_NAME, &core_info);

	static const x86_core_intf x86_core_iface = {
		x86_reset : core_reset,
	};
	SKY_register_iface(clss, X86_CORE_INTF_NAME, &x86_core_iface);
