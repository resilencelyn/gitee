...
/*
static inline bool_t in_ccsr_range(e500_core_t *current_core, uint32 p)
{
 ...
 */
int e500_effective_to_physical(e500_core_t * current_core, uint32 addr, int flags, uint32 * result)
{
    *result = addr;
    return PPC_MMU_OK;
    int i, j;
    ...
    PPC_CPU_State *cpu = (PPC_CPU_State *) current_core->cpu;
    if ((cpu->bptr & 0x80000000) && (addr >> 12 == 0xFFFFF))
    {                                   /* if bootpage translation enabled? */
    ...
    while (i++ < (L2_TLB0_SIZE + L2_TLB1_SIZE))
    {
        ...
        if (entry->size != 0xb)
        {
            ...
            {
                if (current_core->msr & 0x4000)
                {                       /* Pr =1 , we are in user mode */
                    if (!(entry->usxrw & 0x8))
                    {
                        //printf("In %s,usermode,offset=0x%x, entry->usxrw=0x%x,pc=0x%x\n", __FUNCTION__, i, entry->usxrw, current_core->pc);
                        ppc_exception(current_core, DATA_ST, flags, addr);
                        return PPC_MMU_EXC;
                    }
                } else
                {                       /* Or PR is 0,we are in Supervisor mode */
                    if (!(entry->usxrw & 0x4))
                    {                   /* we judge SW bit */
    ...
    {
        ppc_exception(current_core, INSN_TLB, flags, addr);
        return PPC_MMU_EXC;
    } else
    {
        if (ppc_exception(current_core, DATA_TLB, flags, addr))
            return PPC_MMU_EXC;
    }
    ...
    *result = addr;
    ...
    } else
    {
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            if (ppc_read_physical_word(core, pteg_addr, &pte))
            {
                if (!(flags & PPC_MMU_NO_EXC))
                {
            ...
            if ((pte & PTE1_V) && (!(pte & PTE1_H)))
            {
                ...
                {
                    // page found
                    if (ppc_read_physical_word(core, pteg_addr + 4, &pte))
                    {
                        if (!(flags & PPC_MMU_NO_EXC))
                        {
                            PPC_MMU_ERR("read physical in address translate failed\n");
                    ...
                    *result = pap | offset;
                    ...
                    ppc_write_physical_word(core, pteg_addr + 4, pte);
                    return PPC_MMU_OK;
                }
            }
            pteg_addr += 8;
...
int ppc_direct_effective_memory_handle(e500_core_t *current_core, uint32 addr, uint8 * ptr)
{
    uint32 ea;
    int r;
...
int ppc_direct_effective_memory_handle_code(e500_core_t *current_core, uint32 addr, uint8 * ptr)
{
    uint32 ea;
    int r;
...
inline int ppc_read_physical_qword(e500_core_t *current_core, uint32 addr, Vector_t * result)
{
    uint64 vl = 0, vh = 0;
    int ret = ppc_read_physical_dword(current_core, addr, &vl);
    if (ret == PPC_MMU_OK)
    {
        ret = ppc_read_physical_dword(current_core, addr + 8, &vh);
        if (ret != PPC_MMU_OK)
        {
            printf("In %s, Line:%d Read addr:%x Failed!\n", __func__, __LINE__, addr);
        }
    } else
    {
        printf("In %s, Line:%d Read addr:%x Failed!\n", __func__, __LINE__, addr);
...
inline int ppc_read_physical_dword(e500_core_t *core, uint32 addr, uint64 * result)
{
    //int ret = io_mem_read64(addr, result);
    uint32 u32_l = 0, u32_h = 0;
    int ret = ppc_read_physical_word(core, addr, &u32_l);

    if (ret == PPC_MMU_OK)
    {
        ret = ppc_read_physical_word(core, addr + 4, &u32_h);
        if (ret != PPC_MMU_OK)
        {
            printf("In %s, Line:%d Read addr:%x Failed!\n", __func__, __LINE__, addr);
        }
...
int ppc_read_physical_word(e500_core_t *core, uint32 addr, uint32 * result)
{
    ...
    {
        ret = core->memory_ccsr_iface->read(core->memory_ccsr, (addr - core->get_ccsr_base(core->cpu->ccsr)), &r_data, 4);
    } else
    {
        ret = core->core_memory_iface->read(core->obj, addr, &r_data, 4);
    }
...
inline int ppc_read_physical_half(e500_core_t *core, uint32 addr, uint16 * result)
{
    //uint32 r;
    ...
    if (in_ccsr_range(core, addr))
    {
        ret = core->memory_ccsr_iface->read(core->memory_ccsr, (addr - core->get_ccsr_base(core->cpu->ccsr)), &r_data, 2);
    } else
    {
        ret = core->core_memory_iface->read(core->obj, addr, &r_data, 2);
    }
...
inline int ppc_read_physical_byte(e500_core_t *core, uint32 addr, uint8 * result)
{
    //uint32 r;
    ...
    if (in_ccsr_range(core, addr))
    {
        ret = core->memory_ccsr_iface->read(core->memory_ccsr, (addr - core->get_ccsr_base(core->cpu->ccsr)), &r_data, 1);
    } else
    {
        ret = core->core_memory_iface->read(core->obj, addr, &r_data, 1);
    }
...
inline int ppc_read_effective_code(e500_core_t *current_core, uint32 addr, uint32 * result)
{
    if (addr & 3)
    {
        // EXC..bla
    ...
    {
        return ppc_read_physical_word(current_core, p, result);
    }
    return r;
}

inline int ppc_read_effective_qword(e500_core_t *current_core, uint32 addr, Vector_t * result)
{
    uint32 p;
    int r;
    ...
    {
        return ppc_read_physical_qword(current_core, p, result);
    }

    return r;
}

inline int ppc_read_effective_dword(e500_core_t *current_core, uint32 addr, uint64 * result)
{
    PPC_CPU_State *cpu = (PPC_CPU_State *)current_core->cpu;
    uint32 p;
    int r;
    ...
    if (!(r = ppc_effective_to_physical(current_core, addr, PPC_MMU_READ, &p)))
    {
        ...
        if (EA_Offset(addr) > 4088)
        {
            ...
            if ((r = ppc_direct_physical_memory_handle(p, r1)))
                return r;
            if ((r = ppc_effective_to_physical((addr & ~0xfff) + 4096, PPC_MMU_READ, &p)))
                return r;
    ...
    {
        core->memory_ccsr_iface->read(core->memory_ccsr, (p - current_core->get_ccsr_base(cpu->ccsr)), &l_data, 4);
        core->memory_ccsr_iface->read(core->memory_ccsr, (p - current_core->get_ccsr_base(cpu->ccsr) + 4), &h_data, 4);
        r_data = ((uint64_t) h_data << 32) | l_data;
        *result = r_data;
    } else
    {
        core->core_memory_iface->read(core->obj, p, &h_data, 4);
        core->core_memory_iface->read(core->obj, p + 4, &l_data, 4);
        r_data = ((uint64_t) h_data << 32) | l_data;
...
inline int ppc_write_physical_qword(e500_core_t *current_core, uint32 addr, Vector_t * data)
{
    uint64 vl = data->d[0];
    uint64 vh = data->d[1];

    int ret = ppc_write_physical_dword(current_core, addr, vl);
    if (ret == PPC_MMU_OK)
    {
        ret = ppc_write_physical_dword(current_core, addr + 8, vh);
        if (ret != PPC_MMU_OK)
        {
            printf("In %s, Line:%d Write addr:%x Failed!\n", __func__, __LINE__, addr);
        }
    } else
    {
        printf("In %s, Line:%d Write addr:%x Failed!\n", __func__, __LINE__, addr);
...
inline int ppc_write_physical_dword(e500_core_t *core, uint32 addr, uint64 data)
{
    /*
     ...
     * }
     */
    uint32_t h_data, l_data;

    h_data = data >> 32;
    l_data = data & 0xFFFFFFFF;

    if (in_ccsr_range(core, addr))
    {
        core->memory_ccsr_iface->write(core->memory_ccsr, (addr - core->get_ccsr_base(core->cpu->ccsr)), &l_data, 4);
        core->memory_ccsr_iface->write(core->memory_ccsr, (addr - core->get_ccsr_base(core->cpu->ccsr) + 4), &h_data, 4);
    } else
    {
        core->core_memory_iface->write(core->obj, addr, &h_data, 4);
        core->core_memory_iface->write(core->obj, addr + 4, &l_data, 4);
    }
    return PPC_MMU_OK;
}

inline int ppc_write_physical_word(e500_core_t *core, uint32 addr, uint32 data)
{
    int ret = PPC_MMU_OK;

    if (in_ccsr_range(core, addr))
    {
        ret = core->memory_ccsr_iface->write(core->memory_ccsr, (addr - core->get_ccsr_base(core->cpu->ccsr)), &data, 4);
    } else
    {
        ret = core->core_memory_iface->write(core->obj, addr, &data, 4);
    }

    return ret;
}

inline int ppc_write_effective_qword(e500_core_t *current_core, uint32 addr, Vector_t data)
{
    uint32 p;
    int r;
    ...
    {
        return ppc_write_physical_qword(current_core, p, &data);
    }
    return r;
}

inline int ppc_write_effective_dword(e500_core_t *current_core, uint32 addr, uint64 data)
{
    uint32 p;
    int r;
    ...
    if (!((r = ppc_effective_to_physical(current_core, addr, PPC_MMU_WRITE, &p))))
    {
        ...
        if (EA_Offset(addr) > 4088)
        {
            ...
            if ((r = ppc_direct_physical_memory_handle(p, r1)))
                return r;
            if ((r = ppc_effective_to_physical(current_core, (addr & ~0xfff) + 4096, PPC_MMU_WRITE, &p)))
                return r;
            if ((r = ppc_direct_physical_memory_handle(p, r2)))
                return r;
            memmove(&b[0], r1, 7);
        ...
        {
            return ppc_write_physical_dword(current_core, p, data);
        }
#endif
        return ppc_write_physical_dword(current_core, p, data);
    }
 ...
 */
bool_t ppc_prom_set_sdr1(e500_core_t *current_core, uint32 newval, bool_t quiesce)
{
    return ppc_mmu_set_sdr1(current_core, newval, quiesce);
}

bool_t ppc_prom_effective_to_physical(e500_core_t *current_core, uint32 * result, uint32 ea)
{
    return ppc_effective_to_physical(current_core, ea,
                                     PPC_MMU_READ | PPC_MMU_SV | PPC_MMU_NO_EXC, result) == PPC_MMU_OK;
}

bool_t ppc_prom_page_create(e500_core_t *current_core, uint32 ea, uint32 pa)
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            if (ppc_read_physical_word(current_core, pteg_addr, &pte))
            {
                PPC_MMU_ERR("read physical in address translate failed\n");
                return false;
            }
            if (!(pte & PTE1_V))
            {
                ...
                pte2 = (PA_RPN(pa) << 12) | 0;
                if (ppc_write_physical_word(current_core, pteg_addr, pte) || ppc_write_physical_word(current_core, pteg_addr + 4, pte2))
                {
                    return false;
                } else
                {
...
void ppc_opc_dcbz(e500_core_t *current_core)
{
    ...
    for (; i < 32; i += 4)
        ppc_write_effective_word(current_core, a + i, 0);
    ...
    ppc_write_effective_dword(current_core, a, 0)
        || ppc_write_effective_dword(current_core, a + 8, 0)
        || ppc_write_effective_dword(current_core, a + 16, 0) || ppc_write_effective_dword(current_core, a + 24, 0);
#endif
}
...
void ppc_opc_lbz(e500_core_t *current_core)
{
    ...
    uint32 r;
    int ret = ppc_read_effective_byte(current_core, (rA ? current_core->gpr[rA] : 0) + imm, &r);
...
void ppc_opc_lbzux(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0 && rA!=rD
    uint32 r;
    int ret = ppc_read_effective_byte(current_core, current_core->gpr[rA] + current_core->gpr[rB], &r);
...
void ppc_opc_lbzx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rD, rA, rB);
    uint32 r;
    int ret = ppc_read_effective_byte(current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], &r);
...
void ppc_opc_lfd(e500_core_t *current_core)
{
    ...
    uint64 r;
    int ret = ppc_read_effective_dword(current_core, (rA ? current_core->gpr[rA] : 0) + imm,
                                       &r);
...
void ppc_opc_lfdu(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0
    uint64 r;
    int ret = ppc_read_effective_dword(current_core, current_core->gpr[rA] + imm, &r);
...
void ppc_opc_lfdux(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0
    uint64 r;
    int ret = ppc_read_effective_dword(current_core, current_core->gpr[rA] + current_core->gpr[rB], &r);
...
void ppc_opc_lfdx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, frD, rA, rB);
    uint64 r;
    int ret = ppc_read_effective_dword(current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], &r);
...
void ppc_opc_lfs(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, frD, rA, imm);
    uint32 r;
    int ret = ppc_read_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) + imm, &r);
    ...
    if (ret == PPC_MMU_OK)
    {
        ...
        ppc_fpu_pack_double(current_core, &d, &(current_core->fpr[frD]));
    }
}
...
void ppc_opc_lfsu(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0
    uint32 r;
    int ret = ppc_read_effective_word(current_core, current_core->gpr[rA] + imm, &r);
...
void ppc_opc_lfsux(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0
    uint32 r;
    int ret = ppc_read_effective_word(current_core, current_core->gpr[rA] + current_core->gpr[rB], &r);
...
void ppc_opc_lfsx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, frD, rA, rB);
    uint32 r;
    int ret = ppc_read_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], &r);
...
void ppc_opc_lha(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, rD, rA, imm);
    uint32 r;
    int ret = ppc_read_effective_half(current_core, (rA ? current_core->gpr[rA] : 0) + imm, &r);
...
void ppc_opc_lhaux(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rD, rA, rB);
    uint32 r;
    int ret = ppc_read_effective_half(current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], &r);
...
void ppc_opc_lhbrx(e500_core_t *current_core)
{
    ...
    if (ret == PPC_MMU_OK)
    {
        current_core->gpr[rD] = half_from_BE(r);
    }
}
...
void ppc_opc_lmw(e500_core_t *current_core)
{
    ...
    while (rD <= 31)
    {
        if (ppc_read_effective_word(current_core, ea, &(current_core->gpr[rD])))
        {
            return;
        }
        rD++;
...
void ppc_opc_lswi(e500_core_t *current_core)
{
    ...
    while (NB > 0)
    {
        ...
        if (ppc_read_effective_byte(current_core, ea, &v))
        {
            return;
        }
        r <<= 8;
...
void ppc_opc_lwbrx(e500_core_t *current_core)
{
    ...
    if (ret == PPC_MMU_OK)
    {
        current_core->gpr[rD] = word_from_BE(r);
    }
}
 ...
 *      v.127
 */
void ppc_opc_lvx(e500_core_t *current_core)
{
    ...
    int ret = ppc_read_effective_qword(current_core, ea, &r);
...
void ppc_opc_lvebx(e500_core_t *current_core)
{
    ...
    int ret = ppc_read_effective_byte(current_core, ea, &r);
...
void ppc_opc_lvehx(e500_core_t *current_core)
{
    ...
    int ret = ppc_read_effective_half(current_core, ea, &r);
...
void ppc_opc_lvewx(e500_core_t *current_core)
{
    ...
    int ret = ppc_read_effective_word(current_core, ea, &r);
...
void ppc_opc_stb(e500_core_t *current_core)
{
    ...
    ppc_write_effective_byte(current_core, (rA ? current_core->gpr[rA] : 0) + imm, (uint8) current_core->gpr[rS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stbu(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, rS, rA, imm);
    // FIXME: check rA!=0
    int ret = ppc_write_effective_byte(current_core, current_core->gpr[rA] + imm,
...
void ppc_opc_stbux(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0
    int ret = ppc_write_effective_byte(current_core, current_core->gpr[rA] + current_core->gpr[rB],
                                       (uint8) current_core->gpr[rS]);
...
void ppc_opc_stbx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB);
    ppc_write_effective_byte(current_core, (rA ? current_core->gpr[rA] : 0) +
                             current_core->gpr[rB], (uint8) current_core->gpr[rS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stfd(e500_core_t *current_core)
{
    ...
    ppc_write_effective_dword(current_core, (rA ? current_core->gpr[rA] : 0) + imm, current_core->fpr[frS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stfdu(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, frS, rA, imm);
    // FIXME: check rA!=0
    int ret = ppc_write_effective_dword(current_core, current_core->gpr[rA] + imm,
...
void ppc_opc_stfdux(e500_core_t *current_core)
{
    ...
    // FIXME: check rA!=0
    int ret = ppc_write_effective_dword(current_core, current_core->gpr[rA] + current_core->gpr[rB],
                                        current_core->fpr[frS]);
...
void ppc_opc_tlbsx(e500_core_t *current_core)
{
    ...
    while (i++ < (L2_TLB0_SIZE + L2_TLB1_SIZE))
    {
        ...
        if ((ea & mask) == (entry->epn << 12))
        {
            ...
            current_core->mmu.mas[1] |= 0x80000000;
            current_core->mmu.mas[1] |= entry->size << 8;
...
void ppc_opc_tlbrehi(e500_core_t *current_core)
{
    ...
    current_core->mmu.mas[3] = (current_core->mmu.mas[3] & 0xFFFFFFC3FLL) | (entry->u << 6);
    current_core->mmu.mas[3] = (current_core->mmu.mas[3] & 0xFFF) | (entry->rpn << 12);
...
void ppc_opc_stfdx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, frS, rA, rB);
    ppc_write_effective_dword(current_core, (rA ? current_core->gpr[rA] : 0) +
                              current_core->gpr[rB], current_core->fpr[frS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stfiwx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, frS, rA, rB);
    ppc_write_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) +
                             current_core->gpr[rB], (uint32) current_core->fpr[frS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stfs(e500_core_t *current_core)
{
    ...
    ppc_fpu_pack_single(current_core, &d, &s);
    ppc_write_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) + imm, s) != PPC_MMU_FATAL;
}
...
void ppc_opc_stfsu(e500_core_t *current_core)
{
    ...
    ppc_fpu_unpack_double(&d, current_core->fpr[frS]);
    ppc_fpu_pack_single(current_core, &d, &s);
    int ret = ppc_write_effective_word(current_core, current_core->gpr[rA] + imm, s);
...
void ppc_opc_stfsux(e500_core_t *current_core)
{
    ...
    ppc_fpu_unpack_double(&d, current_core->fpr[frS]);
    ppc_fpu_pack_single(current_core, &d, &s);
    int ret = ppc_write_effective_word(current_core, current_core->gpr[rA] + current_core->gpr[rB], s);
...
void ppc_opc_stfsx(e500_core_t *current_core)
{
    ...
    ppc_fpu_unpack_double(&d, current_core->fpr[frS]);
    ppc_fpu_pack_single(current_core, &d, &s);
    ppc_write_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], s) != PPC_MMU_FATAL;
}
...
void ppc_opc_sth(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, rS, rA, imm);
    ppc_write_effective_half(current_core, (rA ? current_core->gpr[rA] : 0) + imm, (uint16) current_core->gpr[rS]) != PPC_MMU_FATAL;
    /*if(current_core->pc >= 0xfff830e4 && current_core->pc <= 0xfff83254)
     * printf("DBG:in %s,pc=0x%x\n", __FUNCTION__, current_core->pc);
     */
}
...
void ppc_opc_sthbrx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB);
    ppc_write_effective_half(current_core, (rA ? current_core->gpr[rA] : 0) +
                             current_core->gpr[rB], (half_from_BE(current_core->gpr[rS]))) != PPC_MMU_FATAL;
}
...
void ppc_opc_sthu(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, rS, rA, imm);
    // FIXME: check rA!=0
    int ret = ppc_write_effective_half(current_core, current_core->gpr[rA] + imm,
...
void ppc_opc_sthux(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB);
    // FIXME: check rA!=0
    int ret = ppc_write_effective_half(current_core, current_core->gpr[rA] + current_core->gpr[rB],
                                       (uint16) current_core->gpr[rS]);
...
void ppc_opc_sthx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB);
    ppc_write_effective_half(current_core, (rA ? current_core->gpr[rA] : 0) +
                             current_core->gpr[rB], (uint16) current_core->gpr[rS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stmw(e500_core_t *current_core)
{
    ...
    while (rS <= 31)
    {
        if (ppc_write_effective_word(current_core, ea, current_core->gpr[rS]))
        {
            return;
        }
        rS++;
...
void ppc_opc_stswi(e500_core_t *current_core)
{
    ...
    while (NB > 0)
    {
        ...
        if (ppc_write_effective_byte(current_core, ea, (r >> 24)))
        {
            return;
        }
        r <<= 8;
...
void ppc_opc_stw(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, rS, rA, imm);
    ppc_write_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) + imm, current_core->gpr[rS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stwbrx(e500_core_t *current_core)
{
    ...
    ppc_write_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) +
                             current_core->gpr[rB], word_from_BE(current_core->gpr[rS])) != PPC_MMU_FATAL;
}
...
void ppc_opc_stwcx_(e500_core_t *current_core)
{
    ...
    if (current_core->have_reservation)
    {
        ...
        if (ppc_read_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], &v))
        {
            return;
        }
        if (v == current_core->reserve)
        {
            if (ppc_write_effective_word
                (current_core, (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB], current_core->gpr[rS]))
            {
                return;
            }
...
void ppc_opc_stwu(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_D_SImm(current_core->current_opc, rS, rA, imm);
    // FIXME: check rA!=0
    int ret = ppc_write_effective_word(current_core, current_core->gpr[rA] + imm,
                                       current_core->gpr[rS]);
...
void ppc_opc_stwux(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB);
    // FIXME: check rA!=0
    int ret = ppc_write_effective_word(current_core, current_core->gpr[rA] + current_core->gpr[rB],
                                       current_core->gpr[rS]);
...
void ppc_opc_stwx(e500_core_t *current_core)
{
    ...
    PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB);
    ppc_write_effective_word(current_core, (rA ? current_core->gpr[rA] : 0) +
                             current_core->gpr[rB], current_core->gpr[rS]) != PPC_MMU_FATAL;
}

/*      stvx        Store Vector Indexed
 *      v.134
 */
void ppc_opc_stvx(e500_core_t *current_core)
{
#ifndef __VEC_EXC_OFF__
    if ((current_core->msr & MSR_VEC) == 0)
    {
    ...
    ppc_write_effective_qword(current_core, ea, current_core->vr[vrS]) != PPC_MMU_FATAL;
}
...
void ppc_opc_stvebx(e500_core_t *current_core)
{
    ...
    ea = (rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB];
    ppc_write_effective_byte(current_core, ea, VECT_B(current_core->vr[vrS], ea & 0xf));
}
...
void ppc_opc_stvehx(e500_core_t *current_core)
{
    ...
    ea = ((rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB]) & ~1;
    ppc_write_effective_half(current_core, ea, VECT_H(current_core->vr[vrS], (ea & 0xf) >> 1));
}
...
void ppc_opc_stvewx(e500_core_t *current_core)
{
    ...
    ea = ((rA ? current_core->gpr[rA] : 0) + current_core->gpr[rB]) & ~3;
    ppc_write_effective_word(current_core, ea, VECT_W(current_core->vr[vrS], (ea & 0xf) >> 2));
}
