...
#define PPC_OPC_TEMPL_XI(opc, A, B) {(A)=((opc)>>21)&0x1f; (B)=((opc)>>16)&0x1f;} // for rD,rA,///
#define PPC_OPC_TEMPL_IX(opc, A, B) {(A)=((opc)>>21)&0x1f; (B)=((opc)>>11)&0x1f;} // for rD,///,rB
#define PPC_OPC_TEMPL_X(opc, A, B, C) {(A)=((opc)>>21)&0x1f; (B)=((opc)>>16)&0x1f; (C)=((opc)>>11)&0x1f;} // for rD,rA,rB
#define PPC_OPC_TEMPL_CRX(opc, A, B, C) {(A)=((opc)>>23)&0x7; (B)=((opc)>>16)&0x1f; (C)=((opc)>>11)&0x1f;} // for crfD,rA,rB
#define PPC_OPC_TEMPL_XT(opc, A, B, C, D) {(A)=(opc>>21)&0x1f; (B)=(opc>>16)&0x1f; (C)=(opc>>11)&0x1f; (D)=opc&0x7;} // for rD,rA,rB,crS
