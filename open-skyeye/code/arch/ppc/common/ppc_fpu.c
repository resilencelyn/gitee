...
inline void ppc_fpu_add(e500_core_t *current_core, ppc_double * res, ppc_double * a, ppc_double * b)
{
...
inline void ppc_fpu_add_quadro(e500_core_t *current_core, ppc_quadro * res, ppc_quadro * a, ppc_quadro * b)
{
...
// FPU however...
inline void ppc_fpu_mul_add(e500_core_t *current_core, ppc_double * res, ppc_double * m1, ppc_double * m2, ppc_double * s)
{
    ppc_quadro p;
    ...
    ppc_fpu_add_quadro(current_core, &x, &p, &q);
    // x = [107]
...
inline void ppc_fpu_sqrt(e500_core_t *current_core, ppc_double * D, const ppc_double * B)
{
    switch (B->type)
    {
        case ppc_fpr_norm:
            ...
            *D = *B;
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                ppc_fpu_add(current_core, D, &D_old, &B_div_D_old);
                D->e--;
...
void ppc_fpu_test(e500_core_t *current_core)
{
    ppc_double A, B, C;
    double a, b, c;
    ...
    printf("%f + %f = \n", a, b);
    ppc_fpu_add(current_core, &C, &A, &B);
    uint64 d;
    uint32 s;

    ppc_fpu_pack_double_as_single(current_core, &C, &d);
    ht_printf("%064qb\n", d);
    ppc_fpu_unpack_double(&C, d);
    ppc_fpu_pack_single(current_core, &C, &s);
 ...
 *      .484
 */
void ppc_opc_fabsx(e500_core_t *current_core)
{
    int frD, frA, frB;
 ...
 *      .485
 */
void ppc_opc_faddx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
    ...
    ppc_fpu_add(current_core, &D, &A, &B);
    current_core->fpscr |= ppc_fpu_pack_double(current_core, &D, &(current_core->fpr[frD]));
    if (current_core->current_opc & PPC_OPC_Rc)
    {
        // update cr1 flags
        PPC_FPU_ERR("fadd.\n");
 ...
 *      .486
 */
void ppc_opc_faddsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
    ...
    ppc_fpu_add(current_core, &D, &A, &B);
    current_core->fpscr |= ppc_fpu_pack_double_as_single(current_core, &D, &(current_core->fpr[frD]));
    if (current_core->current_opc & PPC_OPC_Rc)
    {
        // update cr1 flags
        PPC_FPU_ERR("fadds.\n");
...
void ppc_opc_fcmpo(e500_core_t *current_core)
{
    int crfD, frA, frB;
 ...
 *      .489
 */
void ppc_opc_fcmpu(e500_core_t *current_core)
{
    int crfD, frA, frB;
 ...
 *      .492
 */
void ppc_opc_fctiwx(e500_core_t *current_core)
{
    int frD, frA, frB;
    ...
    ppc_fpu_unpack_double(&B, current_core->fpr[frB]);
    current_core->fpr[frD] = ppc_fpu_double_to_int(current_core, &B);
    if (current_core->current_opc & PPC_OPC_Rc)
    {
        // update cr1 flags
        PPC_FPU_ERR("fctiw.\n");
 ...
 *      .493
 */
void ppc_opc_fctiwzx(e500_core_t *current_core)
{
    int frD, frA, frB;
 ...
 *      .494
 */
void ppc_opc_fdivx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .495
 */
void ppc_opc_fdivsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .496
 */
void ppc_opc_fmaddx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
    ...
    ppc_fpu_mul_add(current_core, &D, &A, &C, &B);
    current_core->fpscr |= ppc_fpu_pack_double(current_core, &D, &(current_core->fpr[frD]));
    if (current_core->current_opc & PPC_OPC_Rc)
    {
        // update cr1 flags
 ...
 *      .497
 */
void ppc_opc_fmaddsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .498
 */
void ppc_opc_fmrx(e500_core_t *current_core)
{
    int frD, rA, frB;
 ...
 *      .499
 */
void ppc_opc_fmsubx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .500
 */
void ppc_opc_fmsubsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .501
 */
void ppc_opc_fmulx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .502
 */
void ppc_opc_fmulsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .503
 */
void ppc_opc_fnabsx(e500_core_t *current_core)
{
    int frD, frA, frB;
 ...
 *      .504
 */
void ppc_opc_fnegx(e500_core_t *current_core)
{
    int frD, frA, frB;
 ...
 *      .505
 */
void ppc_opc_fnmaddx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .506
 */
void ppc_opc_fnmaddsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .507
 */
void ppc_opc_fnmsubx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .508
 */
void ppc_opc_fnmsubsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .509
 */
void ppc_opc_fresx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .511
 */
void ppc_opc_frspx(e500_core_t *current_core)
{
    int frD, frA, frB;
    ...
    ppc_fpu_unpack_double(&B, current_core->fpr[frB]);
    current_core->fpscr |= ppc_fpu_pack_double_as_single(current_core, &B, &(current_core->fpr[frD]));
    if (current_core->current_opc & PPC_OPC_Rc)
    {
        // update cr1 flags
        PPC_FPU_ERR("frsp.\n");
 ...
 *      .512
 */
void ppc_opc_frsqrtex(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .514
 */
void ppc_opc_fselx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .515
 */
void ppc_opc_fsqrtx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .515
 */
void ppc_opc_fsqrtsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .517
 */
void ppc_opc_fsubx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
 ...
 *      .518
 */
void ppc_opc_fsubsx(e500_core_t *current_core)
{
    int frD, frA, frB, frC;
