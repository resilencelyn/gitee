...
uint32 ppc_fpu_round(e500_core_t *current_core, ppc_double * d)
{
    // .132
...
uint32 ppc_fpu_round_single(e500_core_t *current_core, ppc_single * s)
{
    switch (FPSCR_RN(current_core->fpscr))
    {
        case FPSCR_RN_NEAR:
...
uint32 ppc_fpu_round_double(e500_core_t *current_core, ppc_double * s)
{
    switch (FPSCR_RN(current_core->fpscr))
    {
        case FPSCR_RN_NEAR:
...
uint32 ppc_fpu_pack_double(e500_core_t *current_core, ppc_double * d, uint64 * res)
{
    ...
    switch (d->type)
    {
        case ppc_fpr_norm:
            ...
            if (d->e > 0)
            {
                ret |= ppc_fpu_round(current_core, d);
                if (d->m & (1ULL << 56))
                {
                    d->e++;
                    d->m >>= 4;
...
uint32 ppc_fpu_pack_single(e500_core_t *current_core, ppc_double * d, uint32 * res)
{
    ...
    switch (d->type)
    {
        case ppc_fpr_norm:
            ...
            if (d->e > 0)
            {
                ret |= ppc_fpu_round_double(current_core, d);
                if (d->m & (1ULL << 27))
                {
                    d->e++;
                    d->m >>= 4;
...
uint32 ppc_fpu_pack_double_as_single(e500_core_t *current_core, ppc_double * d, uint64 * res)
{
    ...
    switch (s.type)
    {
        case ppc_fpr_norm:
            ...
            if (s.e > 0)
            {
                ret |= ppc_fpu_round_single(current_core, &s);
                if (s.m & (1 << 27))
                {
                    s.e++;
                    s.m >>= 4;
    ...
    //      ht_printf("dm: %qx\n", d.m);
    ret |= ppc_fpu_pack_double(current_core, d, res);
    return ret;
}
