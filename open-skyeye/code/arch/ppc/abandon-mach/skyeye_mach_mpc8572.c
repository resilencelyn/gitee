    ...
    if ((!(io->mpic.iivpr[UART_IRQ] & 0x80000000) && (core->msr & 0x8000) && !(core->ipr & 1 << UART_IRQ)))
    {
        ...
        if ((!(uart->iir & 0x1)) && (uart->ier & 0x3))
        {
            ...
            io->pic_percpu.iack[core_id] =
                (io->pic_percpu.iack[core_id] & 0xFFFF0000) | (io->mpic.ipivpr[UART_IRQ] & 0xFFFF);
        ...
        switch (offset)
        {
            case 0x60040:
                ...
                if (data & 0x3)
                {
                    ...
                    io->pic_percpu.iack[core_id] =
                        (io->pic_percpu.iack[core_id] & 0xFFFF0000) | (io->mpic.ipivpr[0] & 0xFFFF);
...
void mpc8572_mach_init(void *arch_instance, machine_config_t * this_mach)
{
