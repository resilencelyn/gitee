...
#define MAX_IDLE_COUNT 10
  ...
  out_of_recv:
    ...
    if (cpm->scc[0].gsmrl & 0x00000010)
    {
        ...
        short bd_flag = (*(sint16 *) & ram[trans_bd_base + curr_tx_bd]);
        short bd_len = (*(sint16 *) & ram[trans_bd_base + curr_tx_bd + 2]);
        uint32 buf_addr = (*((uint32 *) & ram[trans_bd_base + curr_tx_bd + 4]));
...
static uint32_t mpc8560_io_read_word(void *state, uint32_t offset)
{
    ...
    if (offset >= 0x80000 && offset < 0x8C000)
    {
        return (*((sint32 *) & io->cpm_reg.dpram[offset - 0x80000]));
        //printf("DBG_CPM:in %s,offset=0x%x,data=0x%x,pc=0x%x\n",__FUNCTION__, offset, *result,io->pc);
    }
...
void mpc8560_mach_init(void *arch_instance, machine_config_t * this_mach)
{
