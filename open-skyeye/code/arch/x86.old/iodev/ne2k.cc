...
static int ne2k_did = 0xFFFFFFFF;
static char* ne2k_chipram_regstr[] = {"chipmem"};
static unsigned int ne2k_chipram_regaddr[] = {0x10};
...
Bit32u bx_ne2k_c::page0_read(Bit32u offset, unsigned int io_len)
{
  ...
  default:
	...
	int val = x86_devs_r_plug(ne2k_did, offset, value, 1);
...
Bit32u bx_ne2k_c::chipram_read(void *this_ptr, Bit32u address, unsigned io_len){
  Bit32u retval = 0;
...
/* copied from chipmem_read */
Bit32u bx_ne2k_c::page0_fj_read(void *this_ptr, Bit32u address, unsigned io_len){
...
#if 0 /* inject fault to data packet */
	read_ram = chipram_read;
	printf("in %s, read_ram =0x%x, chipram_read=0x%x\n", __FUNCTION__, read_ram, chipram_read);
	//BX_INFO("############ in ne2k read_ram =0x%x, \n",  read_ram);
      ne2k_did = x86_add_device(BX_PLUGIN_NE2K, LOG_THIS read_ram, theNE2kDevice , 1, BX_NE2K_MEMEND, ne2k_chipram_regstr, ne2k_chipram_regaddr);
#endif
	/* inject the fault into page0 registers */
	page0_fj_read_handler = page0_fj_read;
      ne2k_did = x86_add_device(BX_PLUGIN_NE2K, LOG_THIS page0_fj_read_handler, theNE2kDevice , 0xf, 1, page0_regstr, page0_regaddr);
}
