...
static unsigned int serial_regaddr[]={
	BX_SER_RBR,BX_SER_IER,BX_SER_IIR,BX_SER_LCR,BX_SER_MCR,BX_SER_LSR,BX_SER_MSR,BX_SER_SCR
};
static unsigned int serial_did = 0;
}
...
bx_serial_c::init(void)
{
  ...
  for (i=0; i<BX_N_SERIAL_PORTS; i++) {
    ...
    if (SIM->get_param_bool("enabled", base)->get()) {
      ...
      read_regs = read_handler;
      serial_did = x86_add_device(BX_PLUGIN_SERIAL, LOG_THIS read_regs, theSerialDevice ,8, 1, serial_regstr, serial_regaddr);

#if 0
	/* for debug */
      {
      x86_list_devices();
      printf("reg_num is 0x%x\n", x86_devs_get_reg_num(0));
      printf("reg_num is 0x%x\n", x86_devs_get_reg_width(0));
      int d;
      for(d = 0; d < x86_devs_get_reg_num(0); d ++){
     	printf("%s = %d\n",x86_devs_get_reg_name_by_id(0,d),x86_devs_get_reg_value_by_id(0,d));
      }
      }
#endif
...
Bit32u bx_serial_c::read(Bit32u address, unsigned io_len)
{
  ...
  val = x86_devs_r_plug(serial_did,offset, val, 1);
