...
void x86_devs_save_fin(int d_id,unsigned int addr, int bitnum, int mode)
{
	...
	//skyeye_log(Info_log, __FUNCTION__, "1 addr=0x%x, \n", addr);
...
int x86_devs_r_plug(int d_id, unsigned int addr, int buf,unsigned int count)
{
	...
	//skyeye_log(Info_log, __FUNCTION__, "1 addr=0x%x, buf=0x%x, fstart=0x%x, fend=0x%x\n", addr, buf, fstart, fend);
	...
	//skyeye_log(Info_log, __FUNCTION__, "2 addr=0x%x, buf=0x%x\n", addr, buf);
	return buf;
