 ...
 *       Filename:  i_fsubs.c
 *
 *    Description:  Implementation of the fsubs instruction
 *
 *        Version:  1.0
 *        Created:  12/11/08
 *       Revision:  none
...
#define FSUBS_CYCLES    1
#define FSUBS_CODE_MASK 0X81A008A0
#define PRIVILEDGE  0
...
sparc_instruction_t i_fsubs = { " i_fsubs",
    execute,
    disassemble,
    FSUBS_CODE_MASK,
    PRIVILEDGE,
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & FSUBS_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs1 = bits(instr, RS1_OFF_last, RS1_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
