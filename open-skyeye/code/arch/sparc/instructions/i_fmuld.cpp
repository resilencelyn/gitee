 ...
 *       Filename:  i_fdivd.c
 *
 *    Description:  Implementation of the FMULD instruction
 *
 *        Version:  1.0
 *        Created:  12/11/08
 *       Revision:  none
 ...
 * 31-30  29-25   24-19     18-14       13-5             4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 |   rd  | 110100 |   rs1   |     001001010      |   rs2  |
 * +----+-------+--------+------------------------------------------+
 */
#include "skyeye_fp.h"
...
#define FMULD_CYCLES    1
#define FMULD_CODE_MASK  0x81A00940
#define PRIVILEDGE  0
...
sparc_instruction_t i_fmuld = { " i_fmuld",
    execute,
    disassemble,
    FMULD_CODE_MASK,
    PRIVILEDGE,
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & FMULD_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs1 = bits(instr, RS1_OFF_last, RS1_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
