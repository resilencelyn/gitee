 ...
 * 31-30  29-25   24-19     18-14       13-5             4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 |   rd  | 110100 |   NBZ   |     011001000      |   rs2  |
 * +----+-------+--------+------------------------------------------+
 */
#include "skyeye_fp.h"
...
#define FITOD_CYCLES    1
#define FITOD_CODE_MASK	0X81A01900
#define PRIVILEDGE  0
...
sparc_instruction_t i_fitod = { " i_fitod",
    execute,
    disassemble,
    FITOD_CODE_MASK,
    PRIVILEDGE,
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & FITOD_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
