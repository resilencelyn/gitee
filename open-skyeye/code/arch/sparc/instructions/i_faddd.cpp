...
#define FADDD_CYCLES    1
#define FADDD_CODE_MASK 0X81A00840
#define PRIVILEDGE  0
...
sparc_instruction_t i_faddd = { " i_faddd",
    execute,
    disassemble,
    FADDD_CODE_MASK,
    PRIVILEDGE,
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if (((instr & FADDD_CODE_MASK) == FADDD_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs1 = bits(instr, RS1_OFF_last, RS1_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
