 ...
 *       Filename:  i_fstoi.c
 *
 *    Description:  Implementation of the FSTOI instruction
 *
 *        Version:  1.0
 *        Created:  12/11/08
 *       Revision:  none
 ...
 * 31-30  29-25   24-19     18-14       13-5             4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 | 00000 | 110100 |   NBZ   |     011010001      |   rs2  |
 * +----+-------+--------+------------------------------------------+
 * 
 */
#include "skyeye_fp.h"
...
#define FSTOI_CYCLES    1
#define FSTOI_CODE_MASK 0x81a00920
#define PRIVILEDGE  0
...
sparc_instruction_t i_fstoi = { " i_fstoi",
    execute,
    disassemble,
    FSTOI_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    print_fpinst_RS_RS("fstoi", rs2, rd);
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & FSTOI_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        fmt = bits(instr, FMT_OFF_last, FMT_OFF_first);
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
