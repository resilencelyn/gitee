 ...
 * 31-30  29-25   24-19     18-14    13        12-5          4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 | 00000 | 010011 |   rs1   | i=0 |   non-used   |    rs2    |
 * +----+-------+--------+------------------------------------------+
 * 
 */
...
#define XORCC_CYCLES    1
#define XORCC_CODE_MASK 0x80b90000
#define PRIVILEDGE  1
...
sparc_instruction_t i_xorcc = { " i_xorcc",
    execute,
    disassemble,
    XORCC_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    if (imm < 0)
    {
        ...
        print_inst_RS_RS_RD("xorcc", rs1, rs2, rd);
    } else
    {
        REG(rd) = REG(rs1) ^ (sign_ext13(imm));
        print_inst_RS_IM13_RD("xorcc", rs1, sign_ext13(imm), rd);
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & XORCC_CODE_MASK) && (op == OP) && (op3 == OP3))
    {
        int rd, rs1, rs2, imm;
