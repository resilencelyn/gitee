...
#define MULSCC_CYCLES    1
#define MULSCC_CODE_MASK 0x81200000
#define PRIVILEDGE  0
...
sparc_instruction_t i_mulscc = { " i_mulscc",
    execute,
    disassemble,
    MULSCC_CODE_MASK,
    PRIVILEDGE,
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & MULSCC_CODE_MASK) && (op == OP) && (op3 == OP3))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs1 = bits(instr, RS1_OFF_last, RS1_OFF_first);
        int i = bit(instr, I_OFF);
