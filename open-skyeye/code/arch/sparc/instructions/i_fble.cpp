 ...
 *       Filename:  i_fble.c
 *
 *    Description:  Implementation of the FBLE instruction
 *
 *        Version:  1.0
...
#define FBLE_CYCLES    1
#define FBLE_CODE_MASK 0x1b800000
#define PRIVILEDGE  0
...
sparc_instruction_t i_fble = { " i_fble",
    execute,
    disassemble,
    FBLE_CODE_MASK,
    PRIVILEDGE,
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if (((instr & FBLE_CODE_MASK) == FBLE_CODE_MASK) && (op == OP) && (cond == COND) && (fmt == FMT))
    {
        disp22 = bits(instr, DISP22_OFF_last, DISP22_OFF_first);
        return 1;
    }
