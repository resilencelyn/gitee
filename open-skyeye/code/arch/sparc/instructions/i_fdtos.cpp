 ...
 * 31-30  29-25   24-19     18-14       13-5             4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 |   rd  | 110100 |   NBZ   |     011000110      |   rs2  |
 * +----+-------+--------+------------------------------------------+
 */
#include "skyeye_fp.h"
...
#define FDTOS_CYCLES    1
#define FDTOS_CODE_MASK	0X81A018C0
#define PRIVILEDGE  0
...
sparc_instruction_t i_fdtos = { " i_fdtos",
    execute,
    disassemble,
    FDTOS_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    va_rd.value = (float) (va_rs2.value);
    result = (va_rd.value - va_rs2.value);
#if 0
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & FDTOS_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
