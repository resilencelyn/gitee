...
sparc_instruction_t i_rett = { " i_rett",
    execute,
    disassemble,
    RETT_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    if ((ET == 1) && (S == 0) && sparc_state->is_irq == 0)
    {
        traps->signal(sparc_state->obj, PRIVILEGED_INSTR);
        return (RETT_CYCLES);
    } else if ((ET == 1) && (S == 1) && sparc_state->is_irq == 0)
    {
    ...
    } else if ((ET == 0))
    {
        ...
        {
            traps->signal(sparc_state->obj, WUF);
            return (RETT_CYCLES);
    ...
    if (sparc_state->is_irq && ET == 1)
    {
        sparc_state->is_irq = 0;
    }
    //    DBG("rett 0x%x\n", daddr);
    print_inst_BICC("rett", daddr);

    /*  Add 1 to the CWP    */
    iu_add_cwp(sparc_state);
