...
#define LDA_CYCLES    1
#define LDA_CODE_MASK 0xc0800000
#define PRIVILEDGE  0
...
sparc_instruction_t i_lda = { " i_lda",
    execute,
    disassemble,
    LDA_CODE_MASK,
    PRIVILEDGE,
...
uint32_t l1cache_reg_read(sparc_state_t *sparc_state, uint32_t addr)
{
    uint32_t value = 0;
    switch (addr)
    {
...
uint32_t asi_read_word_access_process(sparc_state_t * sparc_state, int asi, uint32_t addr)
{
    ...
    {
        case 0x1:
            sparc_memory_read_word32(conf_obj, &value, addr);
            break;
        case 0x2:
            //l1cache reg read
            value = l1cache_reg_read(sparc_state, addr);
            break;
        case 0x3:
        ...
        case 0x1c:
            //mmu bypass
            {
                ...
                iface->read(conf_obj, addr, &value, 4);
            }
            break;
        case 0x20:
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    /*  Check the word alignment. The address MULDA be WORD aligned. That means
    ...
    //    DBG("st reg[%d], [ 0x%x ]\n", rd, addr);
    print_inst_RD_IMM("lda", rd, addr);
    conf_object_t *conf_obj = sparc_state->obj;

    value = asi_read_word_access_process(sparc_state, asi, addr);
    /*
     * if( status < 0 )
     * {
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & LDA_CODE_MASK) && (op == OP) && (op3 == OP3))
    {
        op3 = bits(instr, OP3_OFF_last, OP3_OFF_first);
        if (op3 != OP3)
            return 0;
