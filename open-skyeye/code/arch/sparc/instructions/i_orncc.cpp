 ...
 * 31-30  29-25   24-19     18-14    13        12-5          4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 | 00000 | 010110 |   rs1   | i=0 |   non-used   |    rs2    |
 * +----+-------+--------+------------------------------------------+
 * 
 */
...
#define ORNCC_CYCLES    1
#define ORNCC_CODE_MASK 0x80b00000
#define PRIVILEDGE  0
...
sparc_instruction_t i_orncc = { " i_orncc",
    execute,
    disassemble,
    ORNCC_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    if (imm < 0)
    {
        ...
        print_inst_RS_RS_RD("orncc", rs1, rs2, rd);
    } else
    {
        y = sign_ext13(imm);
        z = REG(rd) = (REG(rs1) | ~sign_ext13(imm));
        //        DBG("orcc reg[%d], 0x%x, reg[%d]\n", rs1, imm, rd);
        print_inst_RS_IM13_RD("orncc", rs1, sign_ext13(imm), rd);
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & ORNCC_CODE_MASK) && (op == OP) && (op3 == OP3))
    {
        int rd, rs1, rs2, imm;
