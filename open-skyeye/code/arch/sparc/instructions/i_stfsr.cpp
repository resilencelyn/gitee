...
sparc_instruction_t i_stfsr = { " i_stfsr",
    execute,
    disassemble,
    STFSR_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    status = sparc_memory_store_word32(conf_obj, addr, FPSRREG & 0xffffffff);

    if (status < 0)
    {
        traps->signal(sparc_state->obj, get_sparc_exp(sparc_state));
        return (STFSR_CYCLES);
    }
