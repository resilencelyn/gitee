 ...
 *       Filename:  i_fdivs.c
 *
 *    Description:  Implementation of the FDIVS instruction
 *
 *        Version:  1.0
 *        Created:  12/11/08
 *       Revision:  none
...
#define FDIVS_CYCLES    1
#define FDIVS_CODE_MASK 0x84a409d0
#define PRIVILEDGE  0
...
sparc_instruction_t i_fdivs = { " i_fdivs",
    execute,
    disassemble,
    FDIVS_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
...
#ifdef __WIN32__
            va_rd.data = 0x7fffe000;
#else
            va_rd.data = 0x7fffe000UL;
#endif
...
#ifdef __WIN32__
            va_rd.data = 0x7ff00000;
#else
            va_rd.data = 0x7ff00000UL;
#endif
    ...
    } else
    {
        ...
        } else if (z < S_min.value)
        {
            ...
            print_inst_RS_RS_RD("fdivs", rs1, rs2, rd);
#endif
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & FDIVS_CODE_MASK) && (op == OP) && (opf == OPF) && (fmt == FMT))
    {
        rd = bits(instr, RD_OFF_last, RD_OFF_first);
        rs1 = bits(instr, RS1_OFF_last, RS1_OFF_first);
        rs2 = bits(instr, RS2_OFF_last, RS2_OFF_first);
