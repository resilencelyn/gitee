 ...
 *       Filename:  i_taddcc.c
 *
 *    Description:  Implementation of the TADDCC instruction
 *
 ...
 * 31-30  29-25   24-19     18-14    13        12-5          4-0
 * +----+-------+--------+------------------------------------------+
 * | 10 | 00000 | 100000 |   rs1   | 1/0 |      simm12/rs2          |
 * +----+-------+--------+------------------------------------------+
 * 
 */
...
#define TADDCC_CYCLES    1
#define TADDCC_CODE_MASK 0x81000000
#define PRIVILEDGE  0
...
sparc_instruction_t i_taddcc = { " i_taddcc",
    execute,
    disassemble,
    TADDCC_CODE_MASK,
    PRIVILEDGE,
...
static int execute(sparc_state_t * sparc_state, uint32 instr)
{
    ...
    if (imm < 0)
    {
        ...
        y = REG(rs2);
        print_inst_RS_RS_RD("taddcc", rs1, rs2, rd);
    } else
    {
        z = REG(rd) = (REG(rs1) + sign_ext13(imm));
        y = sign_ext13(imm);
        print_inst_RS_IM13_RD("taddcc", rs1, sign_ext13(imm), rd);
    }
    /*
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if ((instr & TADDCC_CODE_MASK) && (op == OP) && (op3 == OP3))
    {
        int rd, rs1, rs2, imm;
