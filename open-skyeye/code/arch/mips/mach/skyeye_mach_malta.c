...
static void malta_io_do_cycle(void *state)
{
    if (!(io.int_ctrl[0].req0int & 0x1))
    {                                   /* if we already in interrupt handler? */
...
static UInt32 malta_io_read_byte(void *state, UInt32 addr)
{
    UInt32 ret;
...
static UInt32 malta_io_read_halfword(void *state, UInt32 addr)
{
    UInt32 ret;
...
static UInt32 malta_io_read_word(void *state, UInt32 addr)
{
...
static void malta_io_write_byte(void *state, UInt32 addr, UInt32 data)
{
...
static void malta_io_write_halfword(void *state, UInt32 addr, UInt32 data)
{
    MIPS_State *mstate = (MIPS_State *) state;
...
static void malta_io_write_word(void *state, UInt32 addr, UInt32 data)
{
#if 0
...
static void malta_update_intr(machine_config_t * mach)
{
}

static void malta_set_intr(UInt32 irq)
{
...
static void malta_io_reset(machine_config_t * mach)
{
...
void malta_mach_init(void *arch_instance, machine_config_t * this_mach)
{
    int i;
