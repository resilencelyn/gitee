        ...
        case SPECIAL:
            {
                ...
                switch (function(instr))
                {
                    ...
                    case JR:
                        {
                            ...
                            return (bits(mstate->gpr[rs(instr)], 1, 0) == 0x2) ? instr_addr_error : branch_delay;
                        }
                    case JALR:
                        {
                    ...
                    case MULT:
                        {
                            // Multiply
                            Int64 x = (int) mstate->gpr[rs(instr)];
                            Int64 y = (int) mstate->gpr[rt(instr)];
        ...
        case JALX:
            {
                ...
                mstate->branch_target = msb | (target(instr) << 2) | 0x1;
                if (mstate->pipeline == branch_delay)
                {
                    printf("Can't handle branch in branch delay slot");
                }
        ...
        case BEQL:
            {
                ...
                {
                    printf("beqzl \n");
                     /*BEQZL*/ VA off = sign_extend_UInt32(offset(instr), 16);
                    mstate->branch_target = mstate->pc + 4 + (off << 2);
                } else
#endif
                if (y == x)
        ...
        case BNEL:
            {
                ...
                {
                    printf("bnezl \n");
                     /*BNEZL*/ VA off = sign_extend_UInt32(offset(instr), 16);
                    mstate->branch_target = mstate->pc + 4 + (off << 2);
                } else
#endif
                if (y != x)
                    ...
                    case INS:
                        {
                            ...
                            if (pos >= 0 && size > 0 && pos + size <= 32)
                            {
                                ...
                                {
                                    midbits =
                                        mstate->gpr[rs(instr)] >> lsb << lsb << (32 - (msb + 1)) >> (32 - (msb + 1));
