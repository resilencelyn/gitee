...
static void Icache_lru_init(UInt8 * Icache_lru)
{
    UInt8 i;
...
static int Icache_lru_replace(UInt8 * Icache_lru)
{
    return Icache_lru[Icache_assoc - 1];
}

// Touch a cache line (mark it as most-recently used.)
static void Icache_lru_touch(UInt8 * Icache_lru, unsigned int n)
{
...
// Initialize the LRU data.
static void Dcache_lru_init(UInt8 * Dcache_lru)
{
    UInt8 i;
...
static int Dcache_lru_replace(UInt8 * Dcache_lru)
{
    return Dcache_lru[Dcache_assoc - 1];
}

// Touch a cache line (mark it as most-recently used.)
static void Dcache_lru_touch(UInt8 * Dcache_lru, int n)
{
    // Set lru[0] to n and shift the other values accordingly.
    if (Dcache_lru[0] != n)
    {
