...
{
    return mstate->process_exception(mstate, cause, vec);
}
...
/* Gs32eb1 Process a general exception
 *
 ...
 * machine state are handled in inline wrappers on gs32eb1_process_exception() (see
...
/**
* @brief gs32eb1 processor exception interface
*
* @param mstate
* @param cause
* @param vec
 ...
 * (events) word, Cause may be set directly to the exception code, adjusted
 * for exceptions in a branch-delay slot. Any additional changes to the
 * machine state are handled in inline wrappers on ls132r_process_exception() (see
 * "koala.hh".) Exceptions are serviced when the corresponding instruction
 * enters the W pipeline stage: at this stage. Hence, there are always
 * exactly five dead clock cycles after an exception: these are simulated
 * simply by incrementing the clock by five.
...
/**
* @brief ls132r processor exception interface
*
* @param mstate
* @param cause
* @param vec
...
void ls132r_process_exception(MIPS_State * mstate, UInt32 cause, int vec)
{
    ...
    // set isa_mode as 0
    mstate->isa_mode = 0;
    //BEV------“1”代表中断入口在 0xbf000380
    mstate->pc = vec + ls132r_vector_base;
    /* set EXL to one */
    mstate->cp0[SR] |= 0x2;
    /* Set Exl bit to zero, disable interrupt */
    mstate->cp0[SR] &= 0xFFFFFFFD;
