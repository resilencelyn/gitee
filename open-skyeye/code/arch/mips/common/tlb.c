...
void set_tlb_entry(MIPS_State * mstate, int n)
{
    if (mstate->mt_flag)
...
int translate_vaddr(MIPS_State * mstate, VA va, int type, PA * pa)
{
    ...
    else
    {
        ...
        if ((vaddr_region(va) >= vaddr_region(kuseg)) && (vaddr_region(va) < vaddr_region(kseg0)))
        {                               //KUSEG's most significant three bits are 0xx
            /* The user region is valid in every mode, but we still have
             * to handle the 32/64 bit mode differences.
             */
            region_type = KUSEG;
        } else if (vaddr_region(va) == vaddr_region(kseg0))
        {                               //KSEG0's most significant three bits are 100
            if (mstate->mode & umode)
            {                           //User mode access kernel space
        ...
        } else if (vaddr_region(va) == vaddr_region(kseg1))
        {                               //KSEG1's most significant three bits are 101
            if (mstate->mode & umode)
            {
                process_address_error(mstate, type, va);
                return TLB_FAIL;        // shut up the compiler
