...
<el-form ref="form" :model="fd" label-width="100px" :rules="rules">
  <el-form-item label="旧密码" prop="oldPassword">
    <el-input v-model="fd.oldPassword" type="password" autofocus show-password />
  </el-form-item>
  ...
  methods: {
    changePwd() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.loading = true
          const data = {oldPassword: md5(this.fd.oldPassword), newPassword: md5(this.fd.newPassword)}
          axios.post(`user/${this.$user.id}/changePwd`, data).then(res => {
            this.loading = false
            if (res.code === '00') {
