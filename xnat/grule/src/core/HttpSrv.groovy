...
//            }
    ...
    protected Map<String, Object> dbSessionDelegate(HttpContext hCtx) {
        ...
        UserSession session
        if (sessionId) {
            session = repo.byId(UserSession, sessionId)
            if (session) {
                // 一段时间不操作过期
                if (System.currentTimeMillis() - session.updateTime.time > expire.toMillis()) {
                    session.valid = false
                    async { // 删除过期session数据
                        final e = repo.row(UserSession) {root, query, cb ->
                            cb.and(
                ...
                // 如果一直操作, 则等超出continuousExpire时间后的下一天过期(在下一天第一次重新登录)
                    ...
                    (int) (System.currentTimeMillis() / (1000 * 60 * 60 * 24)) > (int) (session.updateTime.time / (1000 * 60 * 60 * 24))
                ) {
                    session.valid = false
                }
                else {
